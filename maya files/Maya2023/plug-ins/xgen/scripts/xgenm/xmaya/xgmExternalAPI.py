...
errWrapXgenDesc = maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenDesc' ]
errWrapXgenDeleteDesc = maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenDeleteDesc' ]
...
def setActive( palette, description, subtype, previewer=False ):
    ...
    if description=="":
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPICouldNotSetActiveType' ]+" ["+palette+":]");
        return
    cmds.file(modified=True)
...
def initInterpolation( palette, description, fileName="" ):
    ...
    # on the maya guides.
    if description=="":
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPICouldNotSetInitInterop' ]+" ["+palette+":]");
        return False
    ...
    try:
        ...
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIExceptionXgmInterpSetup' ])
    return False
...
def copyFileToNewDir( fileName, currentSceneName, sourceFileDir, currentSceneDir, basePaletteName, ns ):
    ...
    if not os.path.isfile(newPath) :
        ...
        if not success:
            xglog.XGError(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCannotCopyXgenFile'  ] % (oldPath, newPath) )
            ...
            while line and cmp(line, "endAttrs"):
                if line.startswith("xgDataPath"):
                    ...
                    if xgDataPath.startswith("${PROJECT}"):
                        ...
                        msg = maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kPalPathError' ]
                        xglog.XGError(msg )
                line = next(fp)
                line = line.strip(' \t\r\n')
...
def importPalette( fileName, deltas, nameSpace="", wrapPatches=True, wrapGuides=True, validator = None, grooming = False, validatePath = True ):
    ...
    if validatePath:
        ...
        for dir in dirs:
            ...
            if not os.path.exists(expandedPath):
                  msg = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kxgDataPathBroken' ] % (palName,dir)
                  xglog.XGError(msg )
    ...
    if ( pal == ""):
        ...
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenCol' ]+palName)
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenDeleteCol' ])
...
def setMessageLevel( type, level ):
    if ( type == "quitOnError" ):
        xglog.XGWarning(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPICannotModifyQuitOnError' ])
        return
...
def modifyFaceBinding( palette, description, mode="Append",
    ...
    """
    if palette=="":
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPINoColSpecifiedForBinding' ])
        return
    if description=="":
        xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPINoDescSpecifiedForBinding' ])
        return
    ...
    try:
        ...
        for desc in descs:
            ...
            for patch in patches:
                ...
                if len(conns) < 1:
                    xglog.XGError(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrFindingGeo' ]+patch)
                    continue
                geom = conns[0]
...
def bakedGroomManagerBake( pal, desc, pb=False ):
    ...
    # display results
    print(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIFilesBaked' ])
    print(files)
...
def renamePalette( oldName, newName ):
    ...
    def renamePalettePath( oldName, newName ):
        ...
        xglog.XGWarning(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kRenamePaletteFolder'  ] % (newPath,oldPath) )
    ...
    if not result:         
        xglog.XGError(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCannotRenamePalette'  ] % (oldName,newName) )
        raise

    if not validPalette == newName :
        xglog.XGWarning(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kPaletteClash'  ] % (newName,validPalette) )
    ...
    try:
        ...
        xglog.XGError(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCannotRenamePalette2'  ] % (oldName,newName) )
        raise
...
def renameDescription( palName, oldName, newName ):
    ...
    def renameDescriptionPath( palName, oldName, newName ):
        ...
        xglog.XGWarning(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kRenameDescriptionFolder'  ] % (newPath,oldPath) )
    ...
    cannotRenameDescriptionErr = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCannotRenameDescription'  ] % (oldName,newName) 
    if not result:         
        xglog.XGError(cannotRenameDescriptionErr)
        raise

    if not validDesc == newName :
        xglog.XGWarning(maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kDescriptionClash'  ] % (newName,validDesc) )
...
def validateDescription( action, palInfo, descInfo, uiParent=None ):
    ...
    if action == ADD_TO_NEW_PALETTE:
        ...
        for s in splitNS:
            if len(s) == 0:
                title = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCreateXgenDescription'  ]
                text = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kInvalidNamespace'  ]
                if uiParent:
    ...
    if selCount and (len(selNS) and len(palNS)) and (selNS != palNS):            
        title = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCreateXgenDescription2'  ]
        text = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCollectionNamespace'  ] % (palNS,selNS)
        if uiParent:
            mbox = QMessageBox(QMessageBox.NoIcon, title, text, QMessageBox.Ok, uiParent )
            mbox.exec_()
        else:
    ...
    if selCount and len(selNS)==0 and len(palNS):
        title = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kCreateXgenDescription3'  ]
        text = maya.stringTable[ 'y_xgenm_xmaya_xgmExternalAPI.kSelectedObjectsNotPartOfCollection'  ] % palNS
        if uiParent:
            mbox = QMessageBox(QMessageBox.NoIcon, title, text, QMessageBox.Ok, uiParent )
            mbox.exec_()
        else:
...
def undoable( name ):
    try:
        ...
        import traceback
        print(maya.stringTable['y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrRunningUndoable' ] % (name, e))
        traceback.print_exc()
    finally:
