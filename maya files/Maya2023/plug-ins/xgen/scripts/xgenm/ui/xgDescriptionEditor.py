    ...
    try:
        ...
        for filename in files:
            ...
            try:
                ...
                print(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kLoadedPathPython'  ] % (uipath, pyname))
            except Exception as e:
                print(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kFailedToLoadPathPython'  ] % (uipath, pyname, e))
                traceback.print_exc()
                continue
            ...
            for name in dir(pymodule):
                ...
                if name in classes:
                    print(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kWarningDefinedTwiceInDirectory'  ] % (name, uipath))
                classes[name] = cls
...
class PaletteExpressionUI(ExpressionUI):
    ...
    def __init__(self,attr):
        ...
        ExpressionUI.__init__(self,attr, maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kGlobalExpression'  ] % attr, "Palette","",isColor)
    ...
    def BuildHorizontalLayout(self):
        ...
        removeButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kRemoveAnn'  ])
...
class ExportUI(QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kXgenExport'  ])
        self.setSizeGripEnabled(True)
        self.setMinimumWidth(DpiScale(550))
        layout = QVBoxLayout()
        self.anim = CheckBoxUI(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kAnimatedXGenExport'  ],
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSetToTrueIfTheGeometryIsAnimated'  ])
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kFrameRange'  ])
        ...
        self.startFrame.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kFirstFrameAnn' ])
        ...
        self.endFrame.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kLastFrameAnn' ])
        ...
        self.exportButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportFileButton'  ])
        self.exportButton.setFixedWidth(DpiScale(100))
        self.exportButton.setAutoRepeat(False)
        self.exportButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportFileAnn'])
        self.connect(self.exportButton, QtCore.SIGNAL("clicked()"),
                     self.accept)
        hbox.addWidget(self.exportButton)
        self.cancelButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCancelButton'  ])
        self.cancelButton.setFixedWidth(DpiScale(100))
        self.cancelButton.setAutoRepeat(False)
        self.cancelButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCancelExportAnn' ])
...
class DescriptionEditorUI(QWidget):
    ...
    def __init__(self,parent = None,fl = Qt.Widget ):
        ...
        if xgg.Maya and cmds.about(batch=True):
            print('=============================================')
            xglog.XGError( maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kCreatingDEUIInBatch' ] )
            print(xgu.callStack())
            print('=============================================')
        ...
        self.setWindowTitle(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kXgenDescriptionEditor'  ])
    ...
    def buildMainUI(self):
        ...
        logExpand = ExpandUI(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kMainLog'  ],False)
    ...
    def buildWelcomeUI(self):
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kGettingStartedWithXgen'  ] % ("<b>","</b>"))
        #label.setStyleSheet('font-size: 15pt;')
        welcomeUILayout.addWidget(label)

        welcomeUILayout.addSpacing(DpiScale(10))
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kUseXgenToCreateArbitraryPrimitivesOnASurface'  ])
        #label.setStyleSheet('font-size: 11pt;')
        welcomeUILayout.addWidget(label)
        welcomeUILayout.addSpacing(DpiScale(5))
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kHairFurFeathersScalesRocksAndMore'  ])
        #label.setStyleSheet('font-size: 11pt;')
        welcomeUILayout.addWidget(label)

        welcomeUILayout.addSpacing(DpiScale(15))
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.ktoStart'  ] % ("<b>","</b>"))
        #label.setStyleSheet('font-size: 11pt;')
        welcomeUILayout.addWidget(label)

        welcomeUILayout.addSpacing(DpiScale(5))
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSelectTheGeometryOrFacesToCreatePrimitiveOn'  ])
        #label.setStyleSheet('font-size: 11pt;')
        label.setWordWrap( True ) 
        welcomeUILayout.addWidget(label)

        welcomeUILayout.addSpacing(DpiScale(5))
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCreateANewXgenDescription'  ])
        #label.setStyleSheet('font-size: 11pt;')
        label.setWordWrap( True ) 
        welcomeUILayout.addWidget(label)
        self.connect(addIconButtonLine(welcomeUILayout, "xgCreateDescription.png", maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCreateNewDescription'  ] ), QtCore.SIGNAL("clicked()"), self.createDescription)
        self.connect(addIconButtonLine(welcomeUILayout, "xgImportDescription.png", maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kImportDescription'  ] ), QtCore.SIGNAL("clicked()"), DescriptionEditorUI.importFileCB )
        self.connect(addIconButtonLine(welcomeUILayout, "xgLibrary.png", maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kWelcomeUIImportPreset'  ] ), QtCore.SIGNAL("clicked()"), self.openLibraryWindow )
    ...
    def createMenuBar(self):
        ...
        self.fileMenu = menubar.addMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kFileMenuName'  ])
        if ( xgg.Maya ):
            self.fileMenu.aboutToShow.connect( self.onFileMenuShow )
            self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kImportMenuBar'  ],importFile)
            self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kImportPresetMenuBar'  ],importPreset)
            self.importGrooming = self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kImportGroomingMenuBar'  ],igImportFile)
            self.fileMenu.addSeparator()
            self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportMenuBar'  ],exportFile)
            self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportPresetMenuBar'  ],exportPreset)        
            self.exportGrooming = self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportGroomingMenuBar'  ],igExportFile)
            self.fileMenu.addSeparator()
        self.fileMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportPatchesForBatchRender'  ], self.exportPatches)
        self.fileMenu.addSeparator()    
        if ( xgg.Maya ):
            self.fileMenu.addAction(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kExportSelectionAsArchives' ], lambda: mel.eval('XgExportArchive();') )
            self.fileMenu.addAction(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kBatchConvertScenesToArchives'], lambda: mel.eval('XgBatchExportArchive();') )
        else:
            self.fileMenu.addAction(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kExportSelectionAsArchives2'], self.exportArchives)
            self.fileMenu.addAction(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kBatchConvertScenesToArchives2'], self.batchExportArchives)
            
        self.palMenu = menubar.addMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCollectionMenuName'  ])
        self.palMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kEditFilePathPalMenu'  ], 
                   self.editPath)
        if ( xgg.Maya ):
            self.palMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kFixPatchNamesPalMenu'  ], 
                               lambda: xg.fixPatchNames(self.currentPalette()))
        self.palMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDeleteCollectionPalMenu'  ], 
                   self.deletePalette)

        self.descMenu = menubar.addMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDescriptionFileMenu'  ])
        self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCreateDescriptionDescMenu'  ], self.createDescription)
        if ( xgg.Maya ):
            self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDuplicateDescriptionDescMenu'  ], copyDescription)
            self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kMoveDescriptionToCollectionDescMenu'  ], moveDescription)
            self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSelectDescriptionDescMenu'  ], 
                                    lambda: cmds.select( self.currentDescription(), replace=True )  )
            self.descMenu.addSeparator()
            self.bindMenu1 = QMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kBindPatches'  ])
            self.addBindActions( self.bindMenu1, None ) 
            self.descMenu.addMenu(self.bindMenu1)
            self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kHideShowPatchesBasedOnGeometryVisibilityDescMenu'  ],
                 lambda: mel.eval("xgmSyncPatchVisibility"))

        self.descMenu.addSeparator()
        self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSetStrayPercentageDescMenu'  ],strayPercentage)
        self.descMenu.addSeparator()
        self.descMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDeleteDescriptionDescMenu'  ],self.deleteDescription)

        if ( xgg.Maya ):
            self.guidesMenu = menubar.addMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kGuidesFileMenu'  ])

            self.guidesMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kBakeGuideVerticesGuidesMenu'  ], 
                lambda: mel.eval("xgmBakeGuideVertices"))
            self.displayGuideRange = self.guidesMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDisplayGuideRangeOfInfluenceGuidesMenu'  ], 
                 self.toggleVisualizer)
            self.displayGuideRange.setCheckable(True)

            self.mrMenu = QMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kMentalrayFileMenu'  ])
            self.mrMenu.setVisible( False )
            self.mrMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSetupMentalrayGeometryShadersMentalrayMenu'  ], 
                 lambda: mel.eval("xgmr -setupGeometryShader -description \"%s\" -palette \"%s\"" % (self.currentDescription(), self.currentPalette() )))
            self.mrAutoGeo = self.mrMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kAutoSetupMentalrayGeometryShadersMentalrayMenu'  ], 
                 self.onAutoCreateMR )
            self.mrAutoGeo.setCheckable( True )
            self.updateMentalrayControls()
            self.mrMenu.addSeparator()
            self.mrMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kApplyHairShaderMentalrayMenu'  ], 
                 lambda: mel.eval("xgmr -applyShader \"hair\" -description \"%s\" -palette \"%s\"" % (self.currentDescription(), self.currentPalette )))

        self.logMenu = menubar.addMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kLogFileMenu'  ])
        self.logMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearLogMenu'  ],
                               lambda: self.messageUI.clearLog())

        self.logMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSaveLog'  ], lambda: self.messageUI.saveLog())
        val = self.getMessageLevel("warning")
        self.logWarnMenu = QMenu(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kWarningLevelLogWarnMenu' ] % str(val))
        self.logWarnMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.k0LogWarnMenu'  ],
        ...
        self.logWarnMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.k5LogWarnMenu'  ],
                                   lambda: self.setMessageLevel("warning",5))
        self.logMenu.addMenu(self.logWarnMenu)
        val = self.getMessageLevel("stats")
        self.logStatsMenu = QMenu(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kStatisticsLevelLogStatsMenu' ] % str(val))
        self.logStatsMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.k0LogStatsMenu'  ],
        ...
        self.logStatsMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.k5LogStatsMenu'  ],
                                   lambda: self.setMessageLevel("stats",5))
        self.logMenu.addMenu(self.logStatsMenu)
        val = self.getMessageLevel("debug")
        self.logDebugMenu = QMenu(maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kDebugLevelLogDebugMenu'] % str(val))
        self.logDebugMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.k0LogDebugMenu'  ],
        ...
        self.logDebugMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.k5LogDebugMenu'  ],
                                   lambda: self.setMessageLevel("debug",5))
        self.logMenu.addMenu(self.logDebugMenu)
        
        self.helpMenu = menubar.addMenu(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kHelpFileMenu'  ])
        self.helpMenu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kHelpOnXgenHelpMenu'  ], self._showHelp)
    ...
    def createTopBar(self,theLayout):
        ...
        # palettes
        label = QLabel( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCollection'  ] )
        ...
        # descriptions
        label = QLabel( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDescription'  ] )
        label.setAlignment( QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter )
        currentHbox.addWidget(label)
    ...
    def createPalExprs(self,theLayout):
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kName'  ])
        label.setFixedWidth(int(labelWidth()))
        label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        label.setIndent(DpiScale(10))
        label.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kNameAnn'  ])
        self.pexprName = QLineEdit()
        self.pexprName.setFixedWidth(DpiScale(100))
        self.pexprName.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExprNameAnn'  ])
        ...
        self.pexprType.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExprTypeAnn'  ])
        ...
        self.pexprAddButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCollectionExprAnn'  ])
    ...
    def getSelModeText( self, sel, mode, cache=False ):
        t =""
        if sel:
            t +=  maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSelected'  ] + ": "
        if mode==self.PreviewMode.All:
            t+= maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kAll'  ]
        elif mode==self.PreviewMode.Collection:
            t+= maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCurrentCollection'  ]
        elif mode==self.PreviewMode.Description:
            t+= maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCurrentDescription'  ]
        if cache:
            t+= maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearArchiveCache'  ]
    ...
    def addBindActions( self, menu, bar ):
        ...
        if bar:
            button = bar.addButton("xgBindFaces.png",
                                   maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kModifyCurrentDescriptionsFaceBindings'  ],
                                   self.appendFacesAction )
            button.setPopupMode(QToolButton.MenuButtonPopup)
            button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize, withMenu=True))
        else:
            menu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kAddFacesBindMenu'  ], self.appendFacesAction )

        menu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kReplaceFacesBindMenu'  ], self.replaceFacesAction )
        menu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kRemoveFacesBindMenu'  ], self.removeFacesAction )
        menu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kMapFacesBindMenu'  ], self.mapFacesAction )
        menu.addSeparator()
        menu.addAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kSelectFacesBindMenu'  ], self.selectBinding )
    ...
    def createShelfBar(self,theLayout):
        ...
        # Add Preview button
        button = bar.addButton( "xgPreview.png", maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreview'  ], self.previewCIP )
        button.setAutoRaise(True)
        button.setPopupMode(QToolButton.MenuButtonPopup)
        button.setToolTip( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kUpdateTheXGenPreview'  ] )
        button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize, withMenu=True))

        menu = QMenu()

        self.previewAutoAction = menu.addAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kUpdatePreviewAutomatically'  ], self.onRefreshPreviewAuto )
        menu.addSeparator()
        self.previewGroup = QActionGroup(menu)
        act = QAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewCurrentDescriptionOnly'  ], self.previewGroup )
        act.setData(self.PreviewMode.Description)
        act.setCheckable(True)
        act = QAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewAllDescriptionsinCollection'  ], self.previewGroup )
        act.setData(self.PreviewMode.Collection)
        act.setCheckable(True)
        act = QAction(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewAllCollections'  ], self.previewGroup )
        ...
        menu.addSeparator()
        self.previewSelAction = menu.addAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewSelectedObjectsOnly'  ], self.onRefreshPreviewSel )
        ...
        # Add Clear button
        button = bar.addButton( "xgPreviewClear.png", maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearPreview'  ], self.clearPreview )
        button.setAutoRaise(True)
        button.setPopupMode(QToolButton.MenuButtonPopup)
        button.setToolTip( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearTheXGenPreview'  ] )
        button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize, withMenu=True))

        menu = QMenu()
        self.clearGroup = QActionGroup(menu)
        act = QAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearCurrentDescriptionOnly'  ], self.clearGroup )
        act.setData(self.PreviewMode.Description)
        act.setCheckable(True)
        act = QAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearAllDescriptionsInCollection'  ], self.clearGroup )
        act.setData(self.PreviewMode.Collection)
        act.setCheckable(True)
        act = QAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearAllCollections'  ], self.clearGroup )
        act.setData(self.PreviewMode.All)
        act.setCheckable(True)
        self.clearGroup.triggered.connect( self.onClearPreviewMode )
        menu.addActions( self.clearGroup.actions() )
        menu.addSeparator()
        self.clearSelAction = menu.addAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearSelectedObjectsOnly'  ], self.onClearPreviewSel )        
        self.clearSelAction.setCheckable(True)
        self.clearCacheAction = menu.addAction( maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kClearCachedArchives'  ], self.onClearPreviewCache )        
        ...
        button = bar.addButton("xgCreateDescription.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCreateANewDescription'  ],
        ...
        button = bar.addButton("xgGuideContext.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kAddOrMoveGuidesForTheCurrentDescription'  ],
                               lambda: mel.eval("XgGuideTool"))
        button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize))

        # Toggle guide visibility
        button = bar.addButton("xgToggleGuide.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kToggleVisibilityOfCurrentDescriptionsGuides'  ],
        ...
        button = bar.addButton("xgToggleGuideReference.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kToggleAbilityToSelectCurrentDescriptionsGuides'  ],
                               lambda: xg.toggleGuideReference(self.currentDescription()))
        button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize))

        # Flip guides across model
        button = bar.addButton("xgFlipGuides.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kFlipSelectedGuidesAcrossXAxis'  ],
        ...
        button = bar.addButton("xgSelectionToggle.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kToggleBetweenXgenPatchesAndGeometry'  ],
                               lambda: xg.selectionToggle(self.currentDescription()))
        button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize))

        # Isolate select of primitives
        button = bar.addButton("xgPrimSelection.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCreatePrimitiveSelectionBox'  ],
                               self.primSelectionContext )
        button.setMinimumSize(ToolButtonSizeHint(button, ShelfToolButtonSize))

        # Cull primitives
        button = bar.addButton("xgCullPrimContextSelect.png",
                               maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kCullPrimitivesWithinSelectionBox'  ],
    ...
    def createTabBar(self,theLayout):
        ...
        def addTab(tabset, typename, clsname, BlankWidget):
            ...
            if tabclassname not in self.uiclasses:
                print(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kNoUiDefinedFor'  ] % (typename, clsname))
            ...
            try:
                ...
                print(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kErrorBuildingUiFor'  ] % (typename, clsname))
                traceback.print_exc()
                tabset.addWidget(BlankWidget(typename))
        ...
        self.tabs.addTab(createScrollArea(self.generatorTab) ,maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPrimitives'  ])
        self.tabs.addTab(createScrollArea(self.createPreviewAndRenderTab()),maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewRender'  ])
        self.tabs.addTab(createScrollArea(self.fxStackTab),maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kModifiers'  ])
        if xgg.Maya:
            self.tabs.addTab(createScrollArea(self.brushTab),maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kGrooming'  ])
        self.tabs.addTab(createScrollArea(self.utilitiesTab),maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kUtilities'  ])
        self.tabs.addTab(createScrollArea(self.palExprsTab),maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExpressions'  ])
    ...
    def createPreviewAndRenderTab(self):
        ...
        expand = ExpandUI(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewSettings'  ])
        expand.addWidget(self.previewerTab)
        layout.addWidget(expand)

        expand = ExpandUI(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kRenderSettings'  ])
        expand.addWidget(self.rendererTab)
        layout.addWidget(expand)

        expand = ExpandUI(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kLODSettings'  ])
    ...
    def preview(self,clean=False,progress=True,idle=True):
        ...
        if clean:
            ...
            if progress:
                setProgressInfo(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kPreviewProgressBar'  ] % text)
                cmd += '-pb '
    ...
    def refresh(self,type):
        ...
        with self.setAttrGuard():            
            ...
            if type == 'Full':
                ...
                xg.XGWarning(3,
                    maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kInvalidRefreshType'])
    ...
    def editPath(self):
        ...
        (res,ok) = QInputDialog.getText(self,maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kEditFilePath2'  ],
                    maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kEditXGenPathDescription'  ],
    ...
    def repoMan(self,value):
        ...
        if str(value)=="save":
            ...
            try:
                ...
                xglog.XGError(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kTheGivenDescriptionDoesntExist'  ] % filename)
                self.buildMenus()
                return
    ...
    def exportPatches(self):
        ...
        if strCurrentScene:
            ...
            xglog.XGError(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kYouMustSaveTheSceneFirst'  ])
            return 

        dialog = ExportUI()
        dialog.setWindowTitle(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kExportPatchesForBatchRenderDialog'  ])
        result = dialog.exec_()
    ...
    def setMessageLevel(self,type,level):
        ...
        if type=="debug":
            self.logDebugMenu.setTitle(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kDebugLevelMenuChange'  ] % str(level))
            xg.setMessageLevel(type,level)
        elif type=="warning":
            self.logWarnMenu.setTitle(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kWarningLevelMenuChange'  ] % str(level))
            xg.setMessageLevel(type,level)
        elif type=="stats":
            self.logStatsMenu.setTitle(maya.stringTable[ 'y_xgenm_ui_xgDescriptionEditor.kStatisticsLevelMenuChange'  ] % str(level))
            xg.setMessageLevel(type,level)
        else:
    ...
    if showIt:
        ...
        if cmds.workspaceControl('XGenDockableWidget', q=True, ex=True):
            ...
            cmds.workspaceControl('XGenDockableWidget',
                                  ...
                                  label=maya.stringTable['y_xgenm_ui_xgDescriptionEditor.kXgen' ],
