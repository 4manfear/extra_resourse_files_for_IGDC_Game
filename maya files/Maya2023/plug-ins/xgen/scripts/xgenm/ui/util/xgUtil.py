...
def ToolButtonSizeHint(button, inSize, bottomText=r'', rightText=r'', withMenu=False):
    ...
    if len(bottomText) > 0:
        ...
        textSize.setWidth(textSize.width() + fm.horizontalAdvance(r' ') * 2)
...
def saveScene():
    ...
    try:
        ...
        saveSceneStr = maya.stringTable[ 'y_xgenm_ui_util_xgUtil.kSaveScene' ]
        if len( sceneName ) > 0:
            modified = cmds.file( query=True, modified=True )
            if modified:
                # modified after last save, prompt to save the scene
                msgBox = QMessageBox(QMessageBox.NoIcon, saveSceneStr, maya.stringTable[ 'y_xgenm_ui_util_xgUtil.kSaveModifiedScene' ], QMessageBox.Save | QMessageBox.Cancel) #, Qt.Dialog | Qt.MSWindowsFixedSizeDialogHint)
                ret = msgBox.exec_()
            ...
            # not saved
            msgBox = QMessageBox(QMessageBox.NoIcon, saveSceneStr, maya.stringTable[ 'y_xgenm_ui_util_xgUtil.kSaveSceneToContinue' ], QMessageBox.Save | QMessageBox.Cancel) #, Qt.Dialog | Qt.MSWindowsFixedSizeDialogHint)
            ret = msgBox.exec_()
            if ret == QMessageBox.Save:
                mel.eval("SaveScene")
