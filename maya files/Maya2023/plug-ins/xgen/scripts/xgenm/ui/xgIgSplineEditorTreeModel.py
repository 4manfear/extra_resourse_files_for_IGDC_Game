...
class IgSplineEditorTreeModel(QAbstractItemModel):
    ...
    # Default names
    DEFAULT_LAYER_NAME  = maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDefaultLayerName']
    DEFAULT_GROUP_NAME  = maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDefaultGroupName']
    DEFAULT_MERGE_NAME  = maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDefaultMergeName']
        ...
        if self.HEADER_DATA is None:
            self.HEADER_DATA = {
                ...
                IgSplineEditorTreeColumn.NAME: {
                    Qt.DisplayRole: maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kColumnName'],
                    Qt.TextAlignmentRole: Qt.AlignCenter
                    },
                IgSplineEditorTreeColumn.WEIGHT: {
                    Qt.DisplayRole: maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kColumnWeight'],
                    Qt.TextAlignmentRole: Qt.AlignCenter
                    },
                IgSplineEditorTreeColumn.EDIT: {
                    Qt.DisplayRole: maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kColumnEdit'],
                    Qt.TextAlignmentRole: Qt.AlignCenter
                    },
                IgSplineEditorTreeColumn.KEY: {
                    Qt.DisplayRole: maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kColumnKey'],
                    Qt.TextAlignmentRole: Qt.AlignCenter
                    }
                }
    ...
    def addModifier(self, nodeType):
        ...
        if len(descriptionHandles) == 0:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kAddModifierNoSelectionTip'])
            return
    ...
    def addLayer(self):
        ...
        if len(groupItems) == 0:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kAddLayerNoSelectionTip'])
            return
    ...
    def duplicateLayerAndGroup(self):
        ...
        # No sculpt group is selected ?
        if len(items) == 0:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDeleteLayerNoSelectionTip2'])
            return
    ...
    def mergeLayers(self, bVisibleOnly):
        ...
        if len(sculptItems) == 0:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kMergeLayerNoSelectionTip'])
            return
        ...
        with MayaCmdsTransaction(r'MergeLayers'):
            while len(sculptItems) > 0:
                ...
                if not bHasVisibleLayer or len(layers) < 1 or (len(items) == 1 and not items[0].isGroup()):
                    xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kMergeLayerMoreSelectionTip'])
                    continue
    ...
    def addGroup(self):
        ...
        if len(transformItems) == 0 and len(sculptItems) == 0:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kAddGroupNoSelectionTip'])
            return
    ...
    def duplicateItem(self):
        ...
        if len(modifierItems) == 0:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDuplicateNoSelectionTip'])
            return
        ...
        with MayaCmdsTransaction(r'Duplicate'):
            for item in modifierItems:
                ...
                if not cmds.objExists(modifier):
                    cmds.warning(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kModifierToDuplicateNotExist'] % modifier)
                    continue

                # Ensure the description exists
                if not cmds.objExists(description):
                    cmds.warning(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDescriptionToDuplicateNotExist'] % description)
                    continue
    ...
    def moveItem(self, direction):
        ...
        if (len(transformItems) == 0
            ...
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kMoveNoSelectionTip'])
            return
    ...
    def deleteItem(self):
        ...
        if (len(transformItems) == 0
                and len(modifierItems) == 0
                and len(sculptItems) == 0):
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kDeleteNoSelectionTip'])
            return
        ...
        if isinstance(item, IgSplineEditorAbstractSculptItem):
            ...
            if column == IgSplineEditorTreeColumn.KEY:
                if isinstance(item, IgSplineEditorSculptLayerItem):
                    ...
                    popupMenu = QMenu()
                    popupMenu.addAction(
                        maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kSetKeyframeKeyAtCurrent'],
                        lambda plug=plug: self.__setKeyframeOnPlug(plug, 0))
                    popupMenu.addAction(
                        maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kSetKeyframeKeyAt0'],
                        lambda plug=plug: self.__setKeyframeOnPlug(plug, 1))
                    popupMenu.addAction(
                        maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kSetKeyframeKeyAt1'],
                        lambda plug=plug: self.__setKeyframeOnPlug(plug, 2))
                    popupMenu.addSeparator()
                    removeKeyItem = popupMenu.addAction(
                        maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kSetKeyframeRemoveKey'],
        ...
        if column == IgSplineEditorTreeColumn.NAME:
            ...
            if isinstance(item, IgSplineEditorDagItem):
                if isinstance(item.getDagShape(), IgSplineEditorDescriptionItem):
                    ...
                    subMenu = popupMenu.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuAddModifier'])
            ...
            if not isinstance(item, IgSplineEditorHistoryItem):
                popupMenu.addAction(
                    maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuGroup'],
                    lambda: self.addGroup())

            # Rename menu.
            popupMenu.addAction(
                maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuRename'],
                lambda index=index: self.delayedEdit(index))
            
            # Duplicte menu. We currently only support duplicating modifiers.
            if isinstance(item, IgSplineEditorModifierItem):
                popupMenu.addAction(
                    maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuDuplicate'],
                    lambda: self.duplicateItem())
            
            if isinstance(item, IgSplineEditorAbstractSculptItem):
                popupMenu.addAction(
                    maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuLayerDuplicate'],
            ...
            if not isinstance(item, IgSplineEditorBaseNodeItem):
                popupMenu.addAction(
                    maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuDelete'],
            ...
            if isinstance(item, IgSplineEditorAbstractSculptItem):
                popupMenu.addAction(
                    maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuMerge'],
                    lambda: self.mergeLayers(False))
            
            # Merge menu
            if isinstance(item, IgSplineEditorAbstractSculptItem):
                popupMenu.addAction(
                    maya.stringTable['y_xgenm_ui_xgIgSplineEditorTreeModel.kContextMenuMergeVisible'],
