...
class PrimitiveTabUI(QWidget):
    def __init__(self,selfType,printableName=""):
        ...
        self.topExpandUI = ExpandUI(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kPrimitiveAttributes' ] )
    ...
    def typeUI(self):
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kPrimitiveType'  ])
        label.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kPrimitiveTypeAnn'  ])
    ...
    def createActiveItem(self,type):
        if type == "Archive":
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kArchive'  ]
        elif type == "Card":
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kCard'  ]
        elif type == "Sphere":
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kSphere'  ]
        elif type == "Spline":
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kSpline'  ]
        else:
            typename = type
    ...
    def setActiveByTypeName(self,typename):
        ...
        for i in range(count):
            if self.active.itemData(i) == typename:
                ...
                break
        if not found:
            raise ValueError(maya.stringTable['y_xgenm_ui_tabs_xgPrimitiveTab.kPrimTabUnknownType' ] % typename)
    ...
    def methodUI(self):
        ...
        hbox.setSpacing(DpiScale(3))
        hbox.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        label = QLabel( maya.stringTable['y_xgenm_ui_tabs_xgPrimitiveTab.kControlUsing' ])
        label.setToolTip( maya.stringTable['y_xgenm_ui_tabs_xgPrimitiveTab.kControlUsingAnn' ] )
        label.setFixedWidth(labelWidth())
        label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        label.setIndent(DpiScale(10))
        hbox.addWidget(label)
        self.method = _ComboBoxUI()
        self.method.addItem(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kAttribute'  ])
        self.method.addItem(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kGuides'  ])
        self.method.setFixedWidth(labelWidth())
        self.method.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kMethodAnn'  ])
    ...
    def regionControlUI(self):
        self.regionControlGroup = ExpandUI(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kRegionControl'  ])
        self.actualLayout().addWidget(self.regionControlGroup)
        
        self.regionMaskExpr = ExpressionUI("regionMask",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kRegionMaskAnn'  ],self.type+"Primitive",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kRegionMask'  ])
        self.regionControlGroup.addWidget(self.regionMaskExpr)
        self.regionMapDir = ExpressionUI("regionMap",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kRegionMapAnn'  ],
             self.type+"Primitive",maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kRegionMap'  ],True,"Region", "xgRegionMap3dPaintContext")
        self.regionControlGroup.addWidget(self.regionMapDir)
        
    def animationUI(self):
        self.animationGroup = ExpandUI(maya.stringTable['y_xgenm_ui_tabs_xgPrimitiveTab.kGuideAnimation'])
        self.actualLayout().addWidget(self.animationGroup)
        
        self.useAnimBox = CheckBoxUI(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kUseAnimation'  ],"useCache",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kUseAnimAnn'  ],self.type+"Primitive")
        self.animationGroup.addWidget(self.useAnimBox)

        self.cacheFileDir = BrowseUI("cacheFileName",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kCacheFileNameAnn'  ],self.type+"Primitive",
             "*.caf *.abc","in",maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kCacheFileName'  ])
        ...
        self.liveMode = CheckBoxUI(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kLiveMode'  ], "liveMode",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kLiveModeAnn'  ], 
             self.type+"Primitive")
        buttonLayout.addWidget(self.liveMode)        

        self.exportGuidesButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kCreateHairSystem'  ])
        self.exportGuidesButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kCreateHairSystemAnn'  ])
        self.exportGuidesButton.setFixedWidth(DpiScale(115))
        buttonLayout.addWidget(self.exportGuidesButton)                    

        self.attachButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kAttachHairSystem'  ])
        self.attachButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kAttachHairSystemAnn'  ])
    ...
    def createHairSystem(self):
        ...
        if guides:
            ...
            xglog.XGWarning(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kThereIsNoGuidesWarning'  ])
    ...
    def attachSlot(self):
        ...
        if sel:                
            ...
            mel.eval(cmd)
            print(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kAttachedCurves'  ], sel)
        else:
            warn = QMessageBox()
            warn.setText(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kNoCurvesSelected'  ])
            warn.exec_()
...
class PrimitiveTabBlankUI(PrimitiveTabUI):
    ...
    def __init__(self, typename, printablename = None):
        ...
        if printablename:
            ...
            labeltext = typename
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_tabs_xgPrimitiveTab.kNoExtraAttributesForSPrimitive'  ] % labeltext)
