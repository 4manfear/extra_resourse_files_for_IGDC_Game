...
class FileRendererTabUI(RendererTabUI):
    def __init__(self):
        ...
        self.outputDir = BrowseUI("outputDir",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kOutputDirAnn'  ],
             "FileRenderer","","out", maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kOutputDir'  ])
        ...
        if ( xgg.Maya ):
            ...
            self.p3dButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kCreateXpdFile'  ])
            self.p3dButton.setFixedWidth(DpiScale(160))
            self.p3dButton.setAutoRepeat(False)
            self.p3dButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kCreateXPDFileAnn'  ])
    ...
    def p3dExport(self):
        ...
        setProgressInfo(maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kXPDProgressInfo'  ])
        cmd = 'xgmFileRender -pb -object {"'+desc+'"}'
        results = mel.eval(cmd)
        print(maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kTheFollowingFilesWereRendered'  ])
        for result in results:
            print("  - ", result)

    def rangeExport(self):
        print(maya.stringTable[ 'y_xgenm_ui_tabs_xgFileRendererTab.kFrameRangeRenderNotYetSupported'  ])
        pass
