...
class RendererTabUI(QWidget):
    ...
    def baseTopUI(self, percent=True):
        ...
        if percent:
            self.percent = FloatUI("percent",
                 maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kPercentAnn'  ],
                 self.type+"Renderer",0.0,100.0,-1000,1000,maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kPercent'  ])
    ...
    def typeUI(self):
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kRendererType'  ])
        label.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kRendererTypeAnn'  ])
    ...
    def getRendererUIName( self, type ):
        ...
        if( type == "File" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kFile'  ]
        elif( type == "Geometry" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kGeometry'  ]
        elif( type == "Guide" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kGuide'  ]
        elif( type == "Mel" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kMel'  ]
        elif( type == "Particle" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kParticle'  ]
        elif( type == "Point" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kPoint'  ]
        elif( type == "Renderman" ):
            typename = maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kRender'  ]
        else:
            typename = type
    ...
    def setActiveByTypeName(self,typename):
        ...
        for i in range(count):
            if self.active.itemData(i) == typename:
                ...
                break
        if not found:
            raise ValueError(maya.stringTable['y_xgenm_ui_tabs_xgRendererTab.kRendererTabUnknownType' ] % typename)
...
class RendererTabBlankUI(RendererTabUI):
    ...
    def __init__(self, typename, printablename = None):
        ...
        if printablename:
            ...
            labeltext = typename
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_tabs_xgRendererTab.kNoExtraAttributesForRenderer'  ] % labeltext)
