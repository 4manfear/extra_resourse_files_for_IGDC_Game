...
class PointRendererTabUI(RendererTabUI):
    def __init__(self):
        ...
        self.outputDir = BrowseUI("outputDir",
             maya.stringTable[ 'y_xgenm_ui_tabs_xgPointRendererTab.kDirectoryForOutput'  ],
             "PointRenderer","","out",maya.stringTable[ 'y_xgenm_ui_tabs_xgPointRendererTab.kOutputDir'  ])
        ...
        if ( xgg.Maya ):
            ...
            self.goButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_tabs_xgPointRendererTab.kRenderPoints'  ])
            self.goButton.setFixedWidth(DpiScale(160))
            self.goButton.setAutoRepeat(False)
            self.goButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_tabs_xgPointRendererTab.kExportPointsForCurrentPrimitives'  ])
    ...
    def goExport(self):
        ...
        setProgressInfo(maya.stringTable[ 'y_xgenm_ui_tabs_xgPointRendererTab.kRenderingPrimitiveLocationsIntoXpds'  ])
        cmd = 'xgmPointRender -pb {"'+desc+'"}'
        results = mel.eval(cmd)
        print(maya.stringTable[ 'y_xgenm_ui_tabs_xgPointRendererTab.kTheFollowingFilesWereRendered'  ])
