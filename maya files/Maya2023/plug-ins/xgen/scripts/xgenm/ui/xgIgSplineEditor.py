...
class IgSplineEditor(QWidget):
    ...
    def __createMenuBar(self):
        ...
        # Edit menu
        editMenu = self.menuBar.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kEditMenu'])
        editMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kEditMenuGroup'],
            lambda: self.newGroupRequested.emit())
        editMenu.addSeparator()
        editMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kEditMenuDuplicate'],
            lambda: self.duplicateRequested.emit())
        editMenu.addSeparator()
        editMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kEditMenuDelete'],
            lambda: self.deleteRequested.emit())

        # Create menu
        createMenu = self.menuBar.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateMenu'])
        createMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateMenuInteractiveGroomSplines'],
            lambda: self.__doCreateSplineDescription())
        createMenu.addSeparator()
        self.__createAddModifierMenu(
            createMenu.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateMenuAddModifier']))
        createMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateMenuAddSculptLayer'],
            lambda: self.newLayerRequested.emit())

        # Select menu
        selectMenu = self.menuBar.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kSelectMenu' ])
        selectMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateMenuSelectSplineOnFace' ],
            lambda: cmds.XgmSplineSelectReplaceBySelectedFaces())
        selectMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateMenuFreezeSelected' ],
            lambda: cmds.XgmSplineSelectConvertToFreeze())

        # Descriptions menu
        descriptionsMenu = self.menuBar.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kDescriptionsMenu'])
        descriptionsMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kDescriptionsMenuTransfer'],
            lambda: self.__doTransferSplineDescription())

        # Descriptions -> Preset menu
        presetMenu = descriptionsMenu.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kPresetMenu' ])
        presetMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kPresetMenuImport' ],
            cmds.XgmSplinePresetImport)
        presetMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kPresetMenuExport' ],
            cmds.XgmSplinePresetExport)

        # Descriptions -> Cache menu
        cacheMenu = descriptionsMenu.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenu'])
        cacheMenu.addAction(
            CreateIcon(r':/createCache.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuCreateNewCache'],
            lambda: cmds.XgmSplineCacheCreate())
        cacheMenu.addAction(
            CreateIcon(r':/importCache.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuImportCache'],
            lambda: cmds.XgmSplineCacheImport())
        cacheMenu.addAction(
            CreateIcon(r':/exportCache.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuExportCache'],
            lambda: cmds.XgmSplineCacheExport())
        cacheMenu.addSeparator()
        cacheMenu.addAction(
            CreateIcon(r':/disableAllCaches.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuDisableAllCache'],
            lambda: cmds.XgmSplineCacheDisableSelectedCache())
        cacheMenu.addAction(
            CreateIcon(r':/enableAllCaches.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuEnableAllCache'],
            lambda: cmds.XgmSplineCacheEnableSelectedCache())
        cacheMenu.addSeparator()
        cacheMenu.addAction(
            CreateIcon(r':/replaceCache.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuReplaceCache'],
            lambda: cmds.XgmSplineCacheReplace())
        cacheMenu.addAction(
            CreateIcon(r':/deleteCache.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuDeleteCache'],
            lambda: cmds.XgmSplineCacheDelete())
        cacheMenu.addAction(
            CreateIcon(r':/deleteCacheHistory.png'),
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCacheMenuDeleteNodesAheadOfCache'],
            lambda: cmds.XgmSplineCacheDeleteNodesAhead())

        # Help menu
        helpMenu = self.menuBar.addMenu(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kHelpMenu'])
        helpMenu.addAction(
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kHelpMenuHelpOnXGen'],
    ...
    def __createTabBar(self):
        ...
        button = container.addButton(r':/newLayerEmpty.png',
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabAddModifierTip'],
            lambda: None)
        text   = maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabAddModifier']
        button.setText(text)
        ...
        button = container.addButton(r':/newLayerEmpty.png',
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabAddSculptLayerTip'],
            lambda: self.newLayerRequested.emit())
        text   = maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabAddSculptLayer']
        button.setText(text)
        button.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        button.setMinimumSize(ToolButtonSizeHint(button, TabToolButtonSize, rightText=text))
        button.setStyleSheet(ToolButtonPushFlatStyle)
        ...
        button = container.addButton(r':/moveLayerUp.png',
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabMoveUpTip'],
        ...
        button = container.addButton(r':/moveLayerDown.png',
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabMoveDownTip'],
            lambda: self.moveRequested.emit(-1))
        button.setMinimumSize(ToolButtonSizeHint(button, TabToolButtonSize))
        button.setStyleSheet(ToolButtonFlatStyle)
        ...
        button = container.addButton(r':/folder-new.png',
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabAddSculptGroupTip'],
            lambda: self.newGroupRequested.emit())
        button.setMinimumSize(ToolButtonSizeHint(button, TabToolButtonSize))
        button.setStyleSheet(ToolButtonFlatStyle)
        ...
        button = container.addButton(r':/trash.png',
            maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTabDeleteTip'],
            lambda: self.deleteRequested.emit())
        button.setMinimumSize(ToolButtonSizeHint(button, TabToolButtonSize))
        button.setStyleSheet(ToolButtonFlatStyle)
    ...
    def __doCreateSplineDescription(self):
        ...
        if not selectedMeshes:
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kCreateNoMeshSelected'])
            return
    ...
    def __doTransferSplineDescription(self):
        ...
        if (bInvalidSelectedObjects or bInvalidSelectedMeshes or bInvalidSelectedDescriptions):
            xglog.XGTip(maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kTransferNoSelection'])
            return
    ...
    if restore:
        ...
        if cmds.workspaceControl(igSplineDockControlName, q=True, ex=True):
            ...
            cmds.workspaceControl(igSplineDockControlName, vis=True,
                ...
                label=maya.stringTable['y_xgenm_ui_xgIgSplineEditor.kIgSplineEditorDockTitle'],
