...
def determineNames(node, index):
    ...
    if len(uiName) == 0:
        uiName = r'{} {}'.format(maya.stringTable['y_xgenm_ui_ae_xgmModifierSculptTemplate.kSculptLayerHeader'], index+1)
    assert(len(uiName))
...
def AExgmModifierSculptTemplate(nodeName):
    ...
    cmds.editorTemplate(beginLayout=maya.stringTable['y_xgenm_ui_ae_xgmModifierSculptTemplate.kSculptModifier'], collapse=False)
    cmdstr = r'xgmCreateAEUiForFloatAttr "mask" "{}" "xgmModifierGuide"'.format(maya.stringTable['y_xgenm_ui_ae_xgmModifierSculptTemplate.kMask' ])
    mel.eval(cmdstr)
    cmds.editorTemplate(endLayout=True)

    # Layer Attributes
    cmds.editorTemplate(beginLayout=maya.stringTable['y_xgenm_ui_ae_xgmModifierSculptTemplate.kSculptLayers'], collapse=False)
