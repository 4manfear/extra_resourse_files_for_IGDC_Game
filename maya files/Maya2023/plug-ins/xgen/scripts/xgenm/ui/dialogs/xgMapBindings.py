...
class MapBindingsUI(QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kBindFacesBasedOnMap'  ])
        self.setSizeGripEnabled(True)
        self.setMinimumWidth(DpiScale(550))
        self.de = xgg.DescriptionEditor
        layout = QVBoxLayout()
        self.dirName = BrowseUI(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kMapDirectory'  ],
                                maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kMapDirectoryAnn'  ])
        self.dirName.optionButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kBrowseUIAnn'  ])
        ...
        self.colourLabel = QLabel(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kBindToFacesWhereMapIs'  ])
        combobox.addWidget(self.colourLabel)
        self.whiteOrBlackcb = _ComboBoxUI()
        self.whiteOrBlackcb.addItem(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kWhite'  ])
        self.whiteOrBlackcb.addItem(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kBlack'  ])
        ...
        self.bindButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kBind'  ])
        self.bindButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kBindAnn'  ])
        self.bindButton.setAutoRepeat(False)
        self.connect(self.bindButton, QtCore.SIGNAL("clicked()"),
                     self.accept)
        hbox.addWidget(self.bindButton)
        self.cancelButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kCancel'  ])
        self.cancelButton.setAutoRepeat(False)
        self.cancelButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kCancelAnn'  ])
    ...
    if result == QDialog.Accepted:
        ...
        if len(mapDir):
            if xgg.Maya:
                ...
                goPort = QMessageBox()
                goPort.setText(maya.stringTable[ 'y_xgenm_ui_dialogs_xgMapBindings.kMapBasedBindingOnlyAvailableInMaya'  ])
                goPort.exec_()
