...
exportPresetDialogTitle = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kXgenExportPreset'  ]
...
class ExportPresetUI(QWidget):
    ...
    def descriptionUI(self,layout):
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kCollectionToExportPreset'  ])
        ...
        hbox.setSpacing(DpiScale(3))
        hbox.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kDescriptionToExportAsPreset'  ])
        label.setFixedWidth(labelWidth())
        label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        label.setIndent(DpiScale(10))
        hbox.addWidget(label)
        ...
        vbox.addWidget(row)
        # row for file name
        self.xgpFile = BrowseUI(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kFileNameForPreset'  ], "", "", "*.xgp", "out")
        self.connect(self.xgpFile.textValue, QtCore.SIGNAL("editingFinished()"), self.onFileSelected)
        vbox.addWidget(self.xgpFile)

        self.exportActiveModule = CheckBoxUI(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kExportActivatedModifiersOnly'  ],"")
    ...
    def thumbnailUI(self,layout):
        ...
        label = QLabel(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kXGenPresetThumbnail'  ])
        label.setFixedWidth(labelWidth())
        ...
        self.snapshotButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kSnapshotButtonForPresetThumbnail'  ])
        ...
        self.browseButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kBrowseButtonForPresetThumbnail'  ])
    ...
    def buttons(self,layout):
        ...
        hbox.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        self.exportButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kExportPreset'  ])
        ...
        self.cancelButton = QPushButton(maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kCancelExportPreset'  ])
    ...
    def validateModifiers(self, pal, desc):
        ...
        for mod in modules:
            ...
            if type == "BakedGroomManagerFXModule" and active == "true": 
                text = maya.stringTable['y_xgenm_ui_dialogs_xgExportPreset.kGroomBakeModifierNotSupport'  ] % mod
                text += maya.stringTable['y_xgenm_ui_dialogs_xgExportPreset.kGroomBakeModifierNotSupportDetail' ]
        ...
        if len(modNotIncluded) != 0:
            text = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kModifiersNotSupportInPreset'  ]
            text += "\n\n" + modNotIncluded
            text += maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kDoYouWantToContinueAgain' ]
    ...
    def exportCB(self):
        ...
        if os.path.isfile(xgpFileName):
            text = maya.stringTable['y_xgenm_ui_dialogs_xgExportPreset.kPresetFileExists' ] % xgpFileName
            selection = QMessageBox.warning(self, exportPresetDialogTitle, text, QMessageBox.Ok | QMessageBox.Cancel)
            if selection == QMessageBox.Cancel:
                return
        ...
        if len(xgpFileName) and len(pal) and len(desc):
            ...
            if result:
                ...
                text = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kExportPresetFail'  ]
            ...
            if not result:
                text = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kExportMaterialFail'  ]
                QMessageBox.warning(self, exportPresetDialogTitle, text)

            result = self.exportThumbnail(xgpFileName)
            if not result:
                text = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kExportThumbnailFail'  ]
                QMessageBox.warning(self, exportPresetDialogTitle, text)

        else:
            text = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kYouMustSpecifyFilenameWhenExportPreset'  ]
            QMessageBox.warning(self, exportPresetDialogTitle, text)
            return
...
def exportPreset():
    ...
    if len(uiPalettes()) == 0:
        tellem = maya.stringTable[ 'y_xgenm_ui_dialogs_xgExportPreset.kThereAreNoCollectionsToExportPreset'  ]
