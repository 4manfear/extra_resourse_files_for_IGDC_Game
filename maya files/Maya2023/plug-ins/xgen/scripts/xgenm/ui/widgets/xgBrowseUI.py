...
class BrowseUI(QWidget):
    ...
    def __init__(self,attr,help="",object="",fileType="",inOrOut="in",mainlabel=""):
        ...
        self.textValue.setMinimumWidth(max(DpiScale(50),
            QFontMetrics(self.textValue.font()).horizontalAdvance("XGen ${DESC}/${FXMODULE}/ XGen")))
        ...
        self.optionButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_widgets_xgBrowseUI.kRaiseFileBrowser'  ])
        # Horizontal layout
        layout = QHBoxLayout()
...
class FileBrowserUI(BrowseUI):
    ...
    def folderExists( folder, desc, uiParent ):
        ...
        if not (os.path.isdir(path) and os.path.exists(path)):
            ...
            text = maya.stringTable[ 'y_xgenm_ui_widgets_xgBrowseUI.kFolderDoesNotExist' ] % path
            mb.setText(text)
            mb.exec_()
    ...
    def createFolder(folder,desc,uiParent):
        ...
        if not os.path.exists(path):
            title = maya.stringTable[ 'y_xgenm_ui_widgets_xgBrowseUI.kFileBrowserUI'  ]
            text = maya.stringTable[ 'y_xgenm_ui_widgets_xgBrowseUI.kFolderDoesNotExist2' ] % path
            mbox = QMessageBox( QMessageBox.NoIcon, title, text, QMessageBox.NoButton, uiParent )
            newFolderBtn = QPushButton(maya.stringTable[ 'y_xgenm_ui_widgets_xgBrowseUI.kCreateNewFolder'  ])
