...
class MayaAttr(object):
    ...
    def getJobNumber(self,jobIn,jobType,attrName):
        ...
        try: 
            ...
            print(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kExprUIFailedToParseJobs' ])
    ...
    def create(self):
        ...
        try:
            ...
            xglog.XGError( maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kExprUIErrCreatingAttrNamed' ] + self.mayaAttrName ) 

    def getMenu(self, event):
        if event.button() == Qt.RightButton:            
            labelmenu = QMenu( )
            labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kRefresh'  ], lambda: self.exprUI.refresh())
            if self.hasAnimation() == 0 and self.hasExpression() == 0:
                labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kCreateNewExpression'  ], lambda: self.openExprEdit())
                labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kSetKey'], lambda: self.setKey())
            else:
                if self.hasExpression():
                    labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kEditExpression'  ], lambda: self.openExprEdit()) 
                    labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kDeleteExpression'  ], lambda: self.deleteExpr()) 
                else:
                    labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kSetKey2'  ], lambda: self.setKey())
                    labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kBreakConnection'  ], lambda: self.breakConnection()) 
...
class ptexBaker(object):
    ...
    def ApplyPaintMaterial(): 
        ...
        for curSel in oldsel:
            try:
                ...
                print(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kExprUIErrQuerySelection' ])
    ...
    def stop():
        errDoPtexBake = maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kExprUIErrDoPtexBake' ]
    ...
    def setupPaintTool( shapeAttrName ):
        ...
        try:
            ...
            xglog.XGError( maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kPaintError'] )
...
class MapExpressionUI(QWidget):
    ...
    def __init__(self,parent,object,attr,strLabel,file,mode,pos,defaultValue,paintOn=False,mapcolor="basedOnAttribute",customAttr="",isBrowseMode=False,paint3dCtx=""):
        ...
        if mode[ : len("3dpaint,") ] == "3dpaint,":
            self.prefix = maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kPaintableTextureMap'  ]
            self.tpu = float( mode[len("3dpaint,"):] )
            self.mode = "3dpaint"
            paintIcon = "xg3dPaint.png"
        elif mode[ : len("vpaint") ] == "vpaint":
            self.prefix = maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kPaintableVertexColorMap'  ]
            self.mode = "vpaint"
            paintIcon = "xgVPaint.png"
        else:
            self.prefix = maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kPtexMap'  ]
            self.mode = "file"
            
        self.edit.setToolTip( self.prefix % maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kPtexFilePath'  ])
        
        self.pos = pos
        
        self.optionButton = QToolButton()
        self.optionButton.setIcon(CreateIcon("xgBrowse.png"))
        self.optionButton.setToolTip(self.prefix % maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kRaiseFileBrowser'  ])
        ...
        if self.isBrowseMode:
            ...
            self.menu = QMenu()
            self.menu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kCreateMap'  ],lambda: self.parent.callCreateMap(self.parent.browseMode))
        ...
        self.paintButton.setToolTip(self.prefix % maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kStartPaintingOnSelectedMeshes'  ])
        
        self.saveButton = QToolButton()
        self.saveButton.setIcon(CreateIcon("xgSave.png"))
        self.saveButton.setFixedSize(DpiScale(24),DpiScale(24))
        self.saveButton.setToolTip(self.prefix % maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kBakeThePtexMapsToDisk'  ])
    ...
    def replaceValue(self):
        ...
        if self.object!="":
            ...
            exprString = r"(\\$%s+\\s*=\\s*%s\\([\"\'])([\\w${}\\-\\\\/]+)[\"\']);" % (self.var, mapExpr)
            exprText = de.getAttr(self.object,self.attr)
            oldExprText = exprText
    ...
    def fileBrowser(self):
        ...
        if len(selectedFile):
            ...
            if selectedFile.find("${DESC}") == -1 and self.isBrowseMode:
                xglog.XGError(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kFolderInDescError'  ])
                return
...
class ColorExpressionUI(QWidget):
    ...
    def replaceValue(self):
        if self.object!="":
            de = xgg.DescriptionEditor
            exprString = r"(\\$%s+\\s*=\\s*)\[(-?[\\d.]*,-?[\\d.]*,-?[\\d.]*)];"  % (self.var)
            exprText = de.getAttr(self.object,self.attr)
            oldExprText = exprText
            re = QRegExp( exprString )
            pos = re.indexIn(exprText, 0)
...
def loadFromRepo(filename):
    if not _currentExpr:
        xglog.XGError(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kExprUICouldNotDetermineCurrentExpr' ])
        return
...
class ExpressionUI(QWidget):
    ...
    to be bound to the expression editor.
    """
    ...
    def __init__(self,attr,help="",object="",mainlabel="",isColor=False,browseMode="",paint3dCtx=""):
        ...
        self.optionButton.setAutoRaise(True)
        self.optionButton.setPopupMode(QToolButton.MenuButtonPopup)
        self.optionButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kOpenExpressionEditor'  ])
        # Warning: override the mousePressEvent for the toolButton
        self.optionButton.mousePressEvent = self.updateOwner

        # arrow menu 
        expressionUICreateMap = maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kExpressionUICreateMap'  ]
        self.menu = QMenu()
        self.menu.addAction(expressionUICreateMap, self.callCreateMap)
        
        self.menu.addSeparator()
        self.menu.addAction(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kSaveExpression'],
                           lambda: self.repoMan("*save"))
        self.buildMenus()
        self.menu.addMenu(_userExprMenu)
        self.menu.addSeparator()
        self.menu.addAction(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kResetExpressionToSlider'], lambda: _currentExpr.resetToSlider())
        self.optionButton.myMenu = self.menu # Keep a reference to the QMenu
        self.optionButton.setMenu(self.menu)
        ...
        if self.isBrowseMode:
            self.optionButton.setIcon(CreateIcon("xgBrowse.png"))
            self.optionButton.setToolTip(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kExpressionUIRaiseFileBrowser'  ])
    ...
    def parseVectorString( self, exprText ):
        ...
        typecomment = r'\\s*#\\s*(\\w+)'
        re = QRegExp( r'\\$(\\w+)\\s*=\\s*\[(-?[\\d.]*),(-?[\\d.]*),(-?[\\d.]*)];(%s)?' % (typecomment))
        offset = 0
        while True:
    ...
    def labelmouseReleaseEvent(self,event):
        if event.button() == Qt.RightButton:
            if self.mayaAttr != None and self.mayaAttr != "":
                ...
                labelmenu = QMenu( )
                labelmenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kExpressionUIRefresh'  ], lambda: self.refresh())
                if self.mayaAttr != "":
                    createMenu = QMenu(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kLinkToMayaAttribute'  ])
                    createMenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kFloat'  ], lambda: self.createMayaAttr(1))
                    createMenu.addAction(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kVector'  ], lambda: self.createMayaAttr(3))
    ...
    def buildMenus(self):
        ...
        if not _userExprMenu:
            _userExprMenu = QMenu(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kLoadExpressionMenu' ])
        else:
    ...
    def onAboutToShowMenus(self):
        ...
        _userExprMenu.addAction(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kLoadExpressionDialog'], lambda: _currentExpr.repoMan("*loaddialog")) 
    ...
    def repoMan(self,value):
        ...
        if str(value)=="*save":
            ...
            try:
                ...
                xglog.XGError(maya.stringTable['y_xgenm_ui_widgets_xgExpressionUI.kExprUIExprNotExist' ] % filename)
                self.buildMenus()
                return
            try:
    ...
    def fileBrowser(self):
        ...
        if len(selectedFile):
            ...
            # do not allow non-correct folders to be chosen by the user
            if selectedFile.find("${DESC}") == -1 and self.isBrowseMode:
                xglog.XGError(maya.stringTable[ 'y_xgenm_ui_widgets_xgExpressionUI.kExpressionUIFolderInDescError'  ])
                return
