...
try:
    ...
    CA_CERTS = os_path.join(
        os_path.dirname(os_path.abspath(__file__ )), "cacerts.txt")
...
class FileCache(object):
    ...
    def __init__(self, cache, safe=safename): # use safe=lambda x: md5.new(x).hexdigest() for the old behavior
        ...
        self.cache = cache
        self.safe = safe
        if not os_path.exists(cache):
            os_makedirs(self.cache)

    def get(self, key):
        from os import path as os_path
        retval = None
        cacheFullPath = os_path.join(self.cache, self.safe(key))
        try:
    ...
    def delete(self, key):
        ...
        cacheFullPath = os_path.join(self.cache, self.safe(key))
        if os_path.exists(cacheFullPath):
            os_remove(cacheFullPath)
...
def proxy_info_from_environment(method='http'):
    ...
    from os import environ as os_eniron
    url = os_environ.get(env_var, os_environ.get(env_var.upper()))
    if not url:
        return
    pi = proxy_info_from_url(url, method)

    no_proxy = os_environ.get('no_proxy', os_environ.get('NO_PROXY', ''))
    bypass_hosts = []
    if no_proxy:
...
try:
    ...
    server_software = os_environ.get('SERVER_SOFTWARE')
