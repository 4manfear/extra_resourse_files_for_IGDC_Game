...
def _retrieve_discovery_doc(url, http, cache_discovery, cache=None):
  ...
  if 'REMOTE_ADDR' in os_environ:
    actual_url = _add_query_parameter(url, 'userIp', os_environ['REMOTE_ADDR'])
...
def createMethod(methodName, methodDesc, rootDesc, schema):
  ...
  def method(self, **kwargs):
    ...
    for key, value in six.iteritems(kwargs):
      ...
      if key in parameters.repeated_params and isinstance(value, type([])):
