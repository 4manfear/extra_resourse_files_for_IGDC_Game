...
class TextBlockData(qt.QTextBlockUserData):

    def __init__(self):
        qt.QTextBlockUserData.__init__(self)
        self.data = {}
...
class TextBlock(object):
    ...
    def __init__(self, data, create_new=False, typ=None):
        if isinstance(data, qt.QTextBlock):
            self.block = data
        elif isinstance(data, qt.QTextCursor):
            if create_new:
        ...
        if create_new:
            ...
            # debug(str(self))
        if not 'type' in list(self.annotations.keys()):
    ...
    def isLast(self):
        return not next(self).block.isValid()
    ...
    def __next__(self):
        return TextBlock(next(self.block))
    ...
    def _initializeDefaults(self):
        if 'decorated' not in list(self.annotations.keys()):
            self.decorated = False
        if 'active_area_start' not in list(self.annotations.keys()):
            self.active_area_start = 0
        if 'indent_string' not in list(self.annotations.keys()):
            self.indent_string = self.CODE_INDENT

    def _decorate(self, decoration):
        c = qt.QTextCursor(self.block)
    ...
    def cursorSelectToLeadingPosition(self, cursor):
        ...
        cursor.movePosition(qt.QTextCursor.Left, qt.QTextCursor.KeepAnchor, delta)

    def leadingCursor(self):
        c = self.startCursor()
        c.movePosition(qt.QTextCursor.Right, qt.QTextCursor.MoveAnchor,
    ...
    def cursorAt(self, position, in_active_area=True):
        ...
        c.movePosition(qt.QTextCursor.Right, qt.QTextCursor.MoveAnchor, p)
        return c
    ...
    def startCursor(self, in_active_area=True):
        ...
        if in_active_area:
            c.movePosition(
                qt.QTextCursor.Right, qt.QTextCursor.MoveAnchor, self.active_area_start)
        return c

    def endCursor(self):
        c = self.startCursor()
        c.movePosition(qt.QTextCursor.EndOfBlock)
        return c
    ...
    def activeContent(self):
        return futureStr(self.block.text())[self.active_area_start:]
    ...
    def content(self, include_decoration=False):
        ...
        return futureStr(self.block.text())[spos:]
    ...
    def contentToCursor(self, cursor, include_decoration=False, in_active_area=False):
        ...
        if self.containsCursor(cursor, in_active_area):
            ...
            epos = -1
        return futureStr(self.block.text())[spos:epos]

    def contentFromCursor(self, cursor):
        spos = cursor.positionInBlock()
        return futureStr(self.block.text())[spos:]

    def appendText(self, textBlock):
        c = self.endCursor()
        c.insertText(textBlock)
