...
float3 getPosBasedOnDistance( float3 initialPosition , float normDistance , __global const float* ramp , const struct TransformationMatrix* matrixA , const struct TransformationMatrix* matrixB , float envelope );
...
float3 getPosBasedOnDistance(
    float3 initialPosition ,
    float normDistance,
    __global const float* ramp , // Should be __constant
    ...
    )
{
    ...
    if( normDistance < 1.0f )
    {
        float integerPart;
        float fraction = fract( normDistance*20.0f , &integerPart );
        int index = integerPart;

        float v0 = ramp[index];
        float v1 = ramp[index+1];
        weight = envelope * (v0+fraction*(v1-v0));
    }

	// This short-circuit exit which exists in the non GPU version of the deformer makes a difference
	// when preMatrix does not match postMatrixInverse
	if( weight < 1.0e-5f )
	{
...
__kernel void softModPerDistance(
    __global float* finalPos ,
    __global const float* initialPos ,
    __global const float* ramp , // Should be __constant
    const float3 falloffCenter,
    const float  falloffRadius,
    const struct TransformationMatrix matrixA,
    const struct TransformationMatrix matrixB,
	const float16 geomMatrix,
    ...
    )
{
	...
	float normDistance = length( mul( geomMatrix , initialPosition )-falloffCenter )/falloffRadius;

    float weight = envelope;

    float3 finalPosition = getPosBasedOnDistance( initialPosition,
                                                  normDistance ,
                                                  ramp ,
                                                  &matrixA,
                                                  &matrixB,
...
__kernel void softModCachedDistance(
    __global float* finalPos ,
    __global const float* initialPos ,
    __global const float* ramp , // Should be __constant
    ...
    )
{
    ...
    float normDistance = distances[positionId]/falloffRadius;
...
__kernel void softModPerDistanceWithWeights(
    __global float* finalPos ,
    __global const float* initialPos ,
    __global const float* ramp , // Should be __constant
    ...
    )
{
    ...
    float weight = envelope * weights[positionId];
...
__kernel void softModCachedDistanceWithWeights(
    __global float* finalPos ,
    __global const float* initialPos ,
    __global const float* ramp , // Should be __constant
    __global const float* weights ,
