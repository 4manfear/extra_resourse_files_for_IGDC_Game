                            ...
                            float4 initialPosition, __global const float4* influenceVecs)
{
	...
	float4 conj0 = quat0 * (float4)(-1.0f, -1.0f, -1.0f, 1.0f);
	float4 conj1 = quat1 * (float4)(1.0f, 1.0f, 1.0f, -1.0f);
...
__kernel void skinCluster_deformPositions(
	...
	__global const float* vertEnvelopes,
	const float envelope,
	const uint positionCount)
{
    unsigned int id = get_global_id(0);
    if ( id >= positionCount ) return;
    const unsigned int positionId = (affectMap ? affectMap[id] : id); // access finalPos and initialPos using this value
    ...
    unsigned int start = offsets[id];
	unsigned int end = offsets[id+1];
    ...
    float e = (vertEnvelopes ? vertEnvelopes[id]*envelope : envelope);
    if (e < 1.0)
        finalPosition = initialPosition + e * (finalPosition - initialPosition);
...
{
    unsigned int id = get_global_id(0);
    if ( id >= normalCount ) return;

	const unsigned int normalId = (affectMap ? affectMap[id] : id); // access finalPos and initialPos using this value
	const unsigned int normalOffset = normalId * 3; // it is a float3
	const unsigned int vtxId = normalToPositionMap[id];
	float4 initialNorm = (float4)(initialNormal[normalOffset], initialNormal[normalOffset+1], initialNormal[normalOffset+2], 0.0f);
	
    unsigned int start = offsets[vtxId];
	unsigned int end = offsets[vtxId+1];
    unsigned int numInfl = (end > start) ? (end - start) : 0;
	...
	const uint positionCount)
{
	...
	float blendWeight = blendWeights[id];
	...
	const uint normalCount)
{
	...
	float blendWeight = blendWeights[vtxId];
