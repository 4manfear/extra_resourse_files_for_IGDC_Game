...
to `<link>` elements
	...
	<resourcelist>
		...
		<link location="RotateTool">Rotate Tool Documentation</link>
	</resourcelist>
	...
	property int min_height : item_height * 4
		...
		ListView {
			...
			delegate : Rectangle {
				...
				Text {
					id : content_text
					color : !( list_scrollbar.visible && scrollbar_hover_detector.containsMouse) && delegate_ma.containsMouse ? Qt.rgba(0.3, 0.4, 0.6, 1.0) : (index == resourceList.current_index ? Qt.rgba( 1.0, 1.0, 0.75, 1.0) : resourceList.text_color )
		...
		//This MouseArea allows to detect when we are hovering over a scrollbar region
		//Its purpose is to solve the problem where the list items are being highlighted when hovering over the scrollbar
		MouseArea {
			id: scrollbar_hover_detector
			hoverEnabled : true
			...
			width: list_scrollbar.width;
			anchors {
				top: list_scrollbar.top;
				right: list_scrollbar.right;
				bottom: list_scrollbar.bottom;
				left: list_scrollbar.left;
				// We set the margin to a negative value to cover the tiny extra space around the scrollbar.
				// Without this, there is a 1-pixel wide band at the right of the scrollbar that still
				// allows items to be highlighted.
				margins: -1;
			}
		}
