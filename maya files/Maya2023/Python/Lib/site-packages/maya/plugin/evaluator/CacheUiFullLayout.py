...
from maya.plugin.evaluator.cache_preferences import CachePreferenceFillDirection
from maya.plugin.evaluator.cache_preferences import CachePreferencePreventFrameSkipping
from maya.plugin.evaluator.cache_preferences import CachePreferenceDynamicsSupportEnabled
from maya.plugin.evaluator.cache_preferences import CachePreferenceHybridCacheMode
...
from maya.plugin.evaluator.cache_optionvar_states import CachePreferenceCacheSmoothMeshes

__all__ = [r'CacheUiFullLayout']
...
PREFS_MONITORED = [ CachePreferenceEnabled()
                  ...
                  , CachePreferenceCacheSmoothMeshes()
                  , CachePreferenceDynamicsSupportEnabled()
                  , CachePreferenceHybridCacheMode()
                  ]
...
class CacheUiFullLayout(CacheUiBase):
    ...
    ID_ROOT = r'cachedPlaybackMasterControl'
    ...
    ID_WARNING_PERCENT = r'cachedPlaybackRAMPercentWarningIcon'
    ID_WARNING_NO_LIMIT = r'cachedPlaybackRAMLimitWarningIcon'
    ...
    def __init__(self, ui_key):
        ...
        assert ui_key not in CacheUiFullLayout.CONTROLS, maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kCachePrefControlExists' ].format( ui_key )
        ...
        self.widgets[self.ID_ROOT] = cmds.frameLayout(maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kFrameCachedPlayback' ])
        with LayoutManager( self.widgets[self.ID_ROOT] ):
            with LayoutManager( cmds.columnLayout( adjustableColumn=True ) ):
                ...
                with LayoutManager( cmds.rowColumnLayout( numberOfColumns=2
                                                        , columnAlign=[(1,r'left'), (2,r'left')]
                                                        , columnAttach=[(1,r'both',0), (2,r'both',10)]
                                                        , adjustableColumn=2
                                                        ) ):
                    ...
                    control = cmds.image( visible=False, image=r'caution.png'
                                        , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kMemoryAllocationWarningTip' ] )
                    ...
                    control = cmds.image( visible=False, image=r'caution.png'
                                        , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kMemoryLimitWarningTip' ] )
                    ...
                    control = cmds.optionMenuGrp( label=CachePreferenceHybridCacheMode().title
                                                , annotation=CachePreferenceHybridCacheMode().info
                                                , changeCommand=callback_tool(self, self.callback_change_hybrid_cache_mode)
                                                , adjustableColumn=2
                                                )
                    for enum_info in CachePreferenceHybridCacheMode().DATA:
                        cmds.menuItem( parent=_menu(control)
                                     , label=enum_info[CachePreferences.KEY_ENUM_NAME]
                                     , annotation=enum_info[CachePreferences.KEY_ENUM_INFO]
                                     )
                    self.widgets[CachePreferenceHybridCacheMode().ov_id] = control
                    cmds.text( label='' )
                    ...
                    control = cmds.checkBoxGrp( label=''
                                              , label1=CachePreferenceCacheSmoothMeshes().title
                                              , annotation=CachePreferenceCacheSmoothMeshes().info
                                              , changeCommand=callback_tool(self, self.callback_change_cache_smooth_meshes)
                                              )
                    self.widgets[CachePreferenceCacheSmoothMeshes().ov_id] = control
                    cmds.text( label='' )

                    #----------------------------------------
                    control = cmds.checkBoxGrp( label=''
                                            , label1=CachePreferenceDynamicsSupportEnabled().title
                                            , annotation=CachePreferenceDynamicsSupportEnabled().info
                                            , changeCommand=callback_tool(self, self.callback_change_dynamics_support_enabled)
                                            )
                    self.widgets[CachePreferenceDynamicsSupportEnabled().ov_id] = control
                    cmds.text( label='' )
        ...
        self.mode_change_job = cmds.scriptJob(event=(r'customEvaluatorChanged', callback_tool(self, self.callback_mode_changed)))
    ...
    def update_ui(self):
        ...
        if (not self.widgets) or (self.ID_ROOT not in self.widgets) or (not cmds.frameLayout( self.widgets[self.ID_ROOT], exists=True )):
            return

        caching_enabled = CachePreferenceEnabled().get_value()
        ui_enabled = cache_ui_enabled()
        cmds.checkBoxGrp( self.widgets[CachePreferenceEnabled().ov_id], edit=True, enable=cmds.evaluationManager(query=True,mode=True)[0] != r'off', value1=caching_enabled )
        ...
        try:
            for option in [CachePreferenceMode(), CachePreferenceFillType(), CachePreferenceFillDirection(), CachePreferenceHybridCacheMode()]:
                cmds.optionMenu( _menu(self.widgets[option.ov_id]), edit=True, enable=ui_enabled, select=option.find_index( option.get_value() ) )
            ...
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kNoUpdate' ].format( ex ) )
        ...
        cmds.checkBoxGrp( self.widgets[CachePreferenceCacheSmoothMeshes().ov_id],      edit=True, enable=ui_enabled, value1=CachePreferenceCacheSmoothMeshes().get_value() )
        cmds.checkBoxGrp( self.widgets[CachePreferenceDynamicsSupportEnabled().ov_id], edit=True, enable=ui_enabled, value1=CachePreferenceDynamicsSupportEnabled().get_value() )
    ...
    def callback_change_cache_mode(tool):
        ...
        try:
            ...
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kBadMode' ].format(selected_item, ex) )

    #----------------------------------------------------------------------
    @staticmethod
    def callback_change_hybrid_cache_mode(tool):
        '''
        Callback invoked when the hybrid cache mode changes.
        Read the new value for the optionVar and set the new state.
        '''
        assert tool
        
        try:
            selected_item = cmds.optionMenu( _menu(tool.widgets[CachePreferenceHybridCacheMode().ov_id]), query=True, select=True ) - 1
            selected_name = CachePreferenceHybridCacheMode().KEYS[selected_item]
            

            CachePreferenceHybridCacheMode().set_value_directly( tool, selected_name )

        except KeyError as ex:
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kBadHybrid' ].format(selected_item, ex) )
    ...
    def callback_change_cache_fill_type(tool):
        ...
        try:
            ...
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kBadFillType' ].format(selected_item, ex) )
    ...
    def callback_change_cache_fill_direction(tool):
        ...
        try:
            ...
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kBadFillDirection' ].format(selected_item, ex) )
    ...
    def callback_ui_deleted(tool):
        ...
        assert tool.ui_key in CacheUiFullLayout.CONTROLS, maya.stringTable['y_maya_plugin_evaluator_CacheUiFullLayout.kCachePrefControlDelete' ].format( tool.ui_key )
    ...
    #----------------------------------------------------------------------
    @staticmethod
    def callback_change_cache_smooth_meshes(tool):
        '''
        Callback invoked when the cache smooth meshes state changes.
        Read the new value for the optionVar and set the new state.
        '''
        assert tool
        
        cache_smooth_meshes = bool(cmds.checkBoxGrp( tool.widgets[CachePreferenceCacheSmoothMeshes().ov_id], query=True, value1=True ))

        CachePreferenceCacheSmoothMeshes().set_value_directly( tool, cache_smooth_meshes )

    #----------------------------------------------------------------------
    @staticmethod
    def callback_change_dynamics_support_enabled(tool):
        '''
        Callback invoked when the dynamics-support-enabled state changes.
        Read the new value for the optionVar and set the new state.
        '''
        assert tool
        
        dynamics_support_enabled = bool(cmds.checkBoxGrp( tool.widgets[CachePreferenceDynamicsSupportEnabled().ov_id], query=True, value1=True ))

        CachePreferenceDynamicsSupportEnabled().set_value_directly( tool, dynamics_support_enabled )
# ===========================================================================
