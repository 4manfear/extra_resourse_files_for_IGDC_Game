...
__all__ = [ r'CacheUiToggle'
          , r'cache_flush_menu_item'
          , r'cache_preferences_menu_item'
          , r'colour_preferences_menu_item'
          ]
...
def in_em_mode():
    ...
    '''
    return cmds.evaluationManager(query=True,mode=True)[0] != r'off'
...
def callback_open_caching_preferences(value):
    ...
    mel.eval(r'preferencesWnd CachedPlayback')
...
def callback_open_colour_preferences(value):
    ...
    mel.eval(r'source colorPrefWnd.mel; colorPrefOpenToFrame general_cachedPlayback')

#======================================================================
def cache_flush_menu_item():
    '''
    Create a menu item that will flush the cache
    :return: Name of the menuItem widget that was created
    '''
    return cmds.menuItem( label=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kFlushCache' ]
                        , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kFlushCacheTip' ]
                        , command=callback_flush_cache
                        )
...
def cache_preferences_menu_item():
    ...
    :return: Name of the menuItem widget that was created
    '''
    return cmds.menuItem( label=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kPrefCachingTitle' ]
                        , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kPrefCachingTip' ]
                        , command=callback_open_caching_preferences
                        )

#======================================================================
def colour_preferences_menu_item():
    '''
    Create a menu item that will open the preferences window to the Colors section
    :return: Name of the menuItem widget that was created
    '''
    return cmds.menuItem( label=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kPrefColoursTitle' ]
                        , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kPrefColoursTip' ]
                        , command=callback_open_colour_preferences
                        )

#======================================================================
class CacheUiToggle(with_metaclass(Singleton, CacheUiBase)):
    '''
    ...
    STATE_DISABLED    = 6
    #
    ICON_ON   = r'cachedPlayback.png'
    ICON_WARN = r'cachedPlaybackWarning.png'
    OKAY_TIP =   maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kCachedPlaybackButtonOkayTip' ]
    #
    ...
    STATE_INFO = { STATE_OKAY        : [True,  ICON_ON,   OKAY_TIP]
                 , STATE_SAFE_MODE   : [True,  ICON_WARN, maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kCachedPlaybackButtonSafeTip'       ]]
                 , STATE_EM_OFF      : [False, ICON_ON,   maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kCachedPlaybackEmOffTip'            ]]
                 , STATE_LOW_MEMORY  : [True,  ICON_WARN, maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kCachedPlaybackButtonLowMemoryTip'  ]]
                 , STATE_NO_MEMORY   : [True,  ICON_WARN, maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kCachedPlaybackButtonNoMemoryTip'   ]]
                 , STATE_CACHING_OFF : [True,  ICON_ON,   OKAY_TIP]
                 , STATE_DISABLED    : [True,  ICON_WARN, maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kCachedPlaybackButtonEMDisabledTip' ]]
                 }
    #----------------------------------------------------------------------
    ...
    def __init__(self):
        ...
        self.ui_key = r'cache_preferences_playback_range_button'
    ...
    def rebuild_popup_menu(self):
        ...
        # -----
        cmds.setParent( r'..', menu=True )
        cmds.menuItem( divider=True )
        # -----
        self.widgets[self.WID_FLUSH_CACHE]  = cache_flush_menu_item()
        cmds.menuItem( divider=True )
        # -----
        self.widgets[self.WID_PREF_CACHING] = cache_preferences_menu_item()
        self.widgets[self.WID_PREF_COLOURS] = colour_preferences_menu_item()

        self.widgets[self.WID_UNSUPPORTED_WEB] = cmds.menuItem( label=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kUnsupportedNodesWebTitle' ]
                                                                , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiToggle.kUnsupportedNodesWebTip' ]
                                                                , command=lambda tool: cmds.showHelp( r'CachedPlaybackUnsupportedNodes' )
                                                                )
        # Hide the cache limitation issue if safe mode is not active
        cmds.menuItem(self.widgets[self.WID_UNSUPPORTED_WEB]
                    , edit=True
                    , visible=use_safe_mode())
    ...
    def create_button(self):
        ...
        self.mode_change_job = cmds.scriptJob( event=(r'customEvaluatorChanged', callback_tool(self, self.callback_update_ui)) )
    ...
    def update_popup_menu(self):
        '''
        Update the popup menu that appears over the icon.
        '''
        

        current_mode = CachePreferenceMode().get_value()
        for mode_data in CachePreferenceMode.DATA:
            if mode_data[CachePreferences.KEY_ENUM_ID] in self.widgets:
                cmds.menuItem(self.widgets[mode_data[CachePreferences.KEY_ENUM_ID]]
                            , edit = True
                            , radioButton=(current_mode == mode_data[CachePreferences.KEY_ENUM_ID]))
    ...
    def callback_update_ui(tool):
        ...
        if em_off:              # If in DG mode no other states will exist so it is first
            ...
            if disable_info is not None:
                ...
                if new_resource_state == r'low':
                    state = tool.STATE_LOW_MEMORY
                elif new_resource_state == r'out':
