...
#======================================================================
class CacheWarningMessages(with_metaclass(Singleton, CachePerformanceTrace)):
    '''
    ...
    def __init__(self):
        ...
        '''
        super(CacheWarningMessages,self).__init__()
        self.silenced = {}
    ...
    def messaging_wanted(self):
        ...
        if not self.cache_mgr.plugin_loaded or not self.cache_mgr.enabled:
            self.dbg( 'Skip warning: Cache evaluator is not enabled' )
            return False
        ...
        if not cmds.evaluationManager( query=True, enabled=True ):
            self.dbg( 'Skip warning: Evaluation manager is not enabled' )
            return False
        ...
        if cmds.evaluationManager( query=True, empty=True ):
            self.dbg( 'Skip warning: Evaluation manager is empty' )
            return False
    ...
    def callback_reset_message_warnings(tool):
        ...
        tool.dbg( 'callback_reset_message_warnings' )
    ...
    def callback_evaluation_mode_changed(tool):
        ...
        tool.dbg( u'callback_evaluation_mode_changed from {} to {}'.format( previous_evaluation_mode, tool.last_evaluation_mode) )
        tool.dbg( u'----------- last_had_animation from {} to {}'.format( previous_had_animation, tool.last_had_animation) )
        ...
        if not tool.last_evaluation_mode and previous_evaluation_mode and previous_had_animation and CachePreferenceShowWarningMessages().get_value():
            print(maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kWarnEMOffMsg' ])
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kWarnEMOff' ] )
    ...
    def callback_em_disabled_changed(tool):
        ...
        '''
        tool.dbg( 'callback_em_disabled_changed' )
        # If caching isn't turned on then no message should be produced
        if not tool.cache_mgr.plugin_loaded or not tool.cache_mgr.enabled:
            return
        ...
        tool.dbg( u'callback_em_disable_changed from {} to {}'.format( previous_disable_message, tool.last_disable_message) )
        ...
        if tool.last_disable_message is not None and CachePreferenceShowWarningMessages().get_value() and tool.last_disable_message != previous_disable_message:
            ...
            cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kWarnEMDisabled' ] )
    ...
    def callback_safe_mode_event(tool):
        ...
        '''
        tool.dbg( 'callback_safe_mode_event' )
        ...
        if messages_needed:
            tool.dbg( '...trigger messages changed' )
            tool.last_safe_messages = new_safe_messages
            if CachePreferenceShowWarningMessages().get_value():
                if new_safe_messages is None:
                    new_safe_messages = maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kSafeModeUnknown' ]
                print(new_safe_messages)
                if warning_needed:
                    cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kSafeModeTriggered' ] )
            else:
                tool.dbg( '--> Suppressed the warning' )
    ...
    def callback_memory_state_changed(tool):
        ...
        '''
        tool.dbg( 'callback_memory_state_changed' )
            ...
            if new_resource_state != tool.last_resource_state:
                if new_resource_state == 'low':
                    if tool.MSG_MEMORY_LOW not in tool.silenced:
                        print( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kMemoryLow'] )
                        cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kLowMemoryWarning' ] )
                    tool.silenced[tool.MSG_MEMORY_LOW] = True
                elif new_resource_state == 'out':
                    if tool.MSG_MEMORY_OUT not in tool.silenced:
                        print( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kMemoryOut'] )
                        cmds.warning( maya.stringTable['y_maya_plugin_evaluator_CacheWarningMessages.kNoMemoryWarning' ] )
