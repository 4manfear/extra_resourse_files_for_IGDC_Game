...
from maya.plugin.evaluator.cache_optionvar_states import CachePreferenceHud, CachePreferenceResourceGuard, CachePreferenceMemoryThreshold, CachePreferenceDiscardFramesOutOfRange, CachePreferenceCacheSmoothMeshes
...
from maya.plugin.evaluator.cache_debugging import CachePerformanceTrace, CP_DBG

__all__ = [ r'cache_preferences_initialize'
          , r'cache_ui_enabled'
          , r'CachePreferences'
          , r'CachePreferenceEnabled'
          , r'CachePreferenceMode'
          , r'CachePreferenceFillType'
          , r'CachePreferenceFillDirection'
          , r'CachePreferencePreventFrameSkipping'
          , r'CachePreferenceDynamicsSupportEnabled'
          , r'CachePreferenceDynamicsAsyncRefresh'
          , r'CachePreferenceHybridCacheMode'
          ]
...
#======================================================================
class CachePreferenceWithTrace(CachePerformanceTrace, OptionVar):
    '''Intermediate class to handle multiple inheritance from trace object and OptionVar'''
    def __init__(self, ov_id, option_type, option_default):
        '''Initialize the two base classes in a way compatible with both Python 2.7 and 3.8'''
        CachePerformanceTrace.__init__(self)
        OptionVar.__init__(self, ov_id, option_type, option_default, PREF_CATEGORY)

#======================================================================
class CachePreferenceEnabled(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "caching playback enabled" preference
    '''
    ov_id = r'cachedPlaybackEnable' # optionVar name and unique preference ID

    def __init__(self):
        ''' Initialize the preference interface '''
        super( CachePreferenceEnabled, self ).__init__(self.ov_id, OPTION_VAR_TYPE_BOOL, True)
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCachedPlaybackEnableText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCachedPlaybackEnableTip'  ]

    def read_preference_from_state(self):
        '''Read the new value of the optionVar from the scene state'''
        
        return self.set_value( CachePreferences().cache_mgr.enabled )
...
#======================================================================
class CachePreferencePreventFrameSkipping(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "caching playback prevent frame skipping" preference
    '''
    ov_id = r'cachedPlaybackPreventFrameSkipping' # optionVar name and unique preference ID

    def __init__(self):
        ''' Initialize the preference interface '''
        super( CachePreferencePreventFrameSkipping, self ).__init__(self.ov_id, OPTION_VAR_TYPE_BOOL, True)
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kPreventFrameSkippingText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kPreventFrameSkippingTip'  ]

    def read_preference_from_state(self):
        '''Read the new value of the optionVar from the scene state'''
        
        return self.set_value( CachePreferences().cache_mgr.prevent_frame_skipping )
...
#======================================================================
class CachePreferenceMode(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "caching playback mode" preference
    '''
    ov_id = r'cachedPlaybackMode' # optionVar name and unique preference ID

    # This could be a dictionary but it's important to preserve the ordering so it's a list of lists.
    DATA = [ [ r'evaluationCache'  , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheModeEval'    ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheModeEvalTip' ]
                                   , CACHE_STANDARD_MODE_EVAL
                                   ]
           , [ r'viewportSoftware' , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheModeVpS'    ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheModeVpSTip' ]
                                   , CACHE_STANDARD_MODE_VP2_SW
                                   ]
           , [ r'viewportHardware' , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheModeVpH'    ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheModeVpHTip' ]
    ...
    def __init__(self):
        ...
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kPreferredCacheModeText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kPreferredCacheModeTip'  ] + _aggregate_enum_info(CachePreferenceMode.DATA)
    ...
    def read_preference_from_state(self):
        ...
        for enum_data in CachePreferenceMode.DATA:
            if cache_mode == enum_data[CachePreferences.KEY_ENUM_DATA]:
                value_changed = self.set_value( enum_data[CachePreferences.KEY_ENUM_ID] )
...
#======================================================================
class CachePreferenceFillType(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "caching playback fill type" preference
    '''
    ov_id = r'cachedPlaybackFillType' # optionVar name and unique preference ID

    # This could be a dictionary but it's important to preserve the ordering so it's a list of lists.
    DATA = [ [ r'syncAsync' , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillBoth'    ]
                            , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillBothTip' ]
                            ]
           , [ r'syncOnly'  , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillPlay'    ]
                            , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillPlayTip' ]
                            ]
           , [ r'asyncOnly' , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillBg'      ]
                            , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillBgTip'   ]
                            ]
           ]
    # The first element of the list is treated as a dictionary key
    KEYS = [data[0] for data in DATA]
    ...
    def __init__(self):
        ...
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillTypeText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillTypeTip'  ] + _aggregate_enum_info(CachePreferenceFillType.DATA)
    ...
    def read_preference_from_state(self):
        ...
        return self.set_value( CachePreferences().cache_mgr.fill_mode )
...
#======================================================================
class CachePreferenceFillDirection(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "caching playback fill direction" preference
    '''
    ov_id = r'cachedPlaybackFillDirection' # optionVar name and unique preference ID

    # This could be a dictionary but it's important to preserve the ordering so it's a list of lists.
    DATA = [ [ r'bidirectional'    , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillFwdBwd'          ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillFwdBwdTip'       ]
                                   ]
           , [ r'forward'          , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillFwd'             ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillFwdTip'          ]
                                   ]
           , [ r'backward'         , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillBwd'             ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillBwdTip'          ]
                                   ]
           , [ r'forwardFromBegin' , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillFwdFromStart'    ]
                                   , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kBgFillFwdFromStartTip' ]
                                   ]
           ]
    ...
    def __init__(self):
        ...
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillDirectionText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheFillDirectionTip'  ] + _aggregate_enum_info(CachePreferenceFillDirection.DATA)
    ...
    def read_preference_from_state(self):
        ...
        return self.set_value( CachePreferences().cache_mgr.fill_order )
...
#======================================================================
class CachePreferenceDynamicsSupportEnabled(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "enable dynamics support" preference
    '''
    ov_id = r'cachePreferenceDynamicsSupportEnabled' # optionVar name and unique preference ID

    def __init__(self):
        ''' Initialize the preference interface '''
        super( CachePreferenceDynamicsSupportEnabled, self ).__init__(self.ov_id, OPTION_VAR_TYPE_BOOL, True)
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kEnableDynamicsSupportText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kEnableDynamicsSupportTip'  ]

    def read_preference_from_state(self):
        '''Read the new value of the optionVar from the scene state'''
        
        return self.set_value( CachePreferences().cache_mgr.dynamics_support_enabled )

    def set_state_from_preference(self):
        '''Apply the current value of the optionVar to the scene state'''
        
        CachePreferences().cache_mgr.dynamics_support_enabled = self.get_value()

#======================================================================
class CachePreferenceDynamicsAsyncRefresh(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "caching playback simulation asynchronous refresh" preference
    '''
    ov_id = r'cachedPlaybackDynamicsAsyncRefresh' # optionVar name and unique preference ID

    def __init__(self):
        ''' Initialize the preference interface '''
        super( CachePreferenceDynamicsAsyncRefresh, self ).__init__(self.ov_id, OPTION_VAR_TYPE_BOOL, True)
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kDynamicsAsyncRefreshText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kDynamicsAsyncRefreshTip'  ]

    def read_preference_from_state(self):
        '''Read the new value of the optionVar from the scene state'''
        
        return self.set_value( CachePreferences().cache_mgr.dynamics_async_refresh )

    def set_state_from_preference(self):
        '''Apply the current value of the optionVar to the scene state'''
        
        CachePreferences().cache_mgr.dynamics_async_refresh = self.get_value()

#======================================================================
class CachePreferenceHybridCacheMode(with_metaclass(Singleton, CachePreferenceWithTrace)):
    '''
    Class containing the information for the "hybrid cache mode" preference
    '''
    ov_id = r'cachedPlaybackHybridCacheMode' # optionVar name and unique preference ID

    # This could be a dictionary but it's important to preserve the ordering so it's a list of lists.
    DATA = [ [ r'disabled' , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeDisabled'    ]
                           , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeDisabledTip' ]
                           ]
           , [ r'smp'      , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeSMP'         ]
                           , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeSMPTip'      ]
                           ]
           , [ r'all'      , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeAll'         ]
                           , maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeAllTip'      ]
                           ]
           ]
    # The first element of the list is treated as a dictionary key
    KEYS = [data[0] for data in DATA]
    ...
    def __init__(self):
        ''' Initialize the preference interface '''
        super( CachePreferenceHybridCacheMode, self ).__init__(self.ov_id, CachePreferenceHybridCacheMode.KEYS, CachePreferenceHybridCacheMode.KEYS[0] )
        self.title = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeText' ]
        self.info  = maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kCacheHybridCacheModeTip'  ] + _aggregate_enum_info(CachePreferenceHybridCacheMode.DATA)
        self.lookup = { hybrid_cache_mode : i+1 for i, hybrid_cache_mode in enumerate(CachePreferenceHybridCacheMode.KEYS) }
    ...
    def read_preference_from_state(self):
        ...
        return self.set_value( CachePreferences().cache_mgr.hybrid_cache_mode )

    def set_state_from_preference(self):
        '''Apply the current value of the optionVar to the scene state'''
        

        new_hybrid_cache_mode = self.get_value()
        CachePreferences().cache_mgr.hybrid_cache_mode = new_hybrid_cache_mode

#======================================================================
class CachePreferences(with_metaclass(Singleton, CachePerformanceTrace)):
    '''
    ...
    def add_preference(self, option_var):
        ...
        option_var.set_in_category( r'preferences', True )
    ...
    def callback_plugin_loaded(plugin_name):
        ...
        if plugin_name == CACHE_PLUGIN_NAME:
            ...
            for option_var in list(CachePreferences().option_vars.values()):
    ...
    def monitor_state_changes(self, enable_listening):
        ...
        if enable_listening:
            ...
            if r'cachingPreferencesChanged' in script_job_events:
                self.pref_change_job = cmds.scriptJob(event=(r'cachingPreferencesChanged', callback_tool(self, self.callback_state_changed)))
    ...
    def update_plugin_clients(self, new_state):
        ...
        '''
        for (client, plugin_callback) in list(self.plugin_clients.values()):
    ...
    def callback_flush_frames_out_of_range(tool):
        ''' Invoked when the visible frame range changes - decides if the cache should flush frames out of the new range '''
    ...
    @staticmethod
    def callback_cache_smooth_meshes_changed(tool):
        ''' Invoked when the "Cache smooth meshes" optionVar changes'''
        # When "Cache smooth meshes" changes, invalidate partitioning and flush
        # the cache so cache config rules are properly re-applied and smooth
        # meshes are properly cached/not cached.
        if tool.cache_mgr.plugin_loaded:
    ...
    def __init__(self):
        ...
        super(CachePreferences, self).__init__()
        self.option_vars = {}
        self.pref_change_job = None
        self.monitor_load = False
        self.monitor_unload = False
        ...
        try:
            ...
            self.add_preference( CachePreferenceDynamicsSupportEnabled() )
            self.add_preference( CachePreferenceDynamicsAsyncRefresh() )
            ...
            self.add_preference( CachePreferenceCacheSmoothMeshes() )
            self.add_preference( CachePreferenceHybridCacheMode() )
            self.add_non_preference( CachePreferenceHud() )
        except Exception as ex:
            cmds.error( maya.stringTable['y_maya_plugin_evaluator_cache_preferences.kErrPrefs' ].format( ex ) )

        # Start listening to playback range changes, for when the cache will be flushed outside of the current range
        cmds.scriptJob( event=(r'playbackRangeChanged', callback_tool(self, self.callback_flush_frames_out_of_range)) )
        CachePreferenceDiscardFramesOutOfRange().add_client( self, callback_tool(self, self.callback_flush_frames_out_of_range) )

        # Start listening to "Cache smooth meshes" changes
        CachePreferenceCacheSmoothMeshes().add_client( self, callback_tool(self, self.callback_cache_smooth_meshes_changed) )
...
def cache_ui_enabled():
    ...
    '''
    enabled = CachePreferenceEnabled().get_value() and cmds.evaluationManager( query=True, mode=True )[0] != r'off'
