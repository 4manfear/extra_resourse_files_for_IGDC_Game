...
from maya.plugin.evaluator.CacheUiToggle import cache_flush_menu_item, cache_preferences_menu_item, colour_preferences_menu_item

__all__ = [r'CacheUiMenu']
...
class CacheUiMenu(CacheUiBase):
    ...
    ID_FLUSH             = r'flushCachedPlaybackCache'
    ID_OPEN_CACHE_PREFS  = r'openCachedPlaybackPreferences'
    ID_OPEN_COLOUR_PREFS = r'openColourPreferences'
    ...
    def __init__(self, ui_key):
        ...
        assert ui_key not in self.CONTROLS, maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kCachePrefMenuExists' ].format( ui_key )
        ...
        self.widgets[self.ID_ROOT] = cmds.menuItem( label=maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kFrameCachedPlayback' ], subMenu=True, tearOff=True, version=r'2019' )

        caching_enabled = CachePreferenceEnabled().get_value()
        control = cmds.menuItem( label=maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kCachedPlaybackEnableText' ]
                               , annotation=maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kCachedPlaybackEnableTip'  ]
        ...
        cmds.menuItem( divider=True )
        self.widgets[self.ID_FLUSH] = cache_flush_menu_item()
        cmds.menuItem( divider=True )
        self.widgets[self.ID_OPEN_CACHE_PREFS] = cache_preferences_menu_item()
        self.widgets[self.ID_OPEN_COLOUR_PREFS] = colour_preferences_menu_item()
        ...
        self.mode_change_job = cmds.scriptJob(event=(r'customEvaluatorChanged', update_menu_callback))
    ...
    def __str__(self):
        ...
        '''
        dump  = maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kMenuTitle'  ]
        dump += maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kMenuKey'    ].format( self.ui_key )
        dump += maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kMenuParent' ].format( self.parent_menu )
        dump += maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kMenuJob'    ].format( self.mode_change_job )
        dump += maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kMenuWidgets'].format( r'\n            '.join( [r'{} = {}'.format(key,value) for key,value in self.widgets.items()] ) )
        return dump
    ...
    def menu_deleted(tool):
        ...
        assert tool.ui_key in tool.CONTROLS, maya.stringTable['y_maya_plugin_evaluator_CacheUiMenu.kCachePrefMenuDelete' ].format( tool.ui_key )
