...
__all__ = [ 'cache_ui_full_layout_create'
          , 'cache_ui_full_layout_update'
          , 'cache_ui_menu_create'
          , 'cache_ui_preferences_tab_create'
          , 'cache_ui_preferences_tab_update'
          , 'cache_ui_preferences_list_item_create'
          , 'cache_ui_toggle_create'
          , 'cache_ui_hud_create'
          , 'cache_ui_hud_menu_item_create'
          , 'cache_ui_colour_preferences_create'
          , 'cache_ui_colour_preferences_update'
          , 'cache_ui_colour_preferences_populate'
          ]
...
def cache_ui_full_layout_create(ui_key):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_full_layout_create({})'.format(ui_key) )
    # If the named control already exists then just update it. (This would happen if the
    # preferences window were minimized then restored for example.)
    # If not then create it.
    if ui_key in CacheUiFullLayout.CONTROLS:
        CacheUiFullLayout.CONTROLS[ui_key].update_ui()
    else:
        CacheUiFullLayout(ui_key)
...
def cache_ui_full_layout_update(ui_key):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_full_layout_update({})'.format(ui_key) )
    if ui_key in CacheUiFullLayout.CONTROLS:
        CacheUiFullLayout.CONTROLS[ui_key].update_ui()
    else:
        CP_DBG.dbg( '--> No UI to update' )
...
def cache_ui_menu_create(ui_key):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_menu_create({})'.format(ui_key) )
    # If the named control already exists then just update it, otherwise create it.
    if ui_key in CacheUiMenu.CONTROLS:
        CacheUiMenu.CONTROLS[ui_key].update_menu()
    else:
        CacheUiMenu(ui_key)
...
def cache_ui_preferences_tab_create(prefs_template):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_preferences_tab_create( {} )'.format(prefs_template) )
    return CacheUiPreferencesTab().create_ui( prefs_template )
...
def cache_ui_preferences_list_item_create(prefs_window, text_scroll_widget, text_scroll_index):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_preferences_list_item_create( {}, {}, {} )'.format(prefs_window, text_scroll_widget, text_scroll_index) )
    return CacheUiPreferencesTab().create_list_entry( prefs_window, text_scroll_widget, text_scroll_index )
...
def cache_ui_preferences_tab_frame_create():
    ...
    '''
    CP_DBG.dbg( 'cache_ui_preferences_tab_frame_create()' )
    CacheUiPreferencesTab().create_frame()
...
def cache_ui_preferences_tab_update():
    ...
    '''
    CP_DBG.dbg( 'cache_ui_preferences_tab_update' )
    CacheUiPreferencesTab().update_ui()
...
def cache_ui_toggle_create():
    ...
    '''
    CP_DBG.dbg( 'cache_ui_toggle_create' )
    return CacheUiToggle().create_button()
...
def cache_ui_hud_create(section, label_width, starting_block):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_hud_create' )
    return CacheUiHud().create_heads_up_display(section , label_width, starting_block )
...
def cache_ui_hud_menu_item_create(previous_item):
    ...
    '''
    CP_DBG.dbg( 'cache_ui_hud_menu_item_create' )
    return CacheUiHud().create_hud_menu_item(previous_item)
...
def cache_ui_colour_preferences_create():
    ...
    '''
    CP_DBG.dbg( 'cache_ui_colour_preferences_create' )
    return CacheUiColour().create_frame()
...
def cache_ui_colour_preferences_populate():
    ...
    '''
    CP_DBG.dbg( 'cache_ui_colour_preferences_populate' )
    CacheUiColour().callback_populate_frame( CacheUiColour() )
