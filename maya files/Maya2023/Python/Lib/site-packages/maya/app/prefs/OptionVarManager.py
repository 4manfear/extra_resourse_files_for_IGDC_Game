...
in the manager
    ...
    my_option_var = OptionVarManager.option_var( 'intVar' )
'''
...
class OptionVarManager(object):
    ...
    def as_str(cls):
        ...
        for option_var in list(cls.option_vars.values()):
            values += '\n    ' + str(option_var)
        return values
    ...
    def option_var(cls, option_var_name):
        ...
        if option_var_name not in cls.option_vars:
            raise KeyError( maya.stringTable['y_maya_app_prefs_OptionVarManager.kOptionVarUnmanagedGet' ].format( option_var_name ) )

        return cls.option_vars[option_var_name]

    #----------------------------------------------------------------------
    @classmethod
    def remember_states(cls, category=None):
        '''
        Remember the current states of all managed optionVars
        :param category: Category filter, process everything if None
        '''
        for option_var in list(cls.option_vars.values()):
            if option_var.in_category(category):
                option_var.remember_current()

    #----------------------------------------------------------------------
    @classmethod
    def restore_states(cls, category=None):
        '''
    ...
    #----------------------------------------------------------------------
    @classmethod
    def revert_to_defaults(cls, category=None):
        '''
    ...
    #----------------------------------------------------------------------
    @classmethod
    def read_preferences_from_state(cls, category=None):
        '''
        Update all of the optionVar values to match the Maya state.
        (Called after preference modifications were undone or cancelled.)
        :param category: Category filter, process everything if None
        '''
        for option_var in list(cls.option_vars.values()):
            if option_var.in_category(category):
    ...
    #----------------------------------------------------------------------
    @classmethod
    def set_state_from_preferences(cls, category=None):
        '''
