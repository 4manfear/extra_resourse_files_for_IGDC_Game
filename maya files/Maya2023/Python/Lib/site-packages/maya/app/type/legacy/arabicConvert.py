...
def filterAndCorrectArabic(input_string):
    ...
    if sys_version_info.major == 2:
        hexText = unicode(input_string)
    else:
        hexText = input_string
...
def convertChar(ch, offset):
    ...
    if inMergeRange(ch):
        ...
        if MERGE_TYPE[index] == t_start:
            result = chr(ord(MERGE_MAP[index]) + (offset % 2))

        elif MERGE_TYPE[index] == t_end:
            result = chr(ord(MERGE_MAP[index]) + offset)

        elif MERGE_TYPE[index] == t_none:
            result = chr(ord(MERGE_MAP[index]))
...
def convertLamAlef(ch, offset):
    if isAlefChar(ch):
        ...
        return chr(ord(lamAlefMap[c]) + (offset % 2))
