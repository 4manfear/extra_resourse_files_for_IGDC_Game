...
def createTypeTool(font=None, style=None, text=None, legacy = False):
    ...
    createTypeToolWithNode(typeTool, font, style, text, legacy)

def createTypeToolWithNode(typeTool, font=None, style=None, text=None, legacy = False):
    typeExtruder = None
    if legacy:
        typeExtruder = cmds.createNode( 'vectorExtrude', n='typeExtrude#', skipSelect=True)
        cmds.setAttr( typeTool+'.legacyDecomposition', 1)
    else:
        typeExtruder = cmds.createNode( 'typeExtrude', n='typeExtrude#', skipSelect=True)
    typeTransform = cmds.createNode( 'transform', n='typeMesh#', skipSelect=True)
    ...
    if font:
        defaultQFont = qt.QFont(font)
        print(defaultQFont.styleName())
        fontInfo = qt.QFontInfo(defaultQFont)
        if fontInfo.family() != font:
            cmds.warning(maya.stringTable['y_maya.app.type.AEtypeTemplate.kFontNotFound' ])
            font = None
    ...
    if font and style:
        ...
        if style not in allStyles:
            cmds.warning(maya.stringTable['y_maya.app.type.AEtypeTemplate.kStyleNotFound' ])
            style = None
    ...
    blinnShader = cmds.shadingNode('blinn', asShader=True, n="typeBlinn", skipSelect=True)
    defaultColour = [(1, 1, 1)]
    ...
    # #add polyRemesh
    addPolyRemeshNodeType(typeTransform,typeExtruder,typeTool,typeMesh)
    ...
    #assign the shader
    cmds.select(typeTransform)
    cmds.sets(e=True, forceElement=blinnShader + 'SG')

    #don't try to show the UI in batch mode
    if not IN_BATCH_MODE:
        cmds.evalDeferred('''import maya.mel as mel; mel.eval('showEditorExact("%s")') ''' % typeTool)
...
def addPolyRemeshNodeType(typeTransform, typeExtruder, typeTool, typeMesh):
    #cmds.select(typeTransform)
    polyRemeshNode = cmds.polyRemesh(nodeState=1, interpolationType=0, tessellateBorders=0, maxEdgeLength=1.0)[0]
...
def addUVNodeToType(typeMesh, typeExtruder):
    ...
    autoProj = cmds.polyAutoProjection(typeMesh, lm =0 ,pb =0 ,ibd = 0 ,cm =0 ,l =2 ,sc =1 ,o =1 ,p =6 ,ps =0.2 ,ws =0, n="typePolyAutoProj#")

def connectTypeAdjustDeformer(typeMesh,typeTransform, typeExtruder, typeTool):
    newAdjustDeformer = cmds.deformer(typeMesh, type="vectorAdjust")[0]
...
def connectShellDeformer(typeMesh, typeTransform, typeExtruder, typeTool):
    ...
    newTypeDeformer = cmds.deformer(typeMesh, type="shellDeformer")[0]
