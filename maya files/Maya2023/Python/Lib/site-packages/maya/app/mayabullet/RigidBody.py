...
def _longName( obj ):
	...
	if type(obj) not in [bytes, text_type]:
		return obj.longName()
	if text_type(obj[0]) != text_type('|'):
		objs = maya.cmds.ls(obj, long=True)
		if len(objs)>1:
			logger.warn( maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kLongNameAmbiguity' ] % obj )
		return objs[0]
	return obj
@Trace()
def _name( obj ):
	from sys import version_info as sys_version_info
	text_type = str if sys_version_info[0] >= 3 else unicode
	return obj.name() if type(obj) not in [bytes, text_type] else obj.split('|')[-1]
@Trace()
...
class CreateRigidBody( CommandWithOptionVars.CommandWithOptionVars ):
	...
	def __init__(self, isActive=True):
		...
		self.l10nCommandName = maya.stringTable['y_maya_app_mayabullet_RigidBody.kCreateRigidBody' ]
		self.optionVarCategory = 'Bullet.Rigid Body'
		...
		if isActive and (currentType != eBodyType.kDynamicRigidBody):
			maya.cmds.optionVar(category=self.optionVarCategory, iv=(bodyTypeVar, eBodyType.kDynamicRigidBody))
		elif (not isActive) and (currentType != eBodyType.kStaticBody):
			maya.cmds.optionVar(category=self.optionVarCategory, iv=(bodyTypeVar, eBodyType.kStaticBody))
	...
	def command(*args, **kwargs ):
		...
		transformNames = []
		if ('bAttachSelected' in kwargs and not kwargs['bAttachSelected']):
		...
		if ('ignoreShape' in kwargs and kwargs['ignoreShape']):
			shapes = []
		else:
		...
		if len(transformNames):
			...
			if ('colliderShapeType' in kwargs):
			...
			for transformName in transformNames:
				...
				if transformName and BulletUtils.getRigidBodyFromTransform(transformName):
					OpenMaya.MGlobal.displayWarning(maya.stringTable['y_maya_app_mayabullet_RigidBody.kAlreadyBulletObject2' ].format(transformName) )
					continue
				...
				if transformName and 'autoFit' in kwargs and kwargs['autoFit']:
	...
	def doCommand(name='bulletRigidBodyShape#',
		...
		if not rbXform:
			OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kErrorRigidBodyNotCreated' ])
			return [ None, None ]
	...
	def command_shape(**kwargs):
		...
		for s in shapes:
			...
			if 'autoFit' in kwargs and kwargs['autoFit']:
	...
	def addOptionDialogWidgets(self):
		...
		# == Collider Properties == 
		widget = maya.cmds.optionMenuGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kColliderType'])

		self.box=maya.stringTable['y_maya_app_mayabullet_RigidBody.kBox']
		sphere=maya.stringTable['y_maya_app_mayabullet_RigidBody.kSphere']
		capsule=maya.stringTable['y_maya_app_mayabullet_RigidBody.kCapsule']
		hull=maya.stringTable['y_maya_app_mayabullet_RigidBody.kHull']
		mesh=maya.stringTable['y_maya_app_mayabullet_RigidBody.kMesh']
		plane=maya.stringTable['y_maya_app_mayabullet_RigidBody.kPlane']
		cylinder=maya.stringTable['y_maya_app_mayabullet_RigidBody.kCylinder']
		hacd=maya.stringTable['y_maya_app_mayabullet_RigidBody.kHACD']
		compound=maya.stringTable['y_maya_app_mayabullet_RigidBody.kCompound']
		none=maya.stringTable['y_maya_app_mayabullet_RigidBody.kNone']
		...
		widget = maya.cmds.optionMenuGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kAxisType'  ])
		xAxis=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kXAxis']
		yAxis=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kYAxis']
		zAxis=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kZAxis']
		...
		widget = maya.cmds.checkBoxGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kAutoFit'], changeCommand=self.autoFitCB, 
									   numberOfCheckBoxes=1)
		widgetDict['autoFit'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kAutoHide'], changeCommand=self.hideShapeCB, 
									   numberOfCheckBoxes=1)
		widgetDict['hideShape'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kLength'  ],
		...
		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kRadius'  ],
										  minValue=0, maxValue=10, fieldMaxValue=9999999)
		widgetDict['radius'] = (maya.cmds.floatSliderGrp, widget)
		maya.cmds.control(widget, edit=True, enable=False)
		
		widget = maya.cmds.floatFieldGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kExtents'  ],
										 numberOfFields=3)
		widgetDict['extents'] = (maya.cmds.floatFieldGrp, widget)
		maya.cmds.control(widget, edit=True, enable=False)
		
		# == RigidBody Properties== 
		widget = maya.cmds.optionMenuGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kBodyType'  ], changeCommand=self.bodyTypeCB)
		staticBody=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kStaticBody']
		kinRigidBody=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kKinematicRigidBody']
		dynRigidBody=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kDynRigidBody' ]
		...
		widget = maya.cmds.checkBoxGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kNeverSleeps'],
									   numberOfCheckBoxes=1)
		widgetDict['neverSleeps'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kMass'],
										  minValue=0, maxValue=10, fieldMaxValue=9999999)
		widgetDict['mass'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kLinearDamp' ],
										  minValue=0, maxValue=1)
		widgetDict['linearDamping'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatFieldGrp( label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kCenterOfMass' ],
										  numberOfFields=3 )
		widgetDict['centerOfMass'] = (maya.cmds.floatFieldGrp, widget)
		
		widget = maya.cmds.floatFieldGrp( label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kColliderShapeOffset' ],
										  numberOfFields=3 )
		widgetDict['colliderShapeOffset'] = (maya.cmds.floatFieldGrp, widget)
		
		widget = maya.cmds.floatSliderGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kColliderShapeMargin'],
										  minValue=0, maxValue=10, fieldMaxValue=9999999)
		widgetDict['colliderShapeMargin'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kAngDamp'],
										  minValue=0, maxValue=1)
		widgetDict['angularDamping'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kFriction'],
										  minValue=0, maxValue=1)
		widgetDict['friction'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kRestitution'],
										  minValue=0, maxValue=1)
		widgetDict['restitution'] = (maya.cmds.floatSliderGrp, widget)
		
		# Initial Conditions
		widget = maya.cmds.checkBoxGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kInitSleep'],
									   numberOfCheckBoxes=1)
		widgetDict['initiallySleeping'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.floatFieldGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kInitVel'],
										 numberOfFields=3)
		widgetDict['initialVelocity'] = (maya.cmds.floatFieldGrp, widget)

		widget = maya.cmds.floatFieldGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kInitAngVel'],
										 numberOfFields=3)
		widgetDict['initialAngularVelocity'] = (maya.cmds.floatFieldGrp, widget)
				
		# Forces
		widget = maya.cmds.floatFieldGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kImpulse'],
										 numberOfFields=3)
		widgetDict['impulse'] = (maya.cmds.floatFieldGrp, widget)

		widget = maya.cmds.floatFieldGrp(label=maya.stringTable['y_maya_app_mayabullet_RigidBody.kTorqueImpulse'],
										 numberOfFields=3)
	...
	def updateOptionBox(self):
		...
		if hasMeshes:
			for (label, item) in list(self.colliderOptionWidgets.values()):
...
class CreateRigidSet( CommandWithOptionVars.CommandWithOptionVars ):
	...
	def __init__( self, sl=True ):
		...
		self.l10nCommandName   = maya.stringTable['y_maya_app_mayabullet_RigidBody.kCreateRigidSet' ]
		self.optionVarCategory = 'Bullet.Rigid Set'
	...
	def addOptionDialogWidgets( self ):
		...
		widget = maya.cmds.textFieldGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kSetName'  ])
		widgetDict['name'] = (maya.cmds.textFieldGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kInputSet'  ],
									annotation=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kInputSetAnnot'  ],
									numberOfCheckBoxes=1)
		widgetDict['inputSet'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kOutputMesh'  ],
									annotation=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kOutputMeshAnnot'  ],
									numberOfCheckBoxes=1)
		widgetDict['outputMesh'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kSelectedObjects'  ],
									annotation=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kSelectedObjectsAnnot'  ],
									numberOfCheckBoxes=1)
		widgetDict['sl'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kHideInputGeometry'  ],
									annotation=maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kHideInputGeometryAnnot'  ],
									numberOfCheckBoxes=1)
		widgetDict['hideShape'] = (maya.cmds.checkBoxGrp, widget)
	...
	def executeCommandCB(self, miscBool=None):
		...
		'''
		logger.info(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kGetOptVarValues'  ])
		optionVarDict = self.getOptionVars()

		# REVISIT: May want to pass in parameters to the command a different way
		logger.info(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kExecuteCmd'  ])
	...
	def addToRigidSet( *args, **kw ):
		...
		if len(rbSets)>1:
			OpenMaya.MGlobal.displayError(maya.stringTable['y_maya_app_mayabullet_RigidBody.kAmbiguousAddToSet' ])
			return 

		if len(rbSets)==0:
			OpenMaya.MGlobal.displayError(maya.stringTable['y_maya_app_mayabullet_RigidBody.kNoRigidSetToAddSelected' ])
			return 

		addObjects = kw['members'] if 'members' in kw else maya.cmds.ls(sl=True, type='transform')
		hideShape = kw['hideShape'] if 'hideShape' in kw else True
		...
		for object in addObjects:
			if BulletUtils.getRigidBodyFromTransform(object):
				OpenMaya.MGlobal.displayInfo(maya.stringTable['y_maya_app_mayabullet_RigidBody.kAlreadyRigidBody' ].format(object))
	...
	def removeFromRigidSet( *args, **kw ):
		...
		removeObjects = kw['members'] if 'members' in kw else maya.cmds.ls(sl=True, type='transform')
		excludeSets = kw['excludeSets'] if 'excludeSets' in kw else []
	...
	def selectRigidSetMembers( *args, **kw ):
		...
		if len(rbSets)==0:
			OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kErrorNoRigidSetSelected'  ])
			return
	...
	def selectRigidSetFromMembers( *args, **kw ):
		...
		if len(objects)==0:
			OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kErrorNoRigidSetMemberSelected'  ])
			return
	...
	def clearRigidSets( *args, **kw ):
		...
		if len(rbSets)==0:
			OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kErrorClearNoRigidSetSelected'  ])
			return
	...
	def deleteRigidSets( *args, **kw ):
		...
		if len(rbSets)==0:
			OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_RigidBody.kErrorDeleteNoRigidSetSelected'  ])
			return
	...
	def extractFromRigidSet( *args, **kw ):
		...
		memberObjects = set(kw['members'] if 'members' in kw else maya.cmds.ls(sl=True, type='transform'))
		...
		for rbSet in rbSets:
			...
			if len(extractObjects):
				...
				for attr, mappedAttr in list(CreateRigidSet.extractableAttrs.items()):
					attrVal =_getAttr(rbInitialState, attr)
					# getAttr returns [()] for vector values
					if type(attrVal) in [list] and type(attrVal[0]) in [tuple]:
						attrVal = attrVal[0]
