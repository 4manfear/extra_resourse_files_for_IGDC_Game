...
class CreateSoftBody(CommandWithOptionVars.CommandWithOptionVars):
	def __init__(self):
		...
		self.l10nCommandName = maya.stringTable['y_maya_app_mayabullet_SoftBody.kCreateSoftBody' ]
		self.optionVarCategory = 'Bullet.Soft Body'
	...
	def command(meshes=None,
		...
		# Sanity Checking
		if len(meshes) == 0:
			maya.OpenMaya.MGlobal.displayError(maya.stringTable['y_maya_app_mayabullet_SoftBody.kSelectMesh' ])
	...
	def addOptionDialogWidgets(self):
		...
		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kGenBendConstraints'  ],
									   numberOfCheckBoxes=1)
		widgetDict['generateBendConstraints'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kSelfCollision'  ],
									   numberOfCheckBoxes=1)
		widgetDict['selfCollision'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kBendResistance'  ],
										  minValue=0, maxValue=1)
		widgetDict['bendResistance'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kLinearStiffness'  ],
										  minValue=.01, maxValue=1)
		widgetDict['linearStiffness'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kFriction'  ],
										  minValue=.01, maxValue=1)
		widgetDict['friction'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kDamping'  ],
										  minValue=.01, maxValue=1)
		widgetDict['damping'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kPressure'  ],
										  minValue=0, maxValue=10)
		widgetDict['pressure'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.floatSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kCollisionMargin'  ],
										  minValue=0.01, maxValue=1)
		widgetDict['collisionMargin'] = (maya.cmds.floatSliderGrp, widget)

		widget = maya.cmds.intSliderGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kPositionIterations'  ],
		...
		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kPreserveSourceMesh'  ],
									   numberOfCheckBoxes=1)
		widgetDict['preserveSourceMesh'] = (maya.cmds.checkBoxGrp, widget)

		widget = maya.cmds.checkBoxGrp(label=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kSingleTransform'  ],
									   numberOfCheckBoxes=1)
...
def _loadUIString(strId):
	try:
		return {
			'kSelectVertsAttached': maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kSelectVertsAttached'  ],
			'kOk': maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kOk'  ],
			'kCancel': maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kCancel'  ],
			'kBulletSoftbodyPerParticleSet': maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kBulletSoftbodyPerParticleSet'  ],
		}[strId]
	except KeyError:
		return " "
...
def setSoftBodyPerParticleMass(newValue=None):
	...
	if (newValue == None): 
		...
		result = maya.cmds.promptDialog(
			...
			message=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kEnterPerParticleMass'  ],
	...
	if newValue < 0.0:
		maya.cmds.select(origSel, r=True)
		maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kCannotHaveANegativeValueForMass'  ])
		return
...
def setSoftBodyPerParticleLinearStiffness(newValue=None):
	...
	if (newValue == None): 
		...
		result = maya.cmds.promptDialog(
			title=_loadUIString('kBulletSoftbodyPerParticleSet'),
			message=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kEnterPerParticleLinearStiffness'  ],
			button=[ okBut,	cancelBut ],
			defaultButton=okBut,
			cancelButton=cancelBut,
			dismissString=cancelBut)
	...
	if newValue < 0.0:
		maya.cmds.select(origSel, r=True)
		maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kCannotHaveANegativeValueForLinearStiffness'  ])
		return
...
def setSoftBodyPerParticleBendResistance(newValue=None):
	...
	if (newValue == None): 
		...
		result = maya.cmds.promptDialog(
			title=_loadUIString('kBulletSoftbodyPerParticleSet'),
			message=maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kEnterPerParticleBendResistance'  ],
			button=[ okBut,	cancelBut ],
			defaultButton=okBut,
			cancelButton=cancelBut,
			dismissString=cancelBut)
	...
	if newValue < 0.0:
		maya.cmds.select(origSel, r=True)
		maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kCannotHaveANegativeValueForBendResistance'  ])
		return
...
def paintSoftBodyVertexProperty(vertexProperty, softbody=None):
	...
	if softbody == None:
		...
		if (softbodies != None) and (len(softbodies) > 0):
			softbody = softbodies[0]
			if len(softbodies) > 1:
				maya.cmds.warning(maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kMultipleBulletUsingFirst'  ])
	...
	if softbody != None:
		...
		maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBody.kNoBulletSoftbodyshape'  ])
