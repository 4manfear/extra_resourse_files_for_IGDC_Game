...
def _longName( obj ):
	...
	if text_type(obj[0]) != text_type('|'):
		...
		if len(objs)>1:
			logger.warn( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kLongNameAmbiguity' ] % obj )
		return objs[0]
	return obj
@Trace()
def _name( obj ):
...
def _createRigidBody( *args, **kw ):
	...
	from sys import version_info as sys_version_info
	text_type = str if sys_version_info[0] >= 3 else unicode
	return [ a if type(a) in [bytes, text_type] else a.longName() for a in rtn]
...
def _getNodesToVisit( rootJointName ):
	...
	if ( rootJointName is None ):
		...
		try:
			...
			logger.error( maya.stringTable['y_maya_app_mayabullet_Ragdoll.kUnableStartJoint'] % rootJointName ) 
			raise
		# end-try
	# end-if	
...
def _createCapsule( currentJoint, childJoint, bAttachToJoint=True,
	...
	logger.info( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kAddingRB'  ] % rbXform )
...
def _addCapsuleWidgets( widgetDict, bIncludeMass=True ):
	...
	if ( bIncludeMass ):
		_addWidget( maya.cmds.floatSliderGrp, 'capsuleMass', widgetDict,
					label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kCapsuleMass'  ],
					minValue=0, maxValue=100, fieldMaxValue=9999999 )
	# end-if
	_addWidget( maya.cmds.floatSliderGrp, 'capsuleBoneRatio', widgetDict,
				label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kCapsuleBoneLengthRatio'  ],
				minValue=0, maxValue=1, fieldMaxValue=9999999 )
	_addWidget( maya.cmds.floatSliderGrp, 'capsuleRadiusRatio', widgetDict,
				label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kCapsuleRadiusLengthRatio'  ],
				minValue=0, maxValue=1, fieldMaxValue=9999999 )
# end
...
def addCapsulesToSkeleton( rootJointName	  = None, 
	...
	if not nodesToVisit or len(nodesToVisit)<1:
		maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kAddCollidersSelectJoint' ])
		return

	while ( len(nodesToVisit) > 0 ):
		currentJoint = nodesToVisit.popleft()
		logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kVisiting'  ] % currentJoint )
		...
		for childJoint in children:
			logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kBoneFromTo'  ] % (currentJoint, childJoint) )
			boneVector = _getTranslation(childJoint)
			...
			# end-for
			if ( bCapsuleExists ): 
				logger.warn( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kSkippingBone'  ] )
				continue
			# end-if
...
class AddColliders( CommandWithOptionVars.CommandWithOptionVars ):
	...
	def __init__( self ):
		...
		self.l10nCommandName   = maya.stringTable['y_maya_app_mayabullet_Ragdoll.kAddColliders' ]
		self.optionVarCategory = 'Bullet.Ragdoll'
...
def createRagdoll( rootJoint=None,
	...
	if not nodesToVisit or len(nodesToVisit)<1:
		maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kCreateRagdollSelectJoint' ])
		return
	...
	currentJoint = None
	while ( len(nodesToVisit) > 0 ):
		logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kStartingIterCurrJoint'  ] \
		...
		logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kVisiting2'  ] % currentJoint )
		...
		for childJoint in childJoints:
			...
			if ( rbAnchorName is not None ):
				logger.info( maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kCreatingConstr'  ] \
								  % (rbAnchorName, rbB) )
...
class CreateRagdoll( CommandWithOptionVars.CommandWithOptionVars ):
	...
	def __init__( self ):
		...
		self.l10nCommandName   = maya.stringTable['y_maya_app_mayabullet_Ragdoll.kCreateRagdoll' ]
		self.optionVarCategory = 'Bullet.Ragdoll'
		self.optionVarPrefix   = 'bullet_Ragdoll_'
		self.optionVarDefaults = {
			'angularDamping'	  : DEFAULT_ANGULAR_DAMPING,
	...
	def addOptionDialogWidgets( self ):
		...
		_addWidget( maya.cmds.floatSliderGrp, 'angularDamping', widgetDict,
					label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kJointAngDampLabel'  ], 
					minValue=0, maxValue=1, fieldMaxValue=9999999 )
		_addWidget( maya.cmds.floatSliderGrp, 'angularSoftness', widgetDict,
					label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kJointAngSoftLabel'  ], 
					minValue=0, maxValue=1, fieldMaxValue=9999999 )
		_addWidget( maya.cmds.floatSliderGrp, 'angularRestitution', widgetDict,
					label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kJointAngRestitutionLabel'  ], 
					minValue=0, maxValue=1, fieldMaxValue=9999999 )

		_addWidget( maya.cmds.textFieldGrp, 'jointNameSeparator', widgetDict,
					label=maya.stringTable[ 'y_maya_app_mayabullet_Ragdoll.kJointNameSepLabel'  ] )
