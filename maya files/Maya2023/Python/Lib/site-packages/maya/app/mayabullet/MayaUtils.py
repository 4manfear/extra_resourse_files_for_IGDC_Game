...
def getScriptDir():
	...
	pluginDir = os_path.dirname( pluginPath ) + "/../scripts"
	pluginDir = os_path.normpath( pluginDir ).replace('\\', '\\\\')
	return pluginDir
# end
...
def _reconnectSelection():
	...
	for bulletShape in selectedBulletShapes:
		if existingSolverShape is None:
			...
			# occurred.
			logger.error( maya.stringTable['y_maya_app_mayabullet_MayaUtils.kMissingSolver' ])
			return
...
def _reconnectBulletShapes():
	...
	for bulletDagPath in bulletDagPaths:
		...
		if existingSolverShape is None:
			...
			# solver, and it's not present, then an error has
			# occurred.
			logger.error( maya.stringTable['y_maya_app_mayabullet_MayaUtils.kMissingSolver2' ])
			return
		mayabullet.BulletUtils.connectToSolver( bulletShape, 
												existingSolverShape )
	bulletDagPaths = []
