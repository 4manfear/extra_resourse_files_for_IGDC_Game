...
def createSoftBodyAnchorConstraint(selectedVerts=None):
    ...
    if len(selectedVerts) == 0:
        maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBodyConstraint.kPleaseSelectASetOfMeshVertsForTheAnchorConstraint'  ]);
        return
    ...
    if objs == None or len(objs) == 0:
        maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBodyConstraint.kPleaseSelectMeshVertsForTheAnchorConstraint'  ])
        return
    ...
    if cons != None and len(cons) == 1:
        ...
        if cons != None and len(cons) == 1:
            softbody = cons[0]
    if softbody == "":
        maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBodyConstraint.kCouldNotDetermineSBShape'  ])
        return
    ...
    if numCandidates==0:
        maya.OpenMaya.MGlobal.displayError(maya.stringTable[ 'y_maya_app_mayabullet_SoftBodyConstraint.kPleaseSelectOneBulletRigidBodyForAnchorConstraint'  ])
        return

    # use first object found or first rigid body found
    anchorConstrError1 = maya.stringTable[ 'y_maya_app_mayabullet_SoftBodyConstraint.kPleaseSelectOnlyOneObjectForAnchorConstraint1'  ]
    anchorConstrError2 = maya.stringTable[ 'y_maya_app_mayabullet_SoftBodyConstraint.kPleaseSelectOnlyOneObjectForAnchorConstraint2'  ]
    anchor = None
    shape = None
    rigidbody = None
