...
def doExportArgList(*args, **kw):
	...
	filter = maya.stringTable['y_maya_app_mayabullet_AlembicExport.kAlembic'] + " (*.abc);;" + maya.stringTable['y_maya_app_mayabullet_AlembicExport.kAllFiles'] + " (*.*)"
	...
	if (exportAll) :
		result = maya.cmds.fileDialog2(\
					returnFilter=1,
					fileFilter=filter,
					caption=maya.stringTable['y_maya_app_mayabullet_AlembicExport.kExportAll2'],
					startingDirectory=startingDir,
					fileMode=0,
					okCaption=maya.stringTable['y_maya_app_mayabullet_AlembicExport.kExportAll3'],
		...
		result = maya.cmds.fileDialog2(
					returnFilter=1,
					fileFilter=filter,
					caption=maya.stringTable['y_maya_app_mayabullet_AlembicExport.kExportSelection2'],
					startingDirectory=startingDir,
					fileMode=0,
					okCaption=maya.stringTable['y_maya_app_mayabullet_AlembicExport.kExportSelection3'],
					optionsUICreate="Alembic_exportFileOptionsUICreate",
					optionsUIInit="Alembic_exportFileOptionsUIInit",
	...
	gAbcBulletExportLastDirectory = os_path.dirname(result[0])
	...
	for cacheTimeRange, startFrame, endFrame, evaluateEvery,enableSample, lowSample, highSample \
		...
		if (evaluateEvery != 1) :
			if (evaluateEvery <= 0) :
				OpenMaya.MGlobal.displayError( maya.stringTable['y_maya_app_mayabullet_AlembicExport.kInvalidEvaluateEvery'])
				return
		...
		if (enableSample) :
			if (lowSample > 0 or lowSample < -evaluateEvery) :
				OpenMaya.MGlobal.displayError( maya.stringTable['y_maya_app_mayabullet_AlembicExport.kInvalidLowFrameRelativeSample'])
				return
			
			if (highSample < 0 or highSample > evaluateEvery) :
				OpenMaya.MGlobal.displayError( maya.stringTable['y_maya_app_mayabullet_AlembicExport.kInvalidHighFrameRelativeSample'])
				return
	...
	if (enablePreRoll) :
		# Check arguments.
		if (preRollStep <= 0) :
			OpenMaya.MGlobal.displayError( maya.stringTable['y_maya_app_mayabullet_AlembicExport.kInvalidPreRollStep'])
			return


		if (preRollStartFrame >= firstCacheFrame) :
			OpenMaya.MGlobal.displayError( maya.stringTable['y_maya_app_mayabullet_AlembicExport.kInvalidPreRollStartFrame'])
			return
	...
	if len(roots):
		...
		if not maya.cmds.pluginInfo( 'AbcBullet', q=True, loaded=True):
			OpenMaya.MGlobal.displayError(maya.stringTable['y_maya_app_mayabullet_AlembicExport.kAbcExportNotLoaded'])
			return
		...
		OpenMaya.MGlobal.displayError( maya.stringTable['y_maya_app_mayabullet_AlembicExport.kInvalidRoots'])
		return
