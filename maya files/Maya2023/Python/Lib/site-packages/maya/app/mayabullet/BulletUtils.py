...
def _longName( obj ):
	...
	if text_type(obj[0]) != text_type('|'):
		...
		if len(objs)>1:
			logger.warn( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kLongNameAmbiguity' ] % obj )
		return objs[0]
	return obj
@Trace()
def _name( obj ):
...
def getSolver( bCreateIfMissing=True, bCheckForDuplicates=True ):
	...
	if len(solvers) > 0:
		...
		if bCheckForDuplicates and len( solvers ) > 1:
			warnMsg = maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kMultipleSolvers'  ] % solver
		...
		if solver:
			...
			errorMsg = maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kErrSolverNotCreated' ]
...
def disconnectFromSolver( bulletShape ):
	...
	@Trace()
	def _inputs(obj):
		return list(zip(	maya.cmds.listConnections( bulletShape, plugs=True, \
	...
	logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kDisconnectShape' ] % bulletShape )
	# into the shape from other nodes

	for outPlug, outSrc in _inputs(bulletShape):
		if not _isType( outSrc, 'bulletSolverShape' ):
			logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kSkipNonSolverConn' ] % outPlug )
			continue
		...
		for inPlug in maya.cmds.connectionInfo(outPlug, \
			...
			if ( _nodeFromPlug(inPlug) != bulletShape ):
				logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kSkipUnrelatedConn' ] % inPlug )
				continue

			logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kDisconnectConn' ] % (outPlug, inPlug) )
	...
	for inPlug in destinations:
		if ( not _isType(inPlug, 'bulletSolverShape') ):
			logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kSkipNonSolverConn2' ] % inPlug )
			continue
		...
		if _nodeFromPlug(outPlug) != bulletShape:
			logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kSkipUnrelatedConn2' ] % outPlug )
			continue

		logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kDisconnectConn2' ] % (outPlug, inPlug) )
		_disconnect( outPlug, inPlug )
	# end-for
...
def connectToSolver( bulletShape, solver ):
	...
	from sys import version_info as sys_version_info
	text_type = str if sys_version_info[0] >= 3 else unicode
	if type(bulletShape) not in [bytes, text_type]:
		bulletShape = _longName(bulletShape)
	if type(solver) in [bytes, text_type]:
	...
	if _isType( bulletShape, 'bulletRigidBodyShape' ):
		...
		warnMsg = maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kUnexpectedShapeInputToSolver' ] \
							 % bulletShape
		maya.cmds.warning( warnMsg )
	# end-if
...
def getRigidBodyFromTransform( transform ):
	...
	init_xform
		rb_xform
			rb_shape
	"""
	logger.debug( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kGettingRB'  ] % transform )
	if transform:
	...
	shapeNodes = _getChildren(xformNode, type=("bulletRigidBodyShape",
											  ...
											  "bulletSoftConstraintShape") )
	if ( len(shapeNodes) > 1 ):
		logger.warn( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kExceptedShapeUnder'  ] % xformNode )
	if ( len(shapeNodes) > 0 ):
		return shapeNodes[0]
	...
	for childXformNode in _getChildren(xformNode, type="transform" ):
		shapeNodes = _getChildren(childXformNode, type=("bulletRigidBodyShape",
											  ...
											  "bulletSoftConstraintShape") )
		if ( len(shapeNodes) > 1 ):
			logger.warn( maya.stringTable[ 'y_maya_app_mayabullet_BulletUtils.kExceptedShapeUnder2'  ] % childXformNode )
		if ( len(shapeNodes) > 0 ):
			return shapeNodes[0]
	# end-for
...
def verifySelectionNotBullet(shapes = None, treatAsError=True):
	...
	if softbodies != None:
		maya.OpenMaya.MGlobal.displayError(maya.stringTable['y_maya_app_mayabullet_BulletUtils.kAlreadyBulletObject1' ])
		return result
	...
	for shape in shapes:
		...
		if rb != None:
			strMsg = maya.stringTable['y_maya_app_mayabullet_BulletUtils.kAlreadyBulletObject2' ].format(shape)
			if treatAsError:
...
def removeBulletObjectsFromList(objects = None):
	...
	if len(rbObjects):
		...
		for rbSet, rbSetMembers in list(toRemoveFromSet.items()):
