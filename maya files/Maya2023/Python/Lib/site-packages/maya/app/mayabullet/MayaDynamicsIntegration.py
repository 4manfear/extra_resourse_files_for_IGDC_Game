...
def connectFieldToShape(field, shape):
    ...
    try:
        ...
        raise RuntimeError(maya.stringTable['y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletUnableToConnectField'] % (shape, field))
        ...
        if cmds.nodeType(shape) == kBulletSolverShapeType:
            for field in fields:
                ...
                for connected in connected_shapes:
                    try:
                        cmds.warning(
                            maya.stringTable['y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletDisconnectWarn' ] 
                            % (connected, field))
                        disconnectFieldFromShape(field, connected)
                    except:
                        cmds.warning(
                            maya.stringTable['y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletCouldNotDisconnect' ] 
                            % (connected, field))
            ...
            for field in fields:
                if field in solver_connections[solver]:
                    ...
                    # this connection
                    cmds.warning(
                        maya.stringTable['y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletSkipField' ] 
                         % field)
                    continue
...
def connectDynamicCB(fields, emitters, collisionObjects, objects, delete):
    ...
    try:
        ...
        if emitters and len(emitters) > 0:
            ...
            cmds.warning( maya.stringTable[ 'y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletConnectEmitter' ])
        
        if collisionObjects and len(collisionObjects) > 0:
            # We do not handle collision objects. Warn the user and continue
            cmds.warning(maya.stringTable[ 'y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletConnectCollision' ])

        if not fields or len(fields) == 0:
            raise RuntimeError(maya.stringTable[ 'y_maya_app_mayabullet_MayaDynamicsIntegration.kBulletConnectNoField' ])
