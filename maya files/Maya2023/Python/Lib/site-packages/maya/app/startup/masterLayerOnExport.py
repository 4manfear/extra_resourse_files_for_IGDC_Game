...
def initialize():
    ...
    for type, callback in list(exportCallbacks.items()):
...
def _noSwitchToMaster():
    ...
    return os_getenv("MAYA_BATCH_RENDER_EXPORT") or os_getenv("MAYA_CUT_COPY_EXPORT") == "" or _exported_file_extension in _supported_renderer_extensions
...
def _beforeFileExport(_):
    ...
    if renderSetup.hasInstance():
        ...
        if _prevVisibleRenderLayer != defaultRenderLayer:
            cmds.warning(maya.stringTable['y_maya_app_startup_masterLayerOnExport.kExportingMasterLayerContent' ])
