...
LAYER_TEXT                    = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kLayerText'                  ]
DEFAULT_LAYER_TEXT            = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kDefaultLayerText'           ]
LIGHT_EDITOR_TEXT_GLOBAL_MODE = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kLightEditorGlobalModelText' ]
LIGHT_EDITOR_TEXT_LAYER_MODE  = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kLightEditorLayerModelText'  ]
...
class EditorTreeView(QTreeView):
	...
	def __init__(self, parent):
		...
		action = QAction(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kNewGroup'], self.contextMenu)
		...
		for lightType in allLightTypes:
			if typeMgr.excludeFromUI(lightType):
				continue
			action = QAction(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kNewLightInGroup'] + lightType, self.contextMenu)
			action.triggered.connect( parent.getLightCreator(lightType) )
			self.createActions.append(action)

		self.renameAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kRename'], self.contextMenu)
		self.renameAction.triggered.connect(self._renameItem)

		self.deleteAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kDelete'], self.contextMenu)
	...
	def setExpanded(self, index, state):
		...
		utils.setExpandedState(self.model.nodeFromIndex(index).mayaHandle.object(), state)
...
class LookThroughWindow(MayaQWidgetDockableMixin, QWidget):
	...
	def lookThroughLight(self, light):
		...
		title = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kLookingThrough'] + light
...
class EditorCentralWidget(QWidget):
	...
	def __init__(self, parent=None):
		...
		for lightType in mayaLights:
			...
			toolTip = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kCreateaNew1' ] + lightType
		...
		if len(pluginLights) > 0:
			...
			for lightType in pluginLights:
				...
				button = RenderSetupButton(self, icon, self.BUTTON_SIZE)
				button.clicked.connect( self.getLightCreator(lightType) )
				toolTip = maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kCreateaNew2' ] + lightType
				button.setToolTip(toolTip)
				self.tabButtons.append(button)
		...
		button = QPushButton(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kNewGroup2'])
		button.clicked.connect(self.newGroup)
		button.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kCreateaNewGroup' ])
		self.tabButtons.append(button)
	...
	def _restoreExpandedState(self, index):
		...
		for i in range(node.childCount()):
			...
			# TODO: implement expanded state appropriately by using code in renderSetup/model/expandedState.py
			self.treeView.setExpanded(childIndex, utils.getExpandedState(node.child(i).mayaHandle.object()))
	...
	def _snapToSelected(self):
		...
		if len(names) == 0:
			print(maya.stringTable['y_maya_app_renderSetup_views_lightEditor_editor.kSnapToObjectNothingSelected'])
			return
