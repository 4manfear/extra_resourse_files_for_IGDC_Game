...
GROUP_TYPE                            = QStandardItem.UserType + GROUP_TYPE_IDX
...
DEFAULT_GROUP_COLOR         = QColor(82, 82, 82)
GROUP_NODE_BAR_COLOR        = QColor(144, 196, 222)
...
class Template(object):
    ...
    def findAllTemplateFiles(self, templateDirectory):
        ...
        for root, _, files in os_walk(templateDirectory):
            for file in files:
            ...
            with open(filepath, "r") as file:
                ...
                try:
                    ...
                    if self.isAcceptableTemplate(objList):
                        ...
                        partialFilepath = os_path.relpath(filepath, templateDirectory)
    ...
    def templateActions(self, templateDirectory):
        ...
        for (objList, filepath) in self.acceptableDictionaries(templateDirectory):
            ...
            # Note: Some file names and some file paths could
...
class ModelProxyItem(Template, standardItem.StandardItem):
    ...
    def getWarning(self):
        for i in range(0, self.rowCount()):
            ...
            if warning:
                return maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kChildHasWarning' ] % child.data(Qt.EditRole)
        return None
    ...
    def headingWidth(self, heading):
        ...
        return fm.horizontalAdvance(heading)
...
class ShaderOverrideProxy(ConnectionOverrideProxy):
    ...
    def createAttributeUI(self, attribute):
        ...
        overrideCtrl = cmds.attrNavigationControlGrp(label=maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kOverrideShader' ], attribute=overrideAttr,
...
class MaterialOverrideProxy(ConnectionOverrideProxy):
    ...
    def createAttributeUI(self, attribute):
        overrideAttr = self._model.attrValuePlugName()
        overrideCtrl = cmds.attrNavigationControlGrp(label=maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kOverrideMaterial' ], attribute=overrideAttr,
...
class MaterialTemplateOverrideProxy(ConnectionOverrideProxy):
    ...
    def __init__(self, model):
        super(MaterialTemplateOverrideProxy, self).__init__(model)

    def _typeName(self):
        return kMaterialTemplateType
    ...
    def createAttributeUI(self, attribute):
        overrideAttr = self._model.attrValuePlugName()
        overrideCtrl = cmds.attrNavigationControlGrp(label=maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kOverrideMaterialTemplate' ], attribute=overrideAttr,
            createNew="materialTemplateOverrideNewNode " + overrideAttr, connectToExisting="materialTemplateOverrideReplaceNode " + overrideAttr)
        return overrideCtrl
...
class GroupProxy(DataModelListObserver, LabelColor, ModelProxyItem):
    ...
    def __init__(self, model):
        super(GroupProxy, self).__init__(model)
    ...
    def aboutToDelete(self):
        """Cleanup method to be called immediately before the object is deleted."""
        super(GroupProxy, self).aboutToDelete()
    ...
    def genericTypeIdx(self):
        # Constant for all derived classes
        return GROUP_TYPE_IDX

    def typeIdx(self):
        return GROUP_TYPE_IDX

    def getWarning(self):
        if self.model.getSelector():
            warning = self.model.getSelector().status()
            if warning:
                return warning
        return super(GroupProxy, self).getWarning()
    ...
    def data(self, role):
        ...
        if role == renderSetupRoles.NODE_TYPE_STR:
            ...
            return super(GroupProxy, self).data(role)
    ...
    def setData(self, value, role):
        if role == renderSetupRoles.NODE_SELF_ENABLED:
            ...
            super(GroupProxy, self).setData(value, role)

    def listItemAdded(self, listItem):
        super(GroupProxy, self).listItemAdded(listItem)

    def listItemRemoved(self, listItem):
        super(GroupProxy, self).listItemRemoved(listItem)

    def delete(self):
        groupModel.delete(self._model)

    def attachChild(self, child, pos):
        self.model.attachChild(pos, child)

    def createListItemProxy(self, child):
        return proxyFactory.create(child)
    ...
    def createGroup(self, groupName, nodeType):
        group = self._model.createGroup(groupName)
        proxy = getProxy(group)
        return proxy.index()
    ...
    def createCollection(self, collectionName, nodeType):
        ...
        # that of the parent.
        if self._model.getSelector():
            collection.setSelectorType(self._model.getSelector().kTypeName)
        proxy = getProxy(collection)
        return proxy.index()

    def isAcceptableTemplate(self, objList):
        """ Only groups and collections can be imported in a group """
        typename = jsonTranslatorUtils.getTypeNameFromDictionary(objList[0])
        containerTypenames = { collectionModel.Collection.kTypeName } | { groupModel.Group.kTypeName }
        return typename in containerTypenames
...
class BaseCollectionProxy(DataModelListObserver, LabelColor, ModelProxyItem):
    ...
    def __init__(self, model):
        ...
        self.addListObserver(self._model)
        self._sortingEnabled = False
        self._kCollection = maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kCollection' ]
    ...
    def getWarning(self):
        selector = self.model.getSelector()
        if selector:
            warning = selector.status()
            if warning:
                return warning
    ...
    def data(self, role):
        if role == Qt.TextAlignmentRole:
            ...
            return self.headingWidth(u"{}:   ".format(self._kCollection))
        else:
...
class CollectionProxy(BaseCollectionProxy):
    ...
    def data(self, role):
        if role == Qt.DisplayRole:
            return u"{}:   {}".format(self._kCollection, self._model.name())
        if role == renderSetupRoles.NODE_TYPE_STR:
            return u"{}:".format(self._kCollection)
        elif role == Qt.EditRole:
            return self._model.name()
        elif role == renderSetupRoles.NODE_COLOR_BAR:
        ...
        if numIndexes == 1:
            actions |= set(
                [CREATE_COLLECTION_ACTION,
                 ...
                 CREATE_MATERIAL_TEMPLATE_OVERRIDE_ACTION,
                 RENAME_ACTION])
    ...
    def isAcceptableTemplate(self, objList):
        """ Only collections and overrides could be imported in a collection """
        typename = jsonTranslatorUtils.getTypeNameFromDictionary(objList[0])
        overrideTypenames = { o.kTypeName for o in overrideUtils.getAllOverrideClasses() } | { collectionModel.Collection.kTypeName } | { groupModel.Group.kTypeName }
        return typename in overrideTypenames
...
class LightsProxy(BaseCollectionProxy):
    ...
    def data(self, role):
        if role == Qt.DisplayRole:
            ...
            return True
        # The following two roles are irrelevant for Lights Collections, but needed
    ...
    def supportsAction(self, action, numIndexes):
        return action in DELETE_ACTION
...
class LightsChildCollectionProxy(BaseCollectionProxy):
    ...
    def data(self, role):
        if role == Qt.DisplayRole or role == Qt.EditRole:
            ...
            return False
        # The following two roles are irrelevant for LightsChildCollection, but needed
        # by the draw code, which is common to all render setup node types.
        elif role == renderSetupRoles.NODE_SELF_ENABLED:
            return True
        elif role == renderSetupRoles.NODE_ENABLED:
...
class RenderLayerProxy(DataModelListObserver, LabelColor, ModelProxyItem):
    ...
    def _build(self):
        ...
        groups = self._model.getGroups()
        for group in groups:
            self.appendRow(proxyFactory.create(group))
    ...
    def data(self, role):
        kLayer = maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kLayer' ]
        if role == Qt.DisplayRole:
            return u"{}:   {}".format(kLayer, self._model.name())
        elif role == renderSetupRoles.NODE_TYPE_STR:
            return  u"{}:".format(kLayer)
        elif role == Qt.EditRole:
            return self._model.name()
        elif role == Qt.BackgroundRole:
            return self.getLabelColor()
            ...
            return self.headingWidth(u"{}:   ".format(kLayer))
    ...
    def createGroup(self, groupName, nodeType):
        group = None
        if nodeType == groupModel.Group.kTypeId:
            group = self._model.createGroup(groupName)
        proxy = getProxy(group)
        return proxy.index()
    ...
    def isAcceptableTemplate(self, objList):
        ...
        return (typename in [o.kTypeName for o in collectionModel.getAllCollectionClasses()] \
            or  typename == groupModel.Group.kTypeName) \
...
class RenderSetupProxy(DataModelListObserver, Template, QStandardItemModel):
    ...
    def __init__(self, parent=None):
        ...
        if self.model:
            self.model.addOpaqueData(PROXY_OPAQUE_DATA, weakref.ref(self))
    ...
    def aboutToDelete(self):
        ...
        if self.model:
            self.model.removeOpaqueData(PROXY_OPAQUE_DATA)
    ...
    def _buildHeader(self):
        ...
        kRenderSetup = maya.stringTable['y_maya_app_renderSetup_views_proxy_renderSetup.kRenderSetup' ]
        header.setText(kRenderSetup)
    ...
    def mimeData(self, indices):
        ...
        if not self.dropMimeDataFailure and (genericModelTypeIdx==COLLECTION_TYPE_IDX or genericModelTypeIdx==GROUP_TYPE_IDX):
            modelTypeIdx = None
            for index in indices:
                item = self.itemFromIndex(index)
    ...
    def exportSelectedToFile(self, filePath, notes, proxies):
        ...
        with open(filePath, "w+") as file:
            if proxies is None:
                json.dump(rs.encode(notes, userPrefs.ExportRenderSettingsAOVs.isEnabled()), fp=file, indent=2, sort_keys=True, separators=(',', ': '))
            else:
                data = []
                for proxy in proxies:
                    data.append(proxy._model.encode(notes))
                json.dump(data, fp=file, indent=2, sort_keys=True, separators=(',', ': '))
