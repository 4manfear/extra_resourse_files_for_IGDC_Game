...
def createProgram(e):
    ...
    if negate:
        e = e[1:]
    if e == '*':
        return (negate, re.compile('.*'))
    e = re.escape(e)
    # Python3 does not escape ':'
    if r'\:' not in e:
        e = e.replace(':', r'\:')
    e = e.replace('\\*', '[a-zA-Z0-9_]*')
    # recursive namespace syntax ::
    e = e.replace(r'\:\:', r'\:([a-zA-Z0-9_]+\:)*')
    if e.startswith(r'\:'):
        e = e[2:]
    return (negate, re.compile(r'^((.*\|)|)' + e + r'$'))
...
class Selection(object):
    ...
    def sets(self):
        return [node for node in self.nonDagNodes() if node.hasFn(OpenMaya.MFn.kSet)]

    def dagNodes(self):
        return (node.object() for node in list(self._paths.keys()))
    ...
    def paths(self):
        lists = (l for l in list(self._paths.values()) if l)
        for l in lists:
    ...
    def names(self, allPaths=True):
        ...
        paths = self.paths() if allPaths else \
            (l.getDagPath(0) for l in list(self._paths.values()) if l)
    ...
    def ls(self, patterns):
        ...
        for pattern in patterns:
            ...
            if '|' in pattern:
                update(names, [n for n in (p.fullPathName() for p in self.paths()) if prog.match(n)])
            else:
                dags = (key for key in list(self._paths.keys()) if prog.match(OpenMaya.MFnDependencyNode(key.object()).name()))
                update(names, (self._paths[key].getDagPath(0).fullPathName() for key in dags))
                update(names, [n for n in (OpenMaya.MFnDependencyNode(o.object()).name() for o in self._nondags) if prog.match(n)])
        return names
    
    def _addMSelection(self, selection):
        for i in range(0,selection.length()):
            try:
                self._addPath(selection.getDagPath(i))
            except TypeError:
                self._addNode(selection.getDependNode(i))
