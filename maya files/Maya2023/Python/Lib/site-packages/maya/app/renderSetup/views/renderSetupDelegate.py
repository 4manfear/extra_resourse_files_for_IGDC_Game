...
kRenderableToolTip = maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kRenderableToolTip' ]
kNonRenderableToolTip = maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kNonRenderableToolTip' ]
kRenderableSelectionToolTip = maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kRenderableSelectionToolTip' ]
kNonRenderableSelectionToolTip = maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kNonRenderableSelectionToolTip' ]
...
class RenderSetupDelegate(baseDelegate.BaseDelegate):
    ...
    LEFT_NON_TEXT_OFFSET = utils.dpiScale(19)
    ...
    kTooltips = {renderSetup.SET_VISIBILITY_ACTION : maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kVisibilityToolTip' ],
                 renderSetup.SET_RENDERABLE_ACTION : kRenderableToolTip,
                 renderSetup.SET_ENABLED_ACTION : maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kEnabledToolTip' ],
                 renderSetup.SET_ISOLATE_SELECTED_ACTION : maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kIsolateToolTip' ],
                 renderSetup.FILTER_MENU : maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kFiltersToolTip' ]}
    ...
    def _addActionIcons(self, painter, rect, item, highlightedColor):
        ...
        if self.lastHitAction:
            ...
            if self.lastHitAction == renderSetup.SET_VISIBILITY_ACTION and item.data(renderSetupRoles.NODE_NEEDS_UPDATE):
                item.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_renderSetupDelegate.kUpdateVisibilityToolTip' ])
    ...
    def getTextRect(self, rect, item):
        ...
        elif item.type() == renderSetup.GROUP_TYPE:
            newRect.setRight(rect.right() - self.GROUP_TEXT_RIGHT_OFFSET)
    ...
    def updateEditorGeometry(self, editor, option, index):
        ...
        if item.type() == renderSetup.LIGHTS_CHILD_COLLECTION_TYPE:
            ...
            #Set left if and only if item.type() == renderSetup.LIGHTS_CHILD_COLLECTION_TYPE
            rect.setLeft(leftOffset)
        elif item.type() == renderSetup.RENDER_LAYER_TYPE or item.type() == renderSetup.COLLECTION_TYPE or item.type() == renderSetup.GROUP_TYPE:
            leftOffset = indent + item.data(renderSetupRoles.NODE_HEADING_WIDTH) + self.LEFT_NON_TEXT_OFFSET
            #Set left if and only if item.type() == renderSetup.RENDER_LAYER_TYPE or item.type() == renderSetup.COLLECTION_TYPE
            rect.setLeft(leftOffset)
        editor.setGeometry(rect)

    def _drawWarning(self, painter, rect, item):
        warning = item.data(renderSetupRoles.NODE_WARNING)
        if warning and len(warning) > 0 and userPrefs.ShowWarningIcons.isEnabled():
