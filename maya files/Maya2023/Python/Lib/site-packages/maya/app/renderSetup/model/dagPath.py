...
def _findShadingEnginesInMaterialTemplate(nodeObject):
    """ Return a generator of all shadingEngine nodes connected to a materialTemplate node"""
    node = OpenMaya.MFnDependencyNode(nodeObject)
    assignPlug = node.findPlug("assign", False)
    shadingEnginePlug = node.findPlug("shadingEngine", False)

    # plug looks like: node.assign[i].shadingEngine
    for i in range(assignPlug.evaluateNumElements()):
        shadingEnginePlug.selectAncestorLogicalIndex(i, assignPlug.attribute())
        srcPlug = shadingEnginePlug.source()
        if srcPlug.node().hasFn(OpenMaya.MFn.kShadingEngine):
            yield srcPlug.node()
...
class DagPath(object):
    ...
    def _findInstObjGroupsPlugsImpl(self, plugName):
        ...
        return itertools.chain([instObjGroup],  # whole instance
            (objGroups.elementByPhysicalIndex(i) \
                for i in range(objGroups.evaluateNumElements()))) # per-face
    ...
    def findMaterialPlugs(self):
        ...
        wholeInstance = next(instPlugs) # the first plug is the whole instance
        hasConnection = False
    ...
    def getInstMaterialAssignPlug(self):
        if not self.node().hasFn(OpenMaya.MFn.kGeometric):
            return None

        node = OpenMaya.MFnDependencyNode(self.node())
        try:
            instMtlAssignMulti = node.findPlug("instMaterialAssign", False)
        except RuntimeError:
            raise _PlugNotFound()

        instanceNr = self.dagPath.instanceNumber()
        return instMtlAssignMulti.elementByLogicalIndex(instanceNr)

    def findShadingEnginesInMaterialTemplate(self):
        """ Find and yield shadingEngines connected through a materialTemplate."""
        shapePath = self._getShapePath()
        if not shapePath:
            return ()

        ima = DagPath(shapePath).getInstMaterialAssignPlug()
        if ima is None:
            return ()
        ...
        if srcPlug.node().hasFn(OpenMaya.MFn.kShadingEngine):
            yield srcPlug.node()
        elif srcPlug.node().hasFn(OpenMaya.MFn.kMaterialTemplate):
            for se in _findShadingEnginesInMaterialTemplate(srcPlug.node()):
                yield se
    ...
    def findShadingEngines(self):
        ...
        for se in self.findShadingEnginesInMaterialTemplate():
            yield se

        for (_,dstPlug) in self.findShadingEngineConnections():
            yield dstPlug.node()

    def findSurfaceShaders(self):
        '''Generator over the surface shaders assigned to this DAG path.
        There can be more than one if shape has per-face shading.'''
        return [o for o in (utils.findSurfaceShader(se, True) for se in self.findShadingEngines()) if o] 
        
    def findDisplacementShaders(self):
        '''Generator over the displacement shaders assigned to this DAG path.
        There can be more than one if shape has per-face shading.'''
        return [o for o in (utils.findDisplacementShader(se, False) for se in self.findShadingEngines()) if o] 
        
    def findVolumeShaders(self):
        '''Generator over the volume shaders assigned to this DAG path.
        There can be more than one if shape has per-face shading.'''
        return [o for o in (utils.findVolumeShader(se, False) for se in self.findShadingEngines()) if o] 
    ...
    def findGeometryGenerator(self):
        ...
        for ktype, attr in list(typeToAttr.items()):
            if obj.hasFn(ktype):
