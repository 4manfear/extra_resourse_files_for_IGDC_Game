...
kCouldNotSelectMissingObject = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kCouldNotSelectMissingObject' ]

kDisabledTextColor = QColor(68, 68, 68)
kHiddenObjectTextColor = QColor(85, 85, 85)
...
class HTMLDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        # disable the text color if the include all lights setting is enabled
        if rsPrefs.IncludeAllLightsSetting.isEnabled() and self._selector.kTypeId == typeIDs.lightsCollectionSelector:
            ctx.palette.setColor(QPalette.Text, kDisabledTextColor)

        #set text color to grey for selected item
        if missing or filteredOut:
            ctx.palette.setColor(QPalette.Text, kHiddenObjectTextColor)
        ctx.clip = clip
...
class CollectionStaticSelectionWidget(QListWidget):
    ...
    PLACEHOLDER_TEXT_PEN_DISABLED = QPen(kDisabledTextColor)
    ...
    def __init__(self, selector, parent=None):
        ...
        self.selectAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kSelect' ], self.contextMenu)
        self.selectAction.triggered.connect(self.selectEntry)
        self.removeAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kRemove' ], self.contextMenu)
        self.removeAction.triggered.connect(self.removeEntry)
        self.removeMissingAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kRemoveMissing' ], self.contextMenu)
        self.removeMissingAction.triggered.connect(self.removeMissingObjects)
        self.selectMissingAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kSelectMissing' ], self.contextMenu)
        self.selectMissingAction.triggered.connect(self.selectMissingObjects)
        self.removeFilteredAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kRemoveFiltered' ], self.contextMenu)
        self.removeFilteredAction.triggered.connect(self.removeFilteredObjects)
        self.selectFilteredAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kSelectFiltered' ], self.contextMenu)
        self.selectFilteredAction.triggered.connect(self.selectFilteredObjects)
        self.sortAlphabeticallyAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kSortAlphabetically' ], self.contextMenu)
        self.sortAlphabeticallyAction.triggered.connect(partial(self.setSortingEnabled, True))
        self.sortChronologicallyAction = QAction(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kSortChronologically' ], self.contextMenu)
    ...
    def populate(self):
        ...
        # If we are populating lights collection then non light objects must be filtered out of selection
    ...
    def addEntry(self):
        ...
        if self._selector.kTypeId == typeIDs.lightsCollectionSelector:
            self._selector.staticSelection.addOnlyLightsToSelection(OpenMaya.MGlobal.getActiveSelectionList())
        else:
    ...
    def dropEvent(self, event):
        ...
        if self._selector.kTypeId == typeIDs.lightsCollectionSelector:
            self._selector.staticSelection.addOnlyLightsToSelection(event.mimeData().text().rstrip().split())
        else:
    ...
    def paintEvent(self, e):
        ...
        if self.count() == 0:
            ...
            oldPen = painter.pen()
            painter.setPen(self.PLACEHOLDER_TEXT_PEN_DISABLED if rsPrefs.IncludeAllLightsSetting.isEnabled() and self._selector.kTypeId == typeIDs.lightsCollectionSelector else self.PLACEHOLDER_TEXT_PEN_ENABLED)
            painter.drawText(self.contentsRect(), Qt.AlignCenter | Qt.TextWordWrap, maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_collectionStaticSelectionWidget.kDragObjectsUsingOutlinerOrSelectFromViewport' ])
