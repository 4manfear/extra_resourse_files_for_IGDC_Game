...
import maya.app.renderSetup.model.group as group
...
import maya.app.renderSetup.lightEditor.model.typeManager as lightTypeManager
...
kInvalidCollectionName = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kInvalidCollectionName' ]
kUnknownCollection     = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kUnknownCollection'     ]
kUnknownContainer  = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kUnknownContainer'     ]
kCollectionUnicity     = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCollectionUnicity'     ]
kAOVCollectionNodeCreationFailed = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kAOVCollectionNodeCreationFailed' ]

# List of undo messages
kAttachCollection              = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kAttachCollection'               ]
kCreateLightsCollection        = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCreateLightsCollection'         ]
kCreateLightsChildCollection   = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCreateLightsChildCollection'    ]
kCreateRenderSettingsCollection= maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCreateRenderSettingsCollection' ]
kCreateAOVCollection           = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCreateAOVCollection'            ]
kCreateAOVChildCollection      = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCreateAOVChildCollection'       ]
kSetRenderability              = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kSetRenderability'               ]
kCollectionDetached            = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCollectionDetached'             ]
kCollectionAttached            = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kCollectionAttached'             ]

kAttachGroup                   = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kAttachGroup'               ]
kGroupDetached            = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kGroupDetached'             ]
kGroupAttached            = maya.stringTable['y_maya_app_renderSetup_model_renderLayer.kGroupAttached'             ]
...
def memberTraversal(node):
    ...
    # If the node is not a collection or group, return its children.
    if not isinstance(node, container.Container):
        return nodeList.forwardListGenerator(node)

    # The node is a collection or group.  If it's disabled, return an empty list, else
    # return its children.
    if isinstance(node, container.Container):
        return node.getChildren() if node.isEnabled() else []
...
class RenderLayer(RenderLayerBase, nodeList.ListBase, childNode.ChildNode):
    ...
    A render layer has an ordered list of collections and groups.  It can
    ...
    #
    # Connections to lowest-priority and highest-priority collections/groups
    # on collection/group linked list.  The lowest-priority collection
    # is considered to be the front of the list, and the highest-priority
    # collection the back of the list.
    containerLowest  = OpenMaya.MObject()
    containerHighest = OpenMaya.MObject()

    containers = OpenMaya.MObject()
    ...
    def initializer():
        ...
        RenderLayer.containers = RenderLayer.initListItems()

        RenderLayer.containerLowest = utils.createDstMsgAttr(
            'containerLowest', 'cl')
        RenderLayer.addAttribute(RenderLayer.containerLowest)

        RenderLayer.containerHighest = utils.createDstMsgAttr(
            'containerHighest', 'ch')
        RenderLayer.addAttribute(RenderLayer.containerHighest)
    ...
    def descendantAdded(self, child):
        '''If the layer is currently visible and a descendant is added (collection, group or override), apply it right away.'''
    ...
    def checkGroupHasDagNodes(self, group):
        ...
        for child in group.getChildren():
            if child.isCollection():
                if child.getSelector().hasDagNodes():
                    any_changed = True
            else:
                any_changed = any_changed or self.checkGroupHasDagNodes(child)
        return any_changed

    @_visible
    def _containerChanged(self, container):
        if container.isGroup():
            changed = self.checkGroupHasDagNodes(container)
        else:
            changed = container.getSelector() and container.getSelector().hasDagNodes()
        self.needsMembershipUpdate = self.needsMembershipUpdate or changed
        if self.needsMembershipUpdate:
            self._startMembershipUpdate()

    def itemChangedRecursive(self, refreshSelf = False):
        for col in nodeList.forwardListGenerator(self):
            col.itemChangedRecursive()
        if refreshSelf:
            self.itemChanged()

    def _selectedNodesChanged(self, container):
        ...
        # - if the container has any override in itself or in its child
        #   collections and the selection changed, then there might be objects
        #   in it that are not overridden by the overrides in that
        #   container/subcollections, or objects that are still overridden
        #   but should no longer be.
        # - if the container is traversing connections to populate itself,
        ...
        # However, we do not need to apply an update if the container or any
        # of its parents is empty.
        self.needsApplyUpdate = self.needsApplyUpdate or \
            (not container.isEmptyRecursive() and
             (container.isTraversingConnections() or container.hasOverridesRecursive()))
    ...
    @undo.chunk('Create and append a group')
    def createGroup(self, groupName):
        ...
        """
        with profiler.ProfilerMgr('createGroup'):
            g = group.create(groupName)
            self.appendGroup(g)
            # Note: No need to update visibility here since it's an empty group
            # which will not modify membership.
            return g

    def _collectionAttached(self, child):
        with undo.NotifyCtxMgr(kCollectionDetached % (self.name(), child.name()),
                                partial(self._selectedNodesChanged, container=child)):
            child._attach(self)
            self.descendantAdded(child)

    def _groupAttached(self, child):
        with undo.NotifyCtxMgr(kGroupDetached % (self.name(), child.name()),
                                partial(self._selectedNodesChanged, container=child)):
            child._attach(self)
            self.descendantAdded(child)
    ...
    @undo.chunk('Append to layer')
    def appendContainer(self, child):
        """ Add a collection or group as the highest-priority render collection/group """
        nodeList.append(self, child)
        if child.isGroup():
            self._groupAttached(child)
        else:
            self._collectionAttached(child)

    @undo.chunk('Append to layer')
    def appendGroup(self, child):
        """ Add a group as the highest-priority render collection/group """
        nodeList.append(self, child)
        self._groupAttached(child)

    def attachContainer(self, pos, child):
        if child.isGroup():
            self.attachGroup(pos, child)
        else:
            self.attachCollection(pos, child)
    ...
    def attachGroup(self, pos, child):
        """ Attach a group at a specific position """

        # This method appends the child group to the requested position
        # with the exception of 'special' collections which have dedicated
        # positions.

        with undo.CtxMgr(kAttachGroup % (child.name(), self.name(), pos)):
            nodeList.insert(self, pos, child)
            self._groupAttached(child)

    @undo.chunk('Detach from layer')
    def detachContainer(self, child):
        if child.typeName() == group.Group.kTypeName:
            self.detachGroup(child)
        else:
    ...
    @undo.chunk('Detach group from layer')
    def detachGroup(self, child):
        """ Detach a group whatever is its position
        """
        group.unapply(child) # NoOp if not applied; otherwise commands are used
        with undo.NotifyCtxMgr(
                kGroupDetached % (self.name(), child.name()),
                partial(self._selectedNodesChanged, container=child)):
            # Must perform detach operations before removing from list,
            # otherwise parenting information is gone.
            child._detach(self)
    ...
    def getCollections(self):
        ...
        return [node for node in list(nodeList.forwardListGenerator(self)) if isinstance(node, collection.Collection)]

    def getGroups(self):
        """ Get list of all existing Groups """
        return [node for node in list(nodeList.forwardListGenerator(self)) if isinstance(node, group.Group)]

    def getContainers(self):
        """ Get list of all existing Containers """
        return [node for node in list(nodeList.forwardListGenerator(self)) if isinstance(node, container.Container)]
    ...
    def hasDefaultCollection(self):
        ...
        for col in [node for node in list(nodeList.forwardListGenerator(self)) if isinstance(node, collection.Collection)]:
            if col.name().startswith(defaultCollectionName):
                return True
        return False

    def getDefaultCollection(self):
        """ Get the default group where newly created nodes are placed """
        defaultCollectionName = "_untitled_"
        for col in [node for node in list(nodeList.forwardListGenerator(self)) if isinstance(node, collection.Collection)]:
            if col.name().startswith(defaultCollectionName):
                return col
    ...
    def getContainerByName(self, containerName, nested=False):
        """ Look for an existing collection/group by name """
        if not containerName:
            raise Exception(kInvalidContainerName)

        for child in nodeList.forwardListGenerator(self):
            if child.name() == containerName:
                return child
            elif nested and (child.typeId() == typeIDs.collection or child.typeId() == typeIDs.group):
                child2_col = child.getCollectionByName(containerName, True)
                if child2_col:
                    return child2_col
                child2_grp = child.getGroupByName(containerName, True)
                if child2_grp:
                    return child2_grp

        raise Exception(kUnknownContainer % (containerName, self.name()))
    ...
    def getGroupByName(self, groupName, nested=False):
        """ Look for an existing group by name """
        if not groupName:
            raise Exception(kInvalidGroupName)

        for child in nodeList.forwardListGenerator(self):
            if child.name() == groupName:
                return child
            elif nested and child.typeId() == typeIDs.group:
                child2 = child.getGroupByName(groupName, True)
                if child2:
                    return child2

        raise Exception(kUnknownGroup % (groupName, self.name()))
    ...
    def hasCollection(self, collectionName):
        for collection_node in [node for node in list(nodeList.forwardListGenerator(self)) if isinstance(node, collection.Collection)]:
            if collection_node.name() == collectionName:
                return True
        return False
    ...
    def getMembers(self):
        ...
        with profiler.ProfilerMgr('RenderLayer::getMembers'):
            ...
            if lightsAddedByDefaultToLayer:
                lights = [_f for _f in (commonUtils.nameToDagPath(name) for name in cmds.ls(type=cmds.listNodeTypes('light'))) if _f]
            ...
            for c in traverse.depthFirst(self, memberTraversal):
                if not isinstance(c, container.Container):
                    continue
                ...
                if c.isCollection():
                    ...
                    paths = []
                    paths = self.findMembersRecursive(c, paths, c.isEnabled())
    ...
    def findMembersRecursive(self, node, paths, groupEnabled):
        if node.hasSelector():
            paths.extend(node.getSelector().members())
        else:
            for item in [child for child in node.getChildren() if child.isEnabled() or not groupEnabled]:
                self.findMembersRecursive(item, paths, groupEnabled)
        return paths
    ...
    def findGroup(self, predicate, creator=None):
        '''Find the group of this layer satisfying the predicate function or creates it
        with the creator function if not found and a creator function is specified.
        Function signatures are:
          predicate(group): returns boolean.
          creator(void) : returns the created node.'''
        for col in self.getGroups():
            if predicate(col):
                return col
        if not creator:
            return None
        col = creator()
        self.appendGroup(col)
        return col
    ...
    def getCorrespondingCollection(self, nodeName, selectedCollectionName):
        """
            ...
            Note: lights collection static selection will be updated for each new light
        """
        ...
        # Search if the node should be part of the Lights Collection
        if not coll and collection.LightsCollection.containsNodeName(nodeName):
            ...
            selectionName = nodeName
            mayaObj = commonUtils.nameToNode(nodeName)
            if lightTypeManager.isLightEditorItem(mayaObj):
                item = utils.nameToUserNode(nodeName)
                if item.typeId() == typeIDs.lightItem:
                    selectionName = item.getLightName(fullPath = True)
            else:
                shape = cmds.listRelatives(nodeName, f=True, s=True)
                selectionName = nodeName if cmds.objectType(nodeName, isAType="shape") else shape[0]

            # If it doesn't exist, create a new child node
            with undo.CtxMgr(kCreateLightsChildCollection % self.name()):

                # Create a new Lights child collection and put it last in the list
                collectionName = selectionName + "_col"
                coll = collection.create(collectionName, collection.LightsChildCollection.kTypeId)
                coll.getSelector().setStaticSelection(selectionName)
                lightsCol.appendChild(coll)

                if selectionName not in list(lightsCol.getSelector().names()):
                    prev = lightsCol.getSelector().getStaticSelection()
                    lightsCol.getSelector().setStaticSelection(prev + '\n' + selectionName if len(prev) else selectionName)
    ...
    #TODO UGLY_RENDER_SETUP_HACK, remove this guard (see Tattribute.h)
    ...
    def _getFrontAttr(self):
        return RenderLayer.containerLowest

    def _getBackAttr(self):
        return RenderLayer.containerHighest

    def _getListItemsAttr(self):
        return RenderLayer.containers
    ...
    def _importContainer(self, containerName, nodeType):
        """ Create a container of given type.
        ...
        if nodeType == collection.LightsCollection.kTypeName:
            ...
            return self.aovCollectionInstance()
        elif nodeType == group.Group.kTypeName:
            return self.createGroup(containerName)
        else:
            return self.createCollection(containerName)

    def _importGroup(self, groupName, nodeType):
        """ Create a group of given type.
    ...
    def _encodeProperties(self, dict):
        ...
        groups_found = jsonTranslatorUtils.encodeObjectArray(self.getGroups())
        if len(groups_found) > 0:
            dict[jsonTranslatorGlobals.GROUPS_ATTRIBUTE_NAME] = groups_found
    ...
    def copyForClipboard(self):
        data = self.encode()
        data[self.typeName()][jsonTranslatorGlobals.VISIBILITY_ATTRIBUTE_NAME] = False
        return data

    def _decodeChildren(self, children, mergeType, prependToName):
        jsonTranslatorUtils.decodeObjectArray(children,
                                              jsonTranslatorUtils.MergePolicy(self.getContainerByName,
                                                                              self._importContainer, mergeType, prependToName))
    ...
    def _decodeProperties(self, dict, mergeType, prependToName):
        ...
        if jsonTranslatorGlobals.COLLECTIONS_ATTRIBUTE_NAME in dict:
            self._decodeChildren(dict[jsonTranslatorGlobals.COLLECTIONS_ATTRIBUTE_NAME], mergeType, prependToName)

        if jsonTranslatorGlobals.GROUPS_ATTRIBUTE_NAME in dict:
            self._decodeChildren(dict[jsonTranslatorGlobals.GROUPS_ATTRIBUTE_NAME], mergeType, prependToName)
    ...
    def isAcceptableChild(self, modelOrData):
        ...
        return ((typeName == collection.LightsCollection.kTypeName and not self.hasLightsCollectionInstance()) or
               ...
               typeName == collection.Collection.kTypeName or typeName == group.Group.kTypeName
...
def delete(renderLayer):
    ...
    # Containers detach themselves on delete.
    for child in renderLayer.getChildren():
