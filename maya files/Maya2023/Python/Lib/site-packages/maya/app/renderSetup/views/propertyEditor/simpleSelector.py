...
import maya.app.renderSetup.model.renderSetupPreferences as rsPrefs
...
class SimpleSelector(object):
    ...
    INVERSE_STRING = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kInverse' ]
    SELECT_STRING = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kSelect' ]
    CREATE_EXPRESSION_STRING = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kCreateExpression' ]
    DRAG_DROP_FILTER_STRING = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kDragDropFilter' ]
    ...
    def _setupExpression(self, expressionLabel, expressionChangedCB, expressionFinishedCB):
        ...
        tip =  maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr'  ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr1' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr2' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr3' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr4' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr5' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr6' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr7' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr8' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr9' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr10' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr11' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr12' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr13' ]
        tip += maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionTooltipStr14' ]
        expression.setToolTip(tip)

        expression.textChanged.connect(expressionChangedCB)
        # expression pattern is saved as its being typed
        expression.textChanged.connect(self.saveExpressionPattern)
        ...
        selectButton.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kExpressionSelectTooltipStr' ])
        ...
        if rsPrefs.IncludeAllLightsSetting.isEnabled() and self.kModelType == selector.LightsCollectionSelector.kTypeName:
            expressionWidget.setEnabled(False)
            expressionLabel.setEnabled(False)
        else:
            expressionWidget.setEnabled(True)
    ...
    def _setupIncludeExpression(self):
        includeExpressionLabel = IncludeExpressionLabel(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kInclude' ])
    ...
    def _setupFilterUI(self, layout):
        ...
        self.filterType.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kFiltersToolTip' ])
        
        for ftype in self._selector.getAvailableFilters():
            # MAYA-97072
            # Will no longer add a kLightsAndTransform type to collection filter list
            # Note that this is a strickly UI fix:
            # kLightsAndTransform should otherwise be returned by getAvailableFilters() since it is needed for MAYA-95550 (light editor overrides)
            if ftype != selector.Filters.kLightsAndTransforms :
                self.filterType.addItem(QIcon(RenderSetupDelegate.getFilterIcon(ftype)), 
                                        selector.Filters.filterUIName(ftype), ftype)

        self.filterType.currentIndexChanged.connect(self._filterTypeChanged)
        ...
        layout.addRow(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kByTypeFilter' ], self.customFilterEdit)       
    ...
    def _filterTypeChanged(self, value):
        ...
        self._selector.setFilterType(filterNameMap[self.filterType.itemText(value)])
    ...
    def _setupStaticSelector(self):
        ...
        addButton = QPushButton(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kAdd' ])
        addButton.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kStaticAddTooltipStr' ])
        ...
        removeButton = QPushButton(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kRemove' ])
        removeButton.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kStaticRemoveTooltipStr' ])
        ...
        selectButton = QPushButton(self.SELECT_STRING)
        selectButton.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kStaticSelectTooltipStr' ])
        selectButton.setMinimumWidth(self.EXPRESSION_BUTTON_WIDTH)
        ...
        # if include all options is enabled, disable the buttons
        if rsPrefs.IncludeAllLightsSetting.isEnabled() and self.kModelType == selector.LightsCollectionSelector.kTypeName:
            staticSelectionWidget.setEnabled(False)
        else:
            staticSelectionWidget.setEnabled(True)
    ...
    def _setupSelectAllButton(self):
        ...
        selectAllButton = QPushButton(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kSelectAll' ])
    ...
    def _setupFilterGroupBox(self, layout):
        filterGroupBox = QGroupBox(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kCollectionFilters' ])
        font = QFont()
        font.setBold(True)
    ...
    def _setupAddToCollectionGroupBox(self, layout):
        addToCollectionGroupBox = QGroupBox(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kAddToCollection' ])
        font = QFont()
        font.setBold(True)
    ...
    def saveExpressionPattern(self, currentText):
        self._selector.setPreviousPattern(currentText)

class BasicSelector(SimpleSelector):

    # User-visible usage message.  Can be used as tool tip.
    kUsage = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kBasicSelectorUsage' ]

    # User-visible display type.
    kDisplayType = maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kBasicSelectorDisplayType' ]
    ...
    def _setupIncludeHierarchy(self, layout):
        self.includeHierarchy = QCheckBox(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kIncludeHierarchy' ])
        self.includeHierarchy.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kIncludeHierarchyTooltipStr' ])
...
class LightsCollectionSelector(SimpleSelector):
    ...
    kModelType = selector.LightsCollectionSelector.kTypeName

    def __init__(self, selector):
        super(LightsCollectionSelector, self).__init__(selector)
        self.includeExpression = None
        self._selector = selector
        self.staticSelector = None
        self.includeAllLightsOption = None
        self._blockChangeMessages = False
    ...
    def _setupCollectionGroupBox(self):
        self._setupIncludeAllLightsOption()
        super(LightsCollectionSelector, self)._setupCollectionGroupBox()

    def _setupIncludeAllLightsOption(self):
        includeAllLightsOptionWidget = QWidget()
        includeAllLightsOptionLayout = QHBoxLayout()
        includeAllLightsOptionLayout.setContentsMargins(utils.dpiScale(12), 0, 0, 0)
        includeAllLightsOptionWidget.setLayout(includeAllLightsOptionLayout)
        self.includeAllLightsOption = QCheckBox(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kIncludeAllLightsOption' ])
        self.includeAllLightsOption.setToolTip(maya.stringTable['y_maya_app_renderSetup_views_propertyEditor_simpleSelector.kIncludeAllLightsOptionStr' ])
        includeAllLightsOptionLayout.addWidget(self.includeAllLightsOption)
        self.includeAllLightsOption.stateChanged.connect(lambda x: self.setIncludeAllLightsOption(x == Qt.Checked))
        self.addToCollectionGroupBoxLayout.addRow(includeAllLightsOptionWidget)

    def setIncludeAllLightsOption(self, value):
        rsPrefs.IncludeAllLightsSetting.setOptionVar(value)

    @guard.member('_blockChangeMessages', True)
    def populateFields(self):
        # When the user toggles include all lights, the actual expression pattern used is "*", but the old expression is displayed to the user as greyed out text
        # If the users toggles off include all lights checkbox, the old expression is once more available to the user
        if rsPrefs.IncludeAllLightsSetting.isEnabled():
            self._selector.setPattern("*")
        else:
            self._selector.setPattern(self._selector.getPreviousPattern())       
        # previousPattern == last text that was typed by user
        expressionText = self._selector.getPreviousPattern()
     
        self.includeExpression.setText(expressionText)
        self.includeAllLightsOption.setCheckState(Qt.Checked if rsPrefs.IncludeAllLightsSetting.isEnabled() else Qt.Unchecked)
        self.staticSelector.populate()
# ===========================================================================
