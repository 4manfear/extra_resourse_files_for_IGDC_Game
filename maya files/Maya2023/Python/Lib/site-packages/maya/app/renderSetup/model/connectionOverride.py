...
kApplyNodeNoRenderLayerConnection = maya.stringTable['y_maya_app_renderSetup_model_connectionOverride.kApplyNodeNoRenderLayerConnection' ]
kAttrValueAlreadyCreated = maya.stringTable['y_maya_app_renderSetup_model_connectionOverride.kAttrValueAlreadyCreated' ]
kMaterialOverrideFailure = maya.stringTable['y_maya_app_renderSetup_model_connectionOverride.kMaterialOverrideFailure' ]
kMaterialTemplateOverrideFailure = maya.stringTable['y_maya_app_renderSetup_model_connectionOverride.kMaterialTemplateOverrideFailure' ]
...
class MaterialTemplateOverride(ConnectionOverride):
    """MaterialTemplate override node.

    Specialization of connection override for material template assignments.

    This override node type implements a material template override
    for DAG nodes.

    A materialTemplate assigns one or more shading engines to specific components (as
    defined by componentTag expressions) to a geometry. Note that a shadingEngine itself
    can be seen as a simple materialTemplate which means to assign that shadingEngine
    to the whole shape.

    Material template assignments in Maya are represented by connections to the
    instMaterialAssign attribute on the shape node. It's an array attribute that represents 
    per-instance assignments in the following way:

    myShape.instMaterialAssign[N] - connection to this represents material template
    assignment to instance number N.

    The connections are made from myTM.msg to myShape.instMaterialAssign[N],
    where myTM is a materialTemplate or shadingEngine node, which represents that this
    material template or shading engine is assigned to that instance of the shape.
    """

    kTypeId = typeIDs.materialTemplateOverride
    kTypeName = "materialTemplateOverride"

    @staticmethod
    def creator():
        return MaterialTemplateOverride()

    @staticmethod
    def initializer():
        MaterialTemplateOverride.inheritAttributesFrom(ConnectionOverride.kTypeName)

    def __init__(self):
        super(MaterialTemplateOverride, self).__init__()
    ...
    def postConstructor(self):
        ...
        # Call parent class postConstructor
        super(MaterialTemplateOverride, self).postConstructor()
    ...
    def isValid(self):
        ...
        # in order to know on what plugs they will apply => they can be considered valid and create their apply 
        # override nodes even if they don't have a connection override set.
        return super(MaterialTemplateOverride, self).isValid()

    def _isPolymorphic(self):
        return False # Attribute is fixed and created in postCostructor above

    @undo.chunk('Set material template on material override')
    def setMaterialTemplate(self, name):
        self.setSource(name+'.message')
    ...
    def _targets(self, selectedNodeNames=None):
        ...
        for dagPath in dagPaths:
            src = dagPathUtils.DagPath(dagPath).getInstMaterialAssignPlug()
            yield src
...
class ApplyConnectionOverride(applyOverride.LeafClass, applyOverride.ApplyOverride):
    ...
    def update(self):
        last = self._getLast()
        toApply = next((ao for ao in ApplyConnectionOverride.reverseGenerator(last) if ao.isApplicable() or not ao.prev()))
    ...
    def insert(self, target, nextOvr=None):
        ...
        if not nextOvr:
            ...
            nextAO = next((ao for ao in ApplyConnectionOverride.reverseGenerator(firstApplyNode) if ao.override() == nextOvr))
            prevAO = nextAO.prev()
    ...
    def extract(self):
        ...
        prevAO, nextAO = self.prev(), next(self)
