...
def resolvePresetFile(fileName):
    ...
    if os_path.dirname(fileName) is None or os_path.dirname(fileName) == "":
        # Get an absolute path from our relative one, does the file exist?
        resolvedPath = os_path.realpath(fileName)
        if os_path.exists(resolvedPath):
            return resolvedPath
        # Is the path in our user presets directory?
        userPresetsDir = prefs.getUserPresetsDirectory()
        if userPresetsDir and os_path.exists(userPresetsDir):
            resolvedPath = os_path.join(userPresetsDir, fileName)
            if os_path.exists(resolvedPath):
                return resolvedPath
        # Is the path in our global presets directory?
        globalPresetsDir = prefs.getGlobalPresetsDirectory()
        if globalPresetsDir and os_path.exists(globalPresetsDir):
            resolvedPath = os_path.join(globalPresetsDir, fileName)
            if os_path.exists(resolvedPath):
                return resolvedPath
    return fileName
...
def resolveTemplateFile(fileName):
    ...
    if os_path.dirname(fileName) is None or os_path.dirname(fileName) == "":
        ...
        if userTemplateDir and os_path.exists(userTemplateDir):
            resolvedPath = os_path.join(userTemplateDir, fileName)
            if os_path.exists(resolvedPath):
                return resolvedPath
        # Is the path in our global template directory?
        globalTemplateDir = prefs.getGlobalTemplateDirectory()
        if globalTemplateDir and os_path.exists(globalTemplateDir):
            resolvedPath = os_path.join(globalTemplateDir, fileName)
            if os_path.exists(resolvedPath):
                return resolvedPath
    return fileName
