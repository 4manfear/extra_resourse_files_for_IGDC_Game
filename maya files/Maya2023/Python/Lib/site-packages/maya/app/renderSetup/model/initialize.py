...
from maya.app.renderSetup.model.selector import Selector, BasicSelector, SimpleSelector, LightsCollectionSelector
...
from maya.app.renderSetup.model.group import getAllGroupClasses
...
import maya.app.renderSetup.model.renderSetupPreferences as rs_prefs
...
# base class.
#
nodeTypes  = [nodeList.ListItem, childNode.ChildNode, Selector, SimpleSelector, LightsCollectionSelector, BasicSelector, RenderLayer, renderSetup.RenderSetup ]
nodeTypes += [container.Container]
nodeTypes += getAllCollectionClasses()
nodeTypes += getAllGroupClasses()
nodeTypes += getAllOverrideClasses()
nodeTypes += getAllApplyOverrideClasses()

# Add all the names of the render setup node types to be listed in the
# outliner. NOTE: The legacy render layers should be among them, but the apply
# override nodes should never be listed, as they are mainly an implementation
# detail (the user should not know about those nodes), and since they are
# constantly deleted and recreated during layer switches or scene deletion,
# hiding them is mandatory. (MAYA-92929)
renderSetupNodeNamesToShowInOutliner = [nodeType.kTypeName for nodeType in getAllCollectionClasses() +
                                                                           getAllGroupClasses() +
                                                                           getAllOverrideClasses()   +
    [nodeList.ListItem, childNode.ChildNode, Selector, SimpleSelector, LightsCollectionSelector, BasicSelector, RenderLayer, renderSetup.RenderSetup ]]
renderSetupNodeNamesToShowInOutliner.append("renderLayer")
...
commands = [nodeListPrivate.RemoveCmd, nodeListPrivate.InsertCmd,
            ...
            undo.NotifyPostRedoCmd, undo.NotifyPostUndoCmd, undo.NotifyDecoratorCmd,
            plug.AddDynamicAttribute,
            renderSetupPrivate.PostApplyCmd
]
syntaxCommands = [modelCmds.RenderSetupCmd, modelCmds.RenderSetupLegacyLayerCmd, localOverride.RenderSetupLocalOverrideCmd]
...
def setVisibilityNodes(val, nodeTypeNamesList):
    ...
    """
    for nodeTypeName in nodeTypeNamesList:
        cmds.setNodeTypeFlag(nodeTypeName, display=val)
...
def initialize(mplugin):
    ...
    # NOTE: It is imperative that the apply overrides be hidden, because they
    # are an implementation detail and deleting them while they are selected
    # leads to crashes. (MAYA-92929)
    setVisibilityNodes(False, [nodeType.kTypeName for nodeType in getAllApplyOverrideClasses()])

    # Hide or show most render setup nodes depending on the value
    # of a preference.
    setVisibilityNodes(rs_prefs.DisplayRSNodesSetting.isEnabled(), renderSetupNodeNamesToShowInOutliner)
