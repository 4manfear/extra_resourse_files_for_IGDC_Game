...
def getTypeNameFromDictionary(encodedData):
    ...
    """
    return list(encodedData.keys())[0] if isinstance(encodedData, dict) else None


def getObjectNotes(encodedData):
    """
        Get the Notes from any Render Setup Object knowing that all nodes
        could or could not contain the 'notes' dynamic attribute.
    """
    if _isRenderSetup(encodedData):
        ...
        if jsonTranslatorGlobals.NOTES_ATTRIBUTE_NAME in list(data.keys()):
    ...
    if isRenderSetupTemplate(encodedData):
        ...
        data = data[list(data.keys())[0]]  # Jump over the node type
        if jsonTranslatorGlobals.NOTES_ATTRIBUTE_NAME in list(data.keys()):
            return data[jsonTranslatorGlobals.NOTES_ATTRIBUTE_NAME]
...
def decodeObjectArray(jlist, policy):
    """
        Decode an array of Render Setups Objects

        jlist is a list of dictionaries (json representation for a array of objects)
