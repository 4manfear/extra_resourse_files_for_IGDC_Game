...
class Collection(basicCollection.BasicCollection):
    ...
    def _clearSelectorWidgets(self):
        ...
        # RuntimeError: 'Internal C++ object (PySide2.QtGui.QVBoxLayout) already deleted.
        # The following from
        # http://stackoverflow.com/questions/4528347/clear-all-widgets-in-a-layout-in-pyqt
        for i in reversed(list(range(self._selectorLayout.count()))):
    ...
    def _getObjects(self):
        # Create a set to avoid duplicates in the list of objects
        objects = list(self.getModelSelector().names())
        return [o for o in objects if not cmds.getClassification(cmds.objectType(o), satisfies="hidden")]

    def _selectAllCb(self, data):
        commonUtils.echoSelect(self._getObjects())
    ...
    def _viewAllCb(self, data):
        ...
        # Pass in the dictionary to show as well as the nodes in the collection for later calculations
    ...
    def _dropCb(self, dragControl, dropControl, messages, x, y, dragType):
        ...
        if not messages or not isinstance(messages[0], string_type):
            return
