"""Render setup singleton node.
   ...
   - Each render layer has a list of collections and/or groups, and an optional list of
     overrides.
   - Each collection has a selector, a list of overrides, and an optional
     list of child collections.
   - Each group has a list of child containers
...
kInvalidRenderLayerName      = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kInvalidRenderLayerName'      ]
kUnknownRenderLayer          = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kUnknownRenderLayer'          ]
kUnknownLegacyRenderLayer    = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kUnknownLegacyRenderLayer'    ]
kRenderSetupNodeTypeMismatch = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kRenderSetupNodeTypeMismatch' ]
kRenderSetupNodeNameMismatch = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kRenderSetupNodeNameMismatch' ]
kSyncingActiveLayerMessage   = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kSyncingActiveLayerMessage'   ]
kFileRefCbFailed             = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kFileRefCbFailed'  ]
kBefore                      = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kBefore' ]
kAfter                       = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kAfter' ]
kLoading                     = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kLoading' ]
kUnloading                   = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kUnloading' ]
kRegisterFailed              = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kRegisterFailed' ]
kUnregisterFailed            = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kUnregisterFailed' ]
kRenderSetupCleanupFailed    = maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kRenderSetupCleanupFailed' ]
...
class InMasterLayerCtxMgr(object):
    ...
    def __init__(self):
        ...
        self.prevVisibleRenderLayer = None

    def __enter__(self):
        if not hasInstance():
            return
        if InMasterLayerCtxMgr.counter == 0:
            # Store current layer and change to master layer if needed
            rs = instance()
            visibleRenderLayer = rs.getVisibleRenderLayer()
            self.prevVisibleRenderLayer = None
            if visibleRenderLayer != rs.getDefaultRenderLayer():
                self.prevVisibleRenderLayer = visibleRenderLayer
                rs.switchToLayer(None)
        InMasterLayerCtxMgr.counter += 1

    def _visibleRenderLayerStillExists(self):
        # Checks to see if the visible render layer still exists after running
        # the specified command as it's possible it will no longer be around,
        # so we won't be able to switch back to it. One situation where this
        # occurs is when importing all and doing a replace whcih deletes the
        # previously visible layer.
        return self.prevVisibleRenderLayer in instance().getRenderLayers()

    def __exit__(self, type, value, traceback):
        if not hasInstance():
            return
        # Revert to previous layer if needed
        if InMasterLayerCtxMgr.counter == 1 and self.prevVisibleRenderLayer and self._visibleRenderLayerStillExists():
            instance().switchToLayer(self.prevVisibleRenderLayer)
        InMasterLayerCtxMgr.counter -= 1

def inMasterLayer(f):
    def wrapper(*args, **kwargs):
        with InMasterLayerCtxMgr():
...
def availableOverrides(nodeName, attrName):
    ...
    return plg.availableOverrides() if \
        plg is not None and not commonUtils.isNodeInstance(plg.node(), override.Override) else 0

def canOverride(nodeName, attrName):
    ''' The method checks if an override could be 'applied' to the specified node/attribute '''
    return availableOverrides(nodeName, attrName) != 0
...
def _refNodeHasFile(refNodeName):
    ...
    try:
        fileName = cmds.referenceQuery(refNodeName, filename=True)
...
class RenderSetup(nodeList.ListBase, nodeNotes.NodeNotes, serializableNode.SerializableNode, OpenMaya.MPxNode):
    ...
    def __init__(self):
        ...
        # Any creation of a render setup node after the singleton instance is for imported or referenced files.
        # These render setups must be considered inactive: only the top-level render setup node matters.
        if hasInstance() or OpenMaya1_0.MFileIO.isReferencingFile() or OpenMaya1_0.MFileIO.isImportingFile():
            return
        ...
        self._callbacks = {
            ...
            sceneObservable.SceneObservable.NODE_REMOVED : self._onMayaNodeRemovedCB,
            ...
            sceneObservable.SceneObservable.BEFORE_FILE_EXPORT : self._beforeFileExport,
            sceneObservable.SceneObservable.FILE_EXPORTED      : self._fileExported}

        for type, callback in list(self._callbacks.items()):
        ...
        self._cbIds.append(OpenMaya.MEventMessage.addEventCallback("renderSetupAutoSave", self._beforeAutosave, self))
    ...
    def _beforeAutosave(self, clientData):
        self._autoSaveInProgress = True
    ...
    def _onMayaNodeAddedCB(self, obj):
        ...
        # Light shapes are a special case: they need to be added to layer
        # membership since, by default, all lights should be visible.
        # There is an exception when the IncludeAllLights setting is off,
        # because then lights have to be explicitly added to the layer.
        if mayaObj.hasFn(OpenMaya.MFn.kShape) and userPrefs.IncludeAllLightsSetting.isEnabled():
            if self.getDefaultRenderLayer().isVisible():
                return
            fn = OpenMaya.MFnDagNode(mayaObj)
            if cmds.getClassification(fn.typeName, satisfies='light'):
                self.getVisibleRenderLayer().addMembers([mayaObj])
                # Don't add light shapes to the default collection
                return

        # During file save, file import and file referencing, don't add to
        # the default collection.
        if OpenMaya1_0.MFileIO.isReadingFile() or \
           OpenMaya1_0.MFileIO.isWritingFile():
            return

        # If not using default/untitled collections, the added object
        # should not be made visible by adding it to the default collection.
        # NOTE: This call has to be made after the case where we check if the
        # object is a light shape, as it would otherwise prevent the newly
        # created light from being added to the layer. (MAYA-91936)
        if not userPrefs.UntitledCollectionsSetting.isEnabled():
            return
        ...
        # If the default layer is active, there is no need to add the object
        ...
        # NOTE: If we ever want to make light cameras part of layer membership,
        # we would have to compute that conditional as another special case
        # just like light shapes, before the code for default collections,
        # so that the light cameras are added to layer. (MAYA-91936)
        if _isLightCamera(mayaObj):
            return

        # Don't add lights to the default collection
        # (light shapes are added to the members of the layer)
        if _isLightNode(mayaObj):
            return

        if self._duplicated is not None:
            # append to duplicate array and add when duplicating is done
            # MObjectHandle is safer as MObject is either valid or queriable to know if underlying object was destroyed or not.
            # Keeping reference to destroyed MObject leads to crash when querying anything on it.
            self._duplicated.append(OpenMaya.MObjectHandle(obj))
        else:
    ...
    def _onMayaNodeRemovedCB(self, obj):
        visibleLayer = self.getVisibleRenderLayer()
        if visibleLayer:
            visibleLayer.setObjectDeleted()
    ...
    def nodes(self):
        ...
        '''
        # Get all node types defined by this plugin.
        typeNames = cmds.pluginInfo('renderSetup', q=True, dependNode=True)

        # Loop over all types, and return nodes we find.
        for typeName in typeNames:
            nodeNames = cmds.ls(type=typeName)
            for nodeName in nodeNames:
                yield commonUtils.nameToNode(nodeName)

        # Also return the legacy render layers attached to the render setup
        # render layers.
        for layer in self.getRenderLayers():
            yield layer.getLegacyNode()

    def _setAllRSNodesDoNotWrite(self, state):
        for n in self.nodes():
            OpenMaya.MFnDependencyNode(n).setDoNotWrite(state)

    def _beforeFileExport(self):
        # Render setup nodes are not part of the scene per se, so don't export
        # them.
        from os import getenv as os_getenv
        if os_getenv('MAYA_BATCH_RENDER_EXPORT'):
            return
        # Autosave in Maya unfortunately also uses export All. We will export render setup nodes if the exportAll event is triggered by Autosave
        if not self._autoSaveInProgress:
            self._setAllRSNodesDoNotWrite(True)
        else:
            cmds.warning(maya.stringTable['y_maya_app_renderSetup_model_renderSetup.kExportingRenderSetupLayerContentAndLightEditor' ])

    def _fileExported(self):
        # Once export is done, restore render setup nodes to being writable.
        from os import getenv as os_getenv
        if os_getenv('MAYA_BATCH_RENDER_EXPORT'):
            return
        self._setAllRSNodesDoNotWrite(False)
        self._autoSaveInProgress = False
    ...
    def _afterDuplicate(self, clientData):
        if len(self._duplicated) > 0:
            self.getVisibleRenderLayer().addDefaultMembers(obj.object() for obj in self._duplicated)
        self._duplicated = None
    ...
    def _beforeUnloadReferenceCB(self, referenceNode, resolvedRefPath):
        ...
        # Inform all selectors that a reference unload will occur.
        for layer in self.getRenderLayers():
            for c in utils.getCollectionsRecursive(layer):
                c.getSelector().beforeBulkChange()
    ...
    def _afterUnloadReferenceCB(self, referenceNode, resolvedRefPath):
        ...
        # Inform all selectors that a reference unload has occurred.
        for layer in self.getRenderLayers():
            for c in utils.getCollectionsRecursive(layer):
                c.getSelector().onBulkChange()
    ...
    def _afterOpenCB(self, clientData):
        ...
        if layer != self._defaultRenderLayer:
            ...
            if refNodeNames:
                ...
                # If the referenced file does not exist, the reference node
                # won't be associated with a file, and won't be loaded.
                # However, calling MFnReference methods on such a reference
                # will always fail, so filter those reference nodes out.
                refNodes = (commonUtils.nameToNode(r) for r in refNodeNames if
                            _refNodeHasFile(r))
    ...
    def decode(self, encodedData, behavior=DECODE_AND_MERGE, prependToName=None):
        ...
        if not isinstance(encodedData, dict):
        ...
        if not self.kTypeName in encodedData:
            raise TypeError(jsonTranslatorGlobals.kUnknownKeys % list(encodedData.keys())[0])

        with InMasterLayerCtxMgr():
            # Decode the content of the scene settings part
            if jsonTranslatorGlobals.SCENE_SETTINGS_ATTRIBUTE_NAME in encodedData:
                # Render setup should not need to know about renderSettings, this was done to avoid a cyclic dependency
            ...
            if (jsonTranslatorGlobals.SCENE_AOVS_ATTRIBUTE_NAME in encodedData and
                ...
                # Render setup should not need to know about aovs, this was done to avoid a cyclic dependency
            ...
            if jsonTranslatorGlobals.SCENE_LIGHTEDITOR_ATTRIBUTE_NAME in encodedData:
                import maya.app.renderSetup.lightEditor.model.editor as lightEditor
                lightEditor.instance().decode(encodedData[jsonTranslatorGlobals.SCENE_LIGHTEDITOR_ATTRIBUTE_NAME], behavior)
    ...
    def encode(self, notes=None, includeSceneSettings=True):
        # Note: The includeSceneSettings option will be true if user would like to export render settings/aovs settings and light group schemas else false
        # User can change this option in the Export All dialog box
        encodedData = super(RenderSetup, self).encode(notes)
        if includeSceneSettings:
            # Render setup should not need to know about renderSettings/aovs, this was done to avoid a cyclic dependency
            import maya.app.renderSetup.model.renderSettings as renderSettings
            import maya.app.renderSetup.model.aovs as aovs
            import maya.app.renderSetup.lightEditor.model.editor as lightEditor
            ...
            encodedData[jsonTranslatorGlobals.SCENE_LIGHTEDITOR_ATTRIBUTE_NAME] = lightEditor.instance().encode()
    ...
    def importAllFromFile(self, filePath, behavior, prependToName):
        ...
        with open(filePath, "r") as file:
            ...
            self.decode(json.load(file), behavior, prependToName)
...
class _Subject(object):
    ...
    def _beforeFileNew(self):
        '''Engine method for before new and before open callbacks.'''

        conversion.Observer2016R2.instance().deactivate()

        if hasInstance():
            self._renderSetupPreDelete()

        # Stop listening to any scene changes.  Note how we observe Maya for
        # SceneObservable, to avoid having two different file new callbacks,
        # thus removing any possible callback order dependence problems.
        sceneObservable.instance().beforeNew()
    ...
    def _renderSetupPreDelete(self):
        """Call the renderSetupPreDelete() methods on render setup observers.

        The order in which observers are called is not specified."""

        assert hasInstance(), 'Call to _renderSetupPreDelete with no instance.'

        # Reset the master scene visibility upon deletion
        instance().switchToLayer(None)

        # Clean the model and remove model's callbacks.  Since we are
        # in a Maya callback, don't let an exception escape out of
        # dispose(), and cause the rest of the cleanup to be skipped.
        # This can cause an application crash (see MAYA-74041).
        try:
            instance().dispose()
        except Exception as e:
            OpenMaya.MGlobal.displayError(kRenderSetupCleanupFailed % str(e))
            # Re-raise the exception, so that the Python callback will be
