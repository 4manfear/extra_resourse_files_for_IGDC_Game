...
kChildAttached = maya.stringTable['y_maya_app_renderSetup_lightEditor_model_group.kChildAttached' ]
kChildDetached = maya.stringTable['y_maya_app_renderSetup_lightEditor_model_group.kChildDetached' ]
...
class LightGroup(nodeList.ListBase, itemModel.LightItemBase):
    ...
    def __init__(self):
        ...
        self.originalImportName = None
        self._lightgroupCallBackIDs = []
    ...
    def activate(self):
        super(LightGroup, self).activate()
        if len(self._lightgroupCallBackIDs) == 0:
            self._lightgroupCallBackIDs.append(OpenMaya.MNodeMessage.addNameChangedCallback(self.thisMObject(), self._onNameChangedCB))

    def deactivate(self):
        super(LightGroup, self).deactivate()
        OpenMaya.MMessage.removeCallbacks(self._lightgroupCallBackIDs)
        self._lightgroupCallBackIDs = []
    ...
    def _onNameChangedCB(self, obj, previousName,clientData):
        '''Update the import/export missing light tracker dictionary if applicable'''
        from maya.app.renderSetup.lightEditor.model.editor import LightEditorImportExportHelper
        if self.originalImportName and LightEditorImportExportHelper.lightgroupToMissingChild and self.originalImportName in LightEditorImportExportHelper.lightgroupToMissingChild:
            LightEditorImportExportHelper.oldParentNameToNewName[self.originalImportName] = self.name()
            LightEditorImportExportHelper.parentHasNewName[self.originalImportName] = True
