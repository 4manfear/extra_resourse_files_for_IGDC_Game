...
# Errors
kUnapplyCmdPrivate = maya.stringTable['y_maya_app_renderSetup_model_override.kUnapplyCmdPrivate' ]
kUnconnectableAttr = maya.stringTable['y_maya_app_renderSetup_model_override.kUnconnectableAttr' ]
kUnfinalized       = maya.stringTable['y_maya_app_renderSetup_model_override.kUnfinalized'       ]
kMissingDependencies = maya.stringTable['y_maya_app_renderSetup_model_override.kMissingDependencies' ]
...
def attributeToPlug(f):
    ...
    def attributeToPlugDecorator(*args, **kwargs):
        if '.' not in args[1]:
            ...
            if nodeName is None:
                raise commonUtils.ReportableException(maya.stringTable['y_maya_app_renderSetup_model_override.kCouldntFigurePlugFromAttr' ] % (args[1], args[1]))
...
class ValueOverride(Override):
    ...
    def _isApplicableTo(self, plg):
        ...
        #TODO UGLY_RENDER_SETUP_HACK, remove this assert (see Tattribute.h)
    ...
    def _targets(self, selectedNodeNames=None):
        ...
        return [plg for plg in (commonUtils.findPlug(node, attrName) for node in nodes) if plg and OpenMaya.MFnAttribute(plg.attribute()).writable and self._isApplicableTo(plg)]
    ...
    def _applyOne(self, plugToOverride, nextOvr=None):
        ...
        # If the override is a child index with a multi array (e.g. ramp colors), we need to check if the index exists first; we'll only apply the override if it does.
        # Attempting to apply an override on an index that does not exists will trigger a new index attr creation.
        # This behavior should be avoided since it is not consistent with Render Setup's expected behaviour (render setup does not create new items)
        if plugToOverride.isChild:
            if plugToOverride.node().apiTypeStr == "kRamp":
                parent = plugToOverride.parent()
                targetIndex = -1
                # Try catch is a workaround for 'plug is not an array' runtime error
                # Mplug::isArray flag is a more appropriate method, but it is currently broken and cannot be used.
                try:
                    targetIndex = parent.logicalIndex()
                except TypeError:
                    pass
                self.missingMultiName = None
                arrayAttributeName = plugToOverride.info.split("[")[0]
                validMultiNodes = cmds.getAttr(arrayAttributeName, mi=True)
                valid = False
                for knownIndex in validMultiNodes:
                    if knownIndex == targetIndex:
                        valid = True
                        break
                # Do not apply the override if invalid and set a missingDependency flag
                if not valid:
                    self.missingMultiName = applyOverrideName
                    return
    ...
    def _unapplyOne(self, apply):
        ...
        if plugToOverride:
            with plug.UnlockedGuard(plugToOverride[0]):
                ...
                # MAYA-79885
                # Plug can now be a multi/a child. (i.e. its parent has an array of other overridable attributes)
                # If it is, we need to unapply the override by finding its parent and the multi/child's index within the parent
                if plugToOverride[0].isChild:
                    parent = plugToOverride[0].parent()
                    hasParent = parent.isCompound
                    # Try catch is a workaround for 'plug is not an array' runtime error
                    # Mplug::isArray flag is a more appropriate method, but it is currently broken and cannot be used.
                    try:
                        index = parent.logicalIndex()
                    except TypeError:
                        pass
                ...
                if index != -1 and hasParent:
                    # MAYA-79885: Unapply the plug from the parent
                    plugToOverride[0].selectAncestorLogicalIndex(index, plugToOverride[0].parent().attribute())
    ...
    def acceptsAttr(self, plugName):
        ...
        if plg is None:
            raise commonUtils.ReportableException(maya.stringTable['y_maya_app_renderSetup_model_override.kCouldntFindPlug' ] % plugName)
...
class AbsOverride(LeafClass, ValueOverride):
    ...
    def status(self):
        ...
        if not self.isFinalized():
            return kUnfinalized
        if self._valuePlugHandle.hasMissingDependency() or self.missingMultiName:
            return kMissingDependencies % self._valuePlugHandle.getMissingDependency() if self._valuePlugHandle.hasMissingDependency() else kMissingDependencies % self.missingMultiName
        return None
...
class RelOverride(LeafClass, ValueOverride):
    ...
    def status(self):
        ...
        # missingMultiName is inherited from ValueOverride class.
        if self.missingMultiName:
            return kMissingDependencies % self.missingMultiName
        return None
