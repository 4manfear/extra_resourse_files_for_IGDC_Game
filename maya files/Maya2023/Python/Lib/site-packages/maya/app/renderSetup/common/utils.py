...
kNameToNodeTypeMismatch = maya.stringTable['y_maya_app_renderSetup_common_utils.kNameToNodeTypeMismatch' ]
kAmbiguousName = maya.stringTable['y_maya_app_renderSetup_common_utils.kAmbiguousName' ]
...
def findPlug(node, attr):
    ...
    if isinstance(node, string_type):
        attrName = attr if isinstance(attr, string_type) else OpenMaya.MFnAttribute(attr).name
...
class SettingContextManager(object):
