...
def _getSources(nodeName):
    ...
    renderSettingsChildCollections = (node for name, node in list(rsccDic.items()) if node.getRenderLayer() == visibleLayer and node.parent().name() not in dic)
    ...
    if len(finalCollections) == 0:
        collections = (node for name, node in list(dic.items()) if node.getRenderLayer() == visibleLayer and node.parent().name() not in dic)
