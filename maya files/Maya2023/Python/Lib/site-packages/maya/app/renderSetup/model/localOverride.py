...
def _setLocalOverrideEnabled(newEnabled):
    ...
    if prevEnabled != enabled():
        ...
        _toggle([ov for ov in utils.getOverridesRecursive(rs) if ov.isLocalRender() and ov.isSelfEnabled() and ov.parent().isEnabled()])
...
class RenderSetupLocalOverrideCmd(OpenMaya.MPxCommand):
    ...
    def doIt(self, args):
        try:
            ...
            errorMsg = maya.stringTable['y_maya_app_renderSetup_model_localOverride.kRenderSetupLocalOverrideCmdParsingError' ] + str(e)
        ...
        if argDb.isQuery:
            ...
            errorMsg = maya.stringTable['y_maya_app_renderSetup_model_localOverride.kRenderSetupLocalOverrideCmdInvalidUsage' ]
            OpenMaya.MGlobal.displayError(errorMsg)
            raise RuntimeError(errorMsg)
...
class ExportListener(object):
    ...
    def __init__(self):
        ...
        self._cbIds = [ OpenMaya.MSceneMessage.addCallback(type, callback) \
            for type, callback in list(callbacks.items()) ]
