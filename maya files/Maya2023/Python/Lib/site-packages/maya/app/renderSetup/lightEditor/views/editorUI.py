...
from PySide2.QtCore import QEvent, QModelIndex, QSize, Qt, QItemSelectionModel
from PySide2.QtGui import QPen, QColor, QPainter, QIcon, QPixmap
from PySide2.QtWidgets import QMainWindow, QHBoxLayout, QVBoxLayout, QBoxLayout, QLabel, QFrame, QMenu, QMenuBar, QToolTip, QComboBox
...
kLightEditorTitle     = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kLightEditorTitle'     ]
kHelp                 = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kHelp'                 ]
kMayaHelp             = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kMayaHelp'             ]
kLightEditorHelp      = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kLightEditorHelp'      ]
kLayerText            = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kLayerText'            ]
kDefaultLayerText     = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kDefaultLayerText'     ]

kFile                 = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kFile'                 ]
kExport               = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kExport'               ]
kImport               = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kImport'               ]

kMerge                  = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kMerge' ]
kMergeExplanation       = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kMergeExplanation' ]
kGeneralOptions         = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kGeneralOptions' ]

kLookThroughLightsToolTip = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kLookThroughLightsToolTip' ]
kSnapToSelectedToolTip    = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kSnapToSelectedToolTip'    ]
kPropertyEditorToolTip    = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kPropertyEditorToolTip'    ]
kAcceptImport             = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kAcceptImport'             ]
kOverwrite              = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kOverwrite' ]
kOverwriteExplanation   = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kOverriteExplanation' ]

def _exportLightGroups():
    prefs._exportNodesOfGivenType(editorModel.instance().encode, kExport)


def _importLightGroupsFromPath(path):
    with open(path, "r") as file:
        LightData = json.load(file)
        if not isinstance(LightData, dict):
            raise TypeError(jsonTranslatorGlobals.kUnknownData)
        editorModel.instance().decode(LightData, LightEditorView.importType)
        createLightEditorWindow()

def _importLightGroups(captionName):
    selectedFilePath = cmds.fileDialog2(caption=captionName,
                                        fileFilter='*.'+userPrefs.getFileExtension(), fileMode=1, dialogStyle=2,
                                        optionsUICreate="importLightgroupOptionsUIBuilder",
                                        startingDirectory=userPrefs.getUserPresetsDirectory())
    if selectedFilePath is not None and len(selectedFilePath[0]) > 0:
        _importLightGroupsFromPath(selectedFilePath[0])

def _importLightGroupSchema():
    _importLightGroups(kImport)
...
# Indentation used by the import/export dialog UI
...
class LightEditorView(QTreeView):
    ...
    HALF_FONT_HEIGHT = viewsUtils.dpiScale(7)
    NO_LIGHTS_IMAGE_SIZE = viewsUtils.dpiScale(62)

    # What kind of import ?
    importType = jsonTranslatorGlobals.DECODE_AND_ADD

    # Used by LightEditor->file->Import light groups via fileDialog2 callback
    @staticmethod
    def addImportOptions(parent):
        with ParentGuard():
            ...
            cmds.radioButton(label=kOverwrite,
                onCommand=LightEditorView.setOverwriteImportType, select=True if LightEditorView.importType==jsonTranslatorGlobals.DECODE_AND_ADD else False)
            cmds.text(label=kOverwriteExplanation, align='left')
            cmds.radioButton(label=kMerge,
                onCommand=LightEditorView.setMergeImportType, select=True if LightEditorView.importType==jsonTranslatorGlobals.DECODE_AND_MERGE else False)
    ...
    def setOverwriteImportType(data):
        ...
        """
        LightEditorView.importType = jsonTranslatorGlobals.DECODE_AND_ADD
    ...
    def setMergeImportType(data):
        ...
        """
        LightEditorView.importType = jsonTranslatorGlobals.DECODE_AND_MERGE
    ...
    def setExpanded(self, index, state):
        ...
        utils.setExpandedState(self._getCorrespondingItem(index).model.thisMObject(), state)

    def aboutToDelete(self):
        # Maya-96493:
        # This is a workaround to reduce the closing time of Light Editor when multiple lights are selected.
        # The delay is corrected when the user closes Light Editor with the majority of light selected.
        # Unfortunaly, there is still a delay when only a small subsets of light is selected. See jira comment for more detail.
    ...
    def mousePressEvent(self, event):
        ...
        event = viewsUtils.updateMouseEvent(event)
    ...
    def paintEvent(self, e):
        ...
        if self.model().rowCount() == 0:
            ...
            painter.drawText(textRect, Qt.AlignCenter, maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kNoLayers' ])
    ...
    def showContextMenu(self, point):
        ...
        if numIndexes > 0:
            ...
            pixmap = QPixmap(viewsUtils.dpiScale(100), viewsUtils.dpiScale(100))
    ...
    def _delete(self):
        # MAYA-83820: improved runtime from O(n^2) to O(nlogn). Multi-select delete now works on large datasets (1000+ light items), taking < 1 min. Down from several hours previously.
        # deleteItem, which disconnect the light's plugs, is the current bottleck and cannot be further improved without affecting safety
        editorItems = editorModel.instance().getTraversal()
        selectedIndexes = self.selectedIndexes()
        selectedIndexLookout = {}
        for index in selectedIndexes:
            selectedIndexLookout[index] = True
        ...
        for itemName in reversed(editorItems):
            ...
            proxyItem = proxy.getProxy(item) if item else None
            if proxyItem and proxyItem.index() in selectedIndexLookout:
                selectedIndexLookout.pop(proxyItem.index(), None)
    ...
    def _restoreExpandedState(self, proxy):
        ...
        for child in proxies:
            # TODO: implement expanded state appropriately by using code in renderSetup/model/expandedState.py
            if utils.getExpandedState(child.model.thisMObject()):
...
class LookThroughWindow(MayaQWidgetDockableMixin, QWidget):
    ...
    def __init__(self):
        ...
        layout.setSpacing(viewsUtils.dpiScale(2))
    ...
    def lookThroughLight(self, light):
        ...
        if light and self.panel:
            self.setWindowTitle(maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kLookingThrough'] + light)
            cmd = "lookThroughModelPanelClipped(\"" + light + "\", \"" + self.panel + "\", 0.001, 1000)"
            mel.eval(cmd)
        else:
            self.setWindowTitle(maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kSelectTheLight'])
...
class LightEditorCentralWidget(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.exportGroup = action.Action(kExport, self.fileMenu)
        self.importGroup = action.Action(kImport, self.fileMenu)
        self.exportGroup.triggered.connect(_exportLightGroups)
        self.importGroup.triggered.connect(_importLightGroupSchema)
        self.fileMenu.addAction(self.exportGroup)
        self.fileMenu.addAction(self.importGroup)
        ...
        self._callbackIds = [
            ...
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterNew, self._onAfterNewOrOpenCB),
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kBeforeOpen, self._onBeforeSceneChangeCB),
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterOpen, self._onAfterNewOrOpenCB),
        ...
        # Notification of render layer name change 
        self.renderLayerNameChangeCbId = [OpenMaya.MNodeMessage.addNameChangedCallback(OpenMaya.MObject.kNullObj, self._onRenderLayerNameChangeCB)]
        ...
        self.scriptJobs.append(cmds.scriptJob(event=["renderLayerChange", self.updateLayerList]))

    @staticmethod
    def _removeAllWarningsAndImportedStatus():
        editorModel.instance().removeAllWarningsAndImportedStatus()
    
    def aboutToDelete(self):
        """Cleanup method to be called immediately before the object is deleted."""
        OpenMaya.MMessage.removeCallbacks(self._callbackIds)
        OpenMaya.MMessage.removeCallbacks(self.renderLayerNameChangeCbId)

        self._callbackIds = []
        self.renderLayerNameChangeCbId = []
    ...
    def renderSetupAdded(self):
        self._addLayerObserver()
        if self.renderLayerNameChangeCbId:
            OpenMaya.MMessage.removeCallbacks(self.renderLayerNameChangeCbId)
            self.renderLayerNameChangeCbId = []
        # Notifies the drop down menu that a render layer has been renamed
        self.renderLayerNameChangeCbId = [OpenMaya.MNodeMessage.addNameChangedCallback(OpenMaya.MObject.kNullObj, self._onRenderLayerNameChangeCB)]

    def updateLayerList(self):
        ''' updateLayerList is called when a layer is added or removed to update the dropdown menu list.'''
        oldState = self.renderLayerDropDown.blockSignals(True) # to prevent switchRenderLayer from being called
        self.renderLayerDropDown.clear()
        self.renderLayerDropDown.addItem(kDefaultLayerText)
        for layer in renderSetup.instance().getRenderLayers():
            self.renderLayerDropDown.addItem(layer.name())
        self.updateSelectedLayerText()
        self.renderLayerDropDown.blockSignals(oldState)

    def updateSelectedLayerText(self):
        # Returns the selected text for the drop down menu
        text = kDefaultLayerText
        ...
        if visibleRenderLayer != defaultRenderLayer:
            text = visibleRenderLayer.name()
        self.renderLayerDropDown.setCurrentIndex(self.renderLayerDropDown.findText(text))
    ...
    def _onRenderLayerChangeCB(self):
        # When user changes layer via render setup the drop down menu should reflect the change
        self.updateSelectedLayerText()

    def _onRenderLayerNameChangeCB(self, obj, oldName, clientData):
        # If obj is a render layer, update the drop down list to reflect the name change
        if obj.apiType() == OpenMaya.MFn.kRenderLayer:
            self.updateLayerList()

    def switchRenderLayer(self, value):
        # When the user selects a layer from the drop down menu, render setup should switch to that layer
        # If value is 0 we are switching to the default render layer
        if value == 0:
            selectedLayer = renderSetup.instance().getDefaultRenderLayer()
        else:
            layers = renderSetup.instance().getRenderLayers()
            value -= 1 # getRenderLayers does not include the default render layer (scene)
            selectedLayer = layers[value]
        renderSetup.instance().switchToLayer(selectedLayer)
    ...
    def _createToolBar(self):
        ...
        for lightType in mayaLights:
            ...
            toolTip = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kCreateaNew1' ] + lightType
        ...
        for lightType in pluginLights:
            ...
            button = RenderSetupButton(self, icon, self.BUTTON_SIZE)
            button.clicked.connect( self.getLightCreator(lightType) )
            toolTip = maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kCreateaNew2' ] + lightType
            button.setToolTip(toolTip)
        ...
        # Add the drop down menu that displays all the layers
        self.renderLayerDropDown = QComboBox()
        self.renderLayerDropDown.setFixedWidth(viewsUtils.dpiScale(130))
        self.renderLayerDropDown.setToolTip(maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kRenderLayerDropDownTooltip' ])
        self.renderLayerDropDown.addItem(kDefaultLayerText)
        for layer in renderSetup.instance().getRenderLayers():
            self.renderLayerDropDown.addItem(layer.name())
        self.renderLayerDropDown.currentIndexChanged.connect(self.switchRenderLayer)
        leftToolbarLayout.addWidget(self.renderLayerDropDown)
        ...
        icon = viewsUtils.createIcon(":/LM_create_LEgroup.png")
        button = RenderSetupButton(self, icon, self.BUTTON_SIZE)
        button.setToolTip(maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kCreateaNewGroup' ])
        ...
        icon = viewsUtils.createIcon(":/RS_accept_update.png")
        button = RenderSetupButton(self, icon, self.BUTTON_SIZE)
        button.setToolTip(kAcceptImport)
        button.clicked.connect(LightEditorCentralWidget._removeAllWarningsAndImportedStatus)
        rightToolbarLayout.addWidget(button)

        icon = viewsUtils.createIcon(":/attributes.png")
        button = RenderSetupButton(self, icon, self.BUTTON_SIZE)
        ...
        layout = QHBoxLayout()
        layout.setContentsMargins(viewsUtils.dpiScale(5), 0, viewsUtils.dpiScale(5), 0)
    ...
    def _createLabels(self):
        ...
        label = QLabel()
        label.setText(maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kName' ])
        label.setMinimumWidth(viewsUtils.dpiScale(180))
        rightLayout.addWidget(label)

        lightAttributes = typeMgr.getAllAttributes()
        for attrLabel, _ in list(lightAttributes.items()):
            label = QLabel()
            label.setText(attrLabel)
            label.setMinimumWidth(viewsUtils.dpiScale(85))
            rightLayout.addWidget(label)
    ...
    def _onAfterNewOrOpenCB(self, *args, **kwargs):
        # Similar to _onAfterSceneChangeCB, but resetting import light group meta data and property editor's pinned item list.
        global _propertyEditorWindow
        editorModel.LightEditorImportExportHelper.resetSnapshot()
        propertyEditor.PropertyEditor.pinnedItemsLightEditor = {}
        if _propertyEditorWindow:
            _propertyEditorWindow.triggerRebuild()
        self._onAfterSceneChangeCB(args, kwargs)
    ...
    def _onBeforeUnloadReferenceCB(self, referenceNode, resolvedRefPath, clientData):
        ...
        if editorModel.hasInstance():
            ...
            for node in fnRef.nodes():
                ...
                item = LightEditor.findEditorItem(node)
                if item:
    ...
    def _onMayaNodeAddedCB(self, obj, clientData):
        ...
        if typeMgr.isValidLightShapeObject(obj) and not editorModel.LightEditor.findEditorItem(obj):
    ...
    def _onDgSelectionChangedCB(self, clientData):
        ...
        for i in range(selectionList.length()):
            ...
            obj = selectionList.getDependNode(i)
            fn = OpenMaya.MFnDependencyNode(obj)
            item = fn.userNode() if isinstance(fn.userNode(), itemModel.LightItemBase) else editorModel.LightEditor.findEditorItem(obj)
            if item:
    ...
    def _snapToSelected(self):
        ...
        if len(names) == 0:
            OpenMaya.MGlobal.displayInfo(maya.stringTable['y_maya_app_renderSetup_lightEditor_views_editorUI.kSnapToObjectNothingSelected'])
            return
...
class LightEditorWindow(MayaQWidgetDockableMixin, QWidget):
    ...
    def __init__(self):
        ...
        self.resize(viewsUtils.dpiScale(self.STARTING_SIZE))
...
def createLightEditorWindow(restore=False):
    ...
    if restore:
        ...
        OpenMayaUI.MQtUtil.addWidgetToMayaLayout(int(mixinPtr), int(parent))
    else:
        size = LightEditorWindow.STARTING_SIZE
        _lightEditorWindow.show(dockable=True, retain=False, width=size.width(), widthSizingProperty='free', height=size.height(), x=250, y=200, plugins='renderSetup.py', 
...
def createPropertyEditorWindow(restore=False):
    ...
    if restore:
        ...
        _propertyEditorWindow.show(dockable=True, retain=False, area='right', width=size.width(), widthSizingProperty='free', height=size.height(), controls=requiredControl, 
