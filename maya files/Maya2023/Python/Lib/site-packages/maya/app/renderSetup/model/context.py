...
def _getContainersBefore(pivot):
    '''
    Returns all the collections before a given node pivot.
    (in application order)
    '''
    from maya.app.renderSetup.model.container import Container
...
def getSelectors(collections):
    ...
    return (c.getSelector() for c in collections if c.hasSelector() and c.getSelectorType())
...
def getContainers(layers):
    '''Returns all the non-blacklisted containers from the layers iterable.'''
    for layer in layers:
        for c in utils.getContainersRecursive(layer):
            if c.name() not in BlackList.names():
                yield c
...
class PivotGuard(object):
    ...
    def enter(self):
        self.containers = [c for c in _getContainersBefore(self.pivot) \
                            if c.name() not in BlackList.names()]
        BlackList.add((c.name() for c in self.containers))
        for collection in self.containers:
            if collection.getSelector():
                collection.getSelector().deactivate()

    def exit(self):
        BlackList.remove((c.name() for c in self.containers))
        for collection in self.containers:
            if collection.getSelector():
...
class PivotContext(StackContext):
    ...
    def conclude(self):
        if self.guard is not None:
            ...
            cols = [c for c in (s.owner() for s in PivotContext.changed or []) if c.getRenderLayer() == self.layer] # pylint: disable=not-an-iterable
            PivotContext.changed = None
            pivot =  reduce(lambda out, x: x if x < out else out,
                            cols, next(iter(cols), None))
            self.guard.exit()
...
class UpdateLayerContext(PivotContext):
    ...
    def initiate(self):
        ...
        layers = [l for l in getRenderLayers() if l is not self.element]
        for s in getSelectors(getCollections(layers)):
            s.deactivate()

    def conclude(self):
        # reactivate all selectors
        for s in getSelectors(getContainers(getRenderLayers())):
            s.activate()
...
##############################################################################
#
# Container contexts makes sure that container's selector stop
...
class ApplyContainerContext(PivotContext):
    ...
    def __enter__(self):
        super(ApplyContainerContext,self).__enter__()
    ...
    def initiate(self):
        super(ApplyContainerContext,self).initiate()

    def conclude(self):
        if self.element.getSelector():
            self.element.getSelector().activate()
            for c in utils.getContainersRecursive(self.element):
                c.getSelector().activate()
        super(ApplyContainerContext,self).conclude()

def applyContainer(f):
    @stateGuards()
    def applyContainerWrapper(*args, **kwargs):
        with ApplyContainerContext(args[0]):
            f(*args, **kwargs)
    return applyContainerWrapper
...
def unapplyContainer(f):
    @stateGuards()
    def unapplyContainerWrapper(*args, **kwargs):
        with UnapplyContainerContext(args[0]):
            f(*args, **kwargs)
    return unapplyContainerWrapper
...
def updateContainer(f):
    @stateGuards()
    def updateContainerWrapper(*args, **kwargs):
        with UpdateContainerContext(args[0]):
            f(*args, **kwargs)
    return updateContainerWrapper
