...
kSet           = maya.stringTable['y_maya_app_renderSetup_lightEditor_model_item.kSet'           ]
...
class LightItemBase(childNode.TreeOrderedItem, childNode.ChildNode):
    ...
    kWarningMessage = "Importing light groups has changed the visibility of this light"
    ...
    # Isolate behavior
    #
    # As of MAYA-95548 this is no longer used, but we're
    # keeping the attributes for backwards compatibility
    #
    ...
    def setSelfEnabled(self, value):
        if value != self.isSelfEnabled():
            with undo.NotifyCtxMgr(kSet % (self.name(), 'enabled', value), self.itemChanged):
                cmds.setAttr(self.getSelfEnabledPlug().name(), value)
    ...
    def setIsolateSelected(self, value):
        if value != self.isIsolateSelected():
            with undo.NotifyCtxMgr(kSet % (self.name(), 'isolateSelected', value), self.itemChanged):
                cmds.setAttr(self.getIsolatePlug().name(), value)
    ...
    def compute(self, plug, dataBlock):
        if plug == self.enabled:
            ...
            #
            soloOK = dataBlock.inputValue(self.layerNumIsolatedChildren).asInt()==0 or \
                self.isIsolateSelected(dataBlock) or \
                self.hasIsolatedAncestors(dataBlock) or \
                self.hasIsolatedChildren(dataBlock)

            value = computeEnabled.computeEnabled(self, dataBlock)
            computeEnabled.setEnabledOutput(self, dataBlock, value and soloOK)
    ...
    def getWarning(self):
        return self.kWarningMessage if self._enabledWarning else None

    def setWarning(self, enabled=True):
        self._enabledWarning = enabled
