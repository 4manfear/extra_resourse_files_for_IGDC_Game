...
kWrongArgTypeCtxMgr = maya.stringTable['y_maya_app_renderSetup_model_utils.kWrongArgTypeCtxMgr'  ]
...
def deleteNode(node):
    ...
    cmds.delete(node if isinstance(node, string_type) else node.name())
...
def getCollectionsRecursive(parent):
    ...
    for child in parent.getContainers():
        if child.hasSelector():
            yield child
        for c in getCollectionsRecursive(child):
            yield c

def getGroupsRecursive(parent):
    """ Return a generator of all groups in parent.
        Note: It is recursive, so all levels of children are searched through"""
    for child in parent.getGroups():
        yield child
        for g in getGroupsRecursive(child):
            yield g

def getContainersRecursive(parent):
    """ Return a generator of all containers in parent.
        Note: It is recursive, so all levels of children are searched through"""
    for child in parent.getContainers():
        yield child
        for c in getContainersRecursive(child):
            yield c
...
def findSetMembersNamesRecursively(members):
    """ Return a generator consisting of all non-set members found in the sets
        passed as arguments recursively. Any member found in the members list
        that is not a set will be yielded. If it is a set, we yield all non-set
        members inside of it recursively.
    """
    for member in members:
        if not cmds.objectType(member, isType='objectSet'):
            path = commonUtils.nameToDagPath(member)
            if path is not None:
                yield path.fullPathName()
            else:
                yield member
        else:
            # Note: The nodesOnly flag is set, because it is sufficient to
            # return the dagNodes in the set (using the flag nodesOnly) instead
            # of all dag paths (there could be many for one given node),
            # because legacy render layer membership does not take dagPaths
            # into account. Even if it is given dagPaths,
            # it will add the corresponding nodes to its membership.
            children = cmds.sets(member, nodesOnly=True, q=True) or []
            for name in findSetMembersNamesRecursively(children):
                yield name

def findSetMembersDagPathsRecursively(members):
    for member in members:
        if not cmds.objectType(member, isType='objectSet'):
            path = commonUtils.nameToDagPath(member)
            if path is not None:
            ...
            children = cmds.sets(member, nodesOnly=True, q=True) or []
            for dagPath in findSetMembersDagPathsRecursively(children):
                yield dagPath

def findValidNestedSetsRecursively(setName):
    yield setName
    for name in cmds.sets(setName, nodesOnly=True, q=True) or []:
        if cmds.objectType(name, isType='objectSet'):
            for childName in findValidNestedSetsRecursively(name):
                yield childName
# ===========================================================================
