...
kGlobalTemplatePathInvalid = maya.stringTable['y_maya_app_renderSetup_model_renderSetupPreferences.kGlobalTemplatePathInvalid' ]

# Error message to display when the global presets path does not exist
kGlobalPresetsPathInvalid = maya.stringTable['y_maya_app_renderSetup_model_renderSetupPreferences.kGlobalPresetsPathInvalid' ]
...
def _getUserDirectory(userDirectoryOptionVar, defaultUserDirectoryName):
    ...
    if not cmds.optionVar(exists=userDirectoryOptionVar):
        dstPath = os_path.join(os_getenv('MAYA_APP_DIR'), defaultUserDirectoryName)
        if not os_path.exists(dstPath):
    ...
    if not cmds.optionVar(exists=userDirectoryOptionVar+'_contents'):
        fromDirectory = os_path.join(os_path.join(os_getenv('MAYA_LOCATION'), 'presets'), defaultUserDirectoryName)
        if os_path.exists(fromDirectory):
...
class BaseSetting(object):
    ...
    def hasEnvVar(cls):
        ...
        return os_getenv(cls.kEnvVar) != None
    ...
    def getEnvVar(cls):
        ...
        if not cls.hasEnvVar():
            return None
        from os import getenv as os_getenv
        return int(os_getenv(cls.kEnvVar)) == 1
    ...
    def setEnvVar(cls, val):
        if val is None:
            ...
            os_environ[cls.kEnvVar] = "1" if val else "0"

    @classmethod
    def unsetEnvVar(cls):
        if cls.hasEnvVar():
            from os import environ as os_environ
            del os_environ[cls.kEnvVar]
...
class IncludeAllLightsSettingContextManager(object):
...
class ShowWarningIcons(BaseSetting):
    """ Accessor and mutator static/class methods for
        Show Warning Icons setting.
    """
    kEnvVar = "MAYA_RENDER_SETUP_ENABLE_WARNING_ICONS"
    kOptionVar = 'renderSetup_showWarningIcons'
    kDefault = True
    ...
    @includeAllLightsSettingDecorator
    def setEnvVar(cls, val):
        super(ShowWarningIcons, cls).setEnvVar(val)

    @classmethod
    @includeAllLightsSettingDecorator
    def unsetEnvVar(cls):
        super(ShowWarningIcons, cls).unsetEnvVar()

    @classmethod
    @includeAllLightsSettingDecorator
    def setOptionVar(cls, val):
        super(ShowWarningIcons, cls).setOptionVar(val)
        cls._notifyOptionChangeObserver()
        
    @classmethod
    @includeAllLightsSettingDecorator
    def unsetOptionVar(cls):
        super(ShowWarningIcons, cls).unsetOptionVar()

class DisplayRSNodesSettingContextManager(object):
...
def initialize():
    ...
    #
    from os import getenv as os_getenv
    defaultGlobalTemplatePath = os_getenv('MAYA_RENDER_SETUP_GLOBAL_TEMPLATE_PATH')
    from os import path as os_path
    if defaultGlobalTemplatePath is not None:
        if os_path.exists(defaultGlobalTemplatePath):
    ...
    #
    defaultGlobalPresetsPath = os_getenv('MAYA_RENDER_SETUP_GLOBAL_PRESETS_PATH')
    if defaultGlobalPresetsPath is not None:
        if os_path.exists(defaultGlobalPresetsPath):
