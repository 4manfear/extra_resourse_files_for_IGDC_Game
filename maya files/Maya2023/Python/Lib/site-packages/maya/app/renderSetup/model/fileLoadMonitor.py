...
kErrorSwitchToRenderLayer = maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kErrorSwitchToRenderLayer' ]
kErrorSwitchToRenderSetup = maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kErrorSwitchToRenderSetup' ]
kWarningVisibleRenderLayer = maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kWarningVisibleRenderLayer' ]
kErrorCombiningLegacyToNew = maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kErrorCombiningLegacyToNew' ]
kErrorCombiningNewToLegacy = maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kErrorCombiningNewToLegacy' ]
kLightVisibilityChanged = maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kLightVisibilityChangedStr' ]
...
def _cleanupLegacyLightsCollections():
	# MAYA-96047: Clean-up any legacy state in lights collections.
	# Using 'selfEnable' on lightCollection and lightChildCollection nodes was
	# deprecated as of Maya 2019. If an old scene is loaded with selfEnabled set
	# to False on these collections we must update the scene, setting this to True,
	# and instead setting selfEnabled False on all override children.
	# We also display a warning about the fact that lights included in these
	# collections are now visible when they might have been invisible before.
	lightsCollectionNames = cmds.ls(type="lightsCollection")
	for lightsColName in lightsCollectionNames:
		lightsCol = utils.nameToUserNode(lightsColName)
		parentDisabled = not lightsCol.isSelfEnabled()
		lightsCol.setSelfEnabled(True)
		for childCol in lightsCol.getChildren():
			childDisabled = not childCol.isSelfEnabled()
			childCol.setSelfEnabled(True)
			if parentDisabled or childDisabled:
				# Lights were not visible in scene so disabled them
				# to preserve scene visibility state.
				nodes = childCol.getSelector().nodes()
				for node in nodes:
					lightItem = lightEditor.instance().findEditorItem(node)
					if lightItem and lightItem.isSelfEnabled():
						# Print a warning about the fact that this light might have been invisible
						# before but is now visible since collection disabling is not supported now.
						cmds.warning(kLightVisibilityChanged % lightItem.getLightName(True))
				# Overrides were not applied in scene so disabled them
				# to preserve scene override state
				for ovr in childCol.getChildren():
					ovr.setSelfEnabled(False)
...
def onReadEnd( _ ):
	...
	if mel.eval('getenv("MAYA_STARTED_FROM_RENDER")') and cmds.about(batch=True) and isNewScene:
			...
			if renderSetup.hasInstance():
				...
				if errorMsg is not None: 
					...
					if not defaultLayer.isVisible(): # MAYA-83988
						errorMsg += maya.stringTable['y_maya_app_renderSetup_model_fileLoadMonitor.kFileSwitchToDefaultRenderLayer' ]                    
	...
	# MAYA-96047: Clean-up any legacy state in lights collections.
	_cleanupLegacyLightsCollections()
