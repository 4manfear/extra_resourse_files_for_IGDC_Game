...
class ApplyOverrideShadingNodeOverride(OpenMayaRender.MPxShadingNodeOverride):
    ...
    def __init__(self, obj):
        ...
        if fragmentMgr is not None and not fragmentMgr.hasFragment(self._fragmentName):
            ...
            fragmentMgr.addShadeFragmentFromBuffer(fragmentBody.encode('ASCII'), False)
...
def getDrawdbClassification(typeid):
    for classif, typeids in list(_classifToTypeIds.items()):
...
def initialize():
    for classif, creator in list(_classifToCreator.items()):
        try:
...
def uninitialize():
    for classif in list(_classifToCreator.keys()):
        try:
