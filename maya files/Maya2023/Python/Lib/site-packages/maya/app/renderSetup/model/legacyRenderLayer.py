...
def delete(legacyRenderLayerName):
    with profiler.ProfilerMgr('legacyRenderLayer::delete'):
        ...
        # Wrap the delete command with catchQuiet so that any error produced does
        # not get echoed to the command output. Note that an error will be 
        # produced when attempting to clean up (ie. delete) default render layers
        # from a referenced file. Referenced objects are read-only and cannot be
        # deleted. Deleting read-only nodes used to fail silently. They don't now,
        # but we don't want the error to appear when opening a scene as a reference.
        # See MAYA-106094 for more information.
        #
        deleteCmd = 'catchQuiet( delete("' + legacyRenderLayerName + '") )'
        mel.eval(deleteCmd)
