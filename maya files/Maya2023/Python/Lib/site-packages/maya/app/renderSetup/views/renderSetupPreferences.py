...
def _loadPreset(preset, basePath):
    ...
    from os import path as os_path
    with open(os_path.join(basePath, presetFile)) as file:
        try:
            renderSettings.decode(json.load(file))
        except:
            cmds.error(kInvalidPresetFound % os_path.join(basePath, presetFile))
...
def deleteUserPreset(preset, warn=True):
    ...
    if (not warn or delete == confirmResponse):
        ...
        os_remove(os_path.join(basePath, presetFile))

# Returns the list of presets in the specified base path.
def _getPresets(renderer, basePath):
    presets = []
    from os import path as os_path
    if basePath is not None and os_path.exists(basePath):
        from os import listdir as os_listdir
        presetFiles = [f for f in os_listdir(basePath) if os_path.isfile(os_path.join(basePath, f)) and os_path.splitext(f)[1] == '.'+prefs.getFileExtension()]
        for presetFile in presetFiles:
            with open(os_path.join(basePath, presetFile)) as file:
                try:
                    rs = json.load(file)
                    if renderer in rs:
                        presets.append(os_path.splitext(presetFile)[0])
                except:
                    cmds.warning(kInvalidPresetFound % os_path.join(basePath, presetFile))
    return presets
