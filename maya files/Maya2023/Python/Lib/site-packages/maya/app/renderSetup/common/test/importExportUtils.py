...
def exportFile(filename, includeSceneSettings=True):
    with open(filename, "w+") as file:
        json.dump(renderSetup.instance().encode(None, includeSceneSettings), fp=file, indent=2, sort_keys=True, separators=(',', ': '))
...
def readFile(file):
    ...
    NOTE: As this is intended for reading JSON files, trailing \r are removed.
    """
    with open(file, 'r') as f:
        data = f.read().split('\n')
        # Strip data of windows newline characters
