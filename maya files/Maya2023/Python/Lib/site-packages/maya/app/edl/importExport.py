...
def _getValidClipObjectName( clipName, isVideo ):
	...
	validName = re.sub(r'[\W]+', '', validName)

	if len(validName) == 0:
		validName = 'shot' if isVideo else 'audio'
...
class ImportExport(OpenMayaMPx.MPxCommand):
	...
	def _setupLoggingHandlers(self):
		...
		fileBaseName = os_path.splitext(self.translator.fileName)[0]
		self.errorFileName = (fileBaseName + "_error.log")

		if os_path.exists(self.errorFileName):
			os_remove(self.errorFileName)
	...
	def _closeLoggingHandlers(self):
		...
		from os import path as os_path, remove as os_remove
		if os_path.exists(self.errorFileName):
			fileSize = os_path.getsize(self.errorFileName)
			os_remove(self.errorFileName)
...
class Importer(ImportExport):
	...
	def _applyStartFrameOverride(self):
		...
		for clip in list(self.clip_dict.values()):
			if clip is None:
	...
	def _checkFrameRates(self, elem, elem_info):
		...
		fps = [k for k, v in list(mayaFrameRates.items()) if v == fps][0]
	...
	def _readClip(self, clip, isVideo, start, end, in_adjustment ):
		...
		try:
			if type(clip_name) == type(b''):
				clip_name=clip_name.decode('utf-8')
			assert type(clip_name) == type('')
		except:
...
class Exporter(ImportExport):
	...
	def doIt(self, fileName):
		...
		if len(shots) == 0:
			cmds.warning(maya.stringTable['y_maya_app_edl_importExport.kEDLNoShot'])
			return
		...
		from os import path as os_path
		sceneName = os_path.basename(cmds.file(q=1, loc=1))
		fps = cmds.currentUnit(q=1, time=1)

		self.seq_info = { "name": os_path.splitext(sceneName)[0] }
		self.seq_info["duration"] = (lastSeqFrame + 1 - firstSeqFrame)
		self.seq_info["framerate"] = [k for k, v in list(mayaFrameRates.items()) if v == fps][0]
		...
		# write video tracks
		trackNumbers = sorted(self.shotNodes.keys())
		...
		# write audio tracks
		trackNumbers = sorted(self.audioNodes.keys())
		for trackNum in trackNumbers:
	...
	def _writeTrack(self, seq_elem, isVideo, nodeList, trackNumber):
		...
		sortedNodeList = sorted(nodeList,
			key=functools.cmp_to_key(videoClipCompare if isVideo else audioClipCompare)
		)
