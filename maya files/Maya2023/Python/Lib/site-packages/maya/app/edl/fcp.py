...
class FCP(Translator):
	...
	def writeSequence(self, seq_info):
		...
		for k, v in list(seq_info.items()):
	...
	def writeTrack(self, seq_elem, track_info):
		...
		for k, v in list(track_info.items()):
			if k == "name":
	...
	def writeClip(self, track_elem, clip_info):
		...
		for k, v in list(clip_info.items()):
			if k == "id":
				clip_elem.set("id", v)
			else:
	...
	def writeFile(self, clip_elem, file_info):
		...
		for k, v in list(file_info.items()):
			elem = ET.SubElement(file_elem, k)
			if k == "pathurl":
				if not os_path.isabs(str(v)):
