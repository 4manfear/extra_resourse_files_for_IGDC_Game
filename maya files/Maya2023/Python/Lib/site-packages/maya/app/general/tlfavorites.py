...
def detachStore(lister):
    ...
    """
    for _, fs in list(_storesByFilename.items()):
...
class TLFavoriteStore(object):
    ...
    def get(self):
        ...
        if self._favorites is None:
            from os import path as os_path
            if os_path.exists(self._fname):
                try:                    
                    self._favorites = eval(open(self._fname, 'r').read())     
                except Exception as ex:
                    msg = maya.stringTable['y_maya_app_general_tlfavorites.kProblemReadingFavoritesFile' ]
