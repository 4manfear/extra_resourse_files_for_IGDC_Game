...
_dict_attributeToPaint_melToUI = {
    'color' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kColor'            ],
    'transparency' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kTransparency'     ],
    'incandescence' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kIncandescence'    ],
    'normalCamera' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kBumpMap'          ],
    'specularColor' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kSpecularColor'    ],
    'reflectivity' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kReflectivity'     ],
    'ambientColor' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kAmbient'          ],
    'diffuse' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kDiffuse'          ],
    'translucence' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kTranslucence'     ],
    'reflectedColor' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kReflectedColor'   ],
    'displacement' : maya.stringTable[ 'y_maya_app_general_art3dPaintGetPaintableAttr.kDisplacement'     ],
}
# Reverse lookup for dict_attrNameToLabelName
_dict_attributeToPaint_uiToMel = dict([(v, k) for k, v in list(_dict_attributeToPaint_melToUI.items())])

def attributeToPaint_uiToMel(value):
    from sys import version_info
    if version_info.major == 2:
        # In Python 2, the resource strings in art3dPaintGetPaintableAttr_res.py
        # are unicode, to match with them we need to convert the non-unicode 
        # string "value" to a unicoded one.
        from sys import getfilesystemencoding as sys_getfilesystemencoding
        encoding = sys_getfilesystemencoding()

        from builtins import str as futureStr
        value = futureStr(value, encoding)

    # Note that in Python 3 there is no difference between regular strings and 
    # unicode string. Hence we can skip the step above.
    return _dict_attributeToPaint_uiToMel.get(value)
