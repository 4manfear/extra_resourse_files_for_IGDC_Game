...
def registerServer(serverName, addrFamily):
    ...
    for port in range(50007,65535):
        try:
            ...
            continue
    if s is not None:
        raise RuntimeError(maya.stringTable['y_maya_app_general_ServerRegistryMMap.kCouldNotFindFreePort'])
    activeServers = getInstance()
    activeServers.addServer(serverName, sockAddr[1])
    return sockAddr


class ServerRegistryMMap(object):
    """
    ...
    def findServer(self, serverName):
        ...
        for i in range(32):
            if self.getBitmap() & (0x1 << i):
                ...
                if record[0].rstrip(b'\x00') == serverName:
                    return i
        return -1
    ...
    def addServer(self, serverName, port):
        ...
        if self.findServer(serverName) >= 0:
            msg = maya.stringTable['y_maya_app_general_ServerRegistryMMap.kCommandPortStillActive' ]
            raise RuntimeError(msg % serverName)
        i = self.firstFreeSlot()
        if i is None:
            # no free slots!
            msg = maya.stringTable['y_maya_app_general_ServerRegistryMMap.kNoMoreCommandPortsAvailible' ]
