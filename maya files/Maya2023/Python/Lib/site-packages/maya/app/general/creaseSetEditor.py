...
def _loadUIString(strId):
	try:
		return {
			'kNonMeshOrTransform': maya.stringTable['y_maya_app_general_creaseSetEditor.kNonMeshOrTransform' ],
			'kPleaseSelectMeshesOrCreaseSets': maya.stringTable['y_maya_app_general_creaseSetEditor.kPleaseSelectMeshesOrCreaseSets' ],
			'kComponentsMovedWarning': maya.stringTable['y_maya_app_general_creaseSetEditor.kComponentsMovedWarning' ],
			'kOkButton': maya.stringTable['y_maya_app_general_creaseSetEditor.kOkButton' ],
			'kCancelButton' :maya.stringTable['y_maya_app_general_creaseSetEditor.kCancelButton' ],
			'kInvalidCreaseEditorClass': maya.stringTable['y_maya_app_general_creaseSetEditor.kInvalidCreaseEditorClass' ],
		}[strId]
	except KeyError:
		return " "
...
def unbakeValuesIntoCreaseSets(meshes, name='creaseSet#'):
    ...
    with MayaUndoChunk('creaseSetFromMesh'): # group the following actions in this block into a single undo chunk
        ...
        newCreaseSets = []
        for val, elements in list(valueCompDictE.items()):
            newCreaseSets.append( newCreaseSet(name=name, elements=elements, creaseLevel=val) )
        for val, elements in list(valueCompDictV.items()):
            newCreaseSets.append( newCreaseSet(name=name, elements=elements, creaseLevel=val) )
...
def subdivideBaseMesh(meshes, level=1, adjustSmoothLevelDisplay=False):
    ...
    if (len(memberMeshTsNotSelected) != 0):
        maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kCreaseSetsHaveComponents' ]%memberMeshTsNotSelected)
        return []
...
def lookupColorValue(v, minvalue=0.0, maxvalue=1.0, colors=None):
    ...
    if minvalue >= maxvalue:
        logger.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kBadMinMaxValue' ])
        return colors[0]
...
def convertCreasesToVertValues(meshT):
    ...
    if edgeIds.length() > 0:
        ...
        creaseValsE = dict(list(zip(edgeIds, creaseData)))
        ...
        while not meshItE.isDone():
            if meshItE.index(0) in vertexColorDict:
            ...
            if meshItE.index(1) in vertexColorDict:
    ...
    if vertIds.length() > 0:
        creaseValsV = dict(list(zip(vertIds, creaseData)))
        ...
        while not meshItV.isDone():
            if meshItV.index() in vertexColorDict:
...
def calcCreaseValueColorSet(meshTs=None, minvalue=0.0, maxvalue=4.0, colorSetName='creaseValues'):
    ...
    for meshT in meshTs:
        ...
        for (i, (k, v)) in enumerate(vertexColorDict.items()):
...
def _findPixmapResource(filename):
    ...
    if filename.startswith(':'):  # it is a QRC resource (starts with ':', load it directly
        ...
        searchpaths = os_environ['XBMLANGPATH'].split(os_pathsep)
        for p in searchpaths:
            p = p.replace('%B','')  # Remove the trailing %B found in Linux paths
            from os import path as os_path
            fullpath = os_path.join(p, filename)
...
class CreaseSetTreeWidget(QTreeWidget):
    ...
    def __init__(self, parent=None, name=None):
        ...
        tmpHeaders[self.COLUMN_SETNAME]     = maya.stringTable['y_maya_app_general_creaseSetEditor.kSet' ]
        tmpHeaders[self.COLUMN_CREASEVALUE] = maya.stringTable['y_maya_app_general_creaseSetEditor.kCrease' ]
        tmpHeaders[self.COLUMN_NUMMEMBERS]  = maya.stringTable['y_maya_app_general_creaseSetEditor.kMembers' ]
        tmpHeaders[self.COLUMN_SELMEMBERS]  = maya.stringTable['y_maya_app_general_creaseSetEditor.kSelected' ]
        ...
        self.rgbColorIcons = {}            
        for k, v in list(wireframeColorIndexes.items()):
    ...
    def setItemValues(self, item=None, name=None, colorIndex=None, creaseLevel=None, members=None, numSelectedMembers=None, showSetMembers=False):
        ...
        '''
        if isinstance(item, type(None)):  # NOTE: Using isinstance() instead of item==None since PySide2 does not allow for that __eq__ operator
    ...
    def commitDataCB (self, item, column, oldValue, newValue):
        ...
        if column == self.COLUMN_SETNAME:
            ...
            try:
                ...
                logger.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kUnableToConvertFloat' ]%newValue)
    ...
    def contextMenuEvent(self, event):
        ...
        if len(creasesets) == 0:
            self.contextMenuActions = [
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextCreateSet' ], self.newCreaseSetCB, True)
                ]
        elif len(creasesets) == 1:
            self.contextMenuActions = [
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextAdd' ], self.addMembersCB, componentsSelected),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextRemove' ], self.removeMembersCB, componentsSelected),
                ('---', None, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextRemoveOne' ], self.deleteCreaseSetCB, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextSplitOne' ], self.splitCreaseSetCB, componentsSelected),
                ('---', None, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextMemberSel' ], self.selectMembersCB, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextNodeSelOne' ], self.selectSelectedSetNodesCB, True),
                ]
        else: # len(creasesets) > 1
            self.contextMenuActions = [
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextRemove2' ], self.removeMembersCB, componentsSelected),
                ('---', None, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextRemoveMultiple' ], self.deleteCreaseSetCB, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextSplitMultiple' ], self.splitCreaseSetCB, componentsSelected),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextMerge' ], self.mergeCreaseSetsCB, True),
                ('---', None, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextMemberSel2' ], self.selectMembersCB, True),
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextNodeSelMultiple' ], self.selectSelectedSetNodesCB, True),
                ]

        if _edgeSetColorSupported and len(creasesets) > 0:
            colorMenuItems = [
                # Set Color Override submenu
                (maya.stringTable['y_maya_app_general_creaseSetEditor.kCreaseSetColorMenu' ], [
                    ...
                    (maya.stringTable['y_maya_app_general_creaseSetEditor.kContextNoColor' ], lambda:self.setComponentColor(-1), True),
    ...
    def selectMembersCB(self):
        ...
        if len(highlightedSets) > 0:
            ...
            maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kPleaseSelectOneOrMore' ])
    ...
    def addMembersCB(self):
        ...
        if len(selObjs) > 0:
            ...
            if len(curSets) != 1:
                maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kSelectASingleCreaseSet' ])
                return
            setName = curSets[0]
    ...
    def mergeCreaseSetsCB(self):
        ...
        if len(selSetNames) < 2:
            maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kSelectAtLeastTwo' ])
            return False
    ...
    def splitCreaseSetCB(self):
        ...
        if (len(selMeshes) == 0) and (len(selComps) == 0):
            maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kSelectOneOrMoreMeshesToSplit' ])
            return []
        ...
        if len(selSetNames) == 0:
            maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kSelectACreaseSetToSplit' ])
            return []
        ...
        with MayaUndoChunk('splitCreaseSet'): # group the following actions in this block into a single undo chunk
            for srcSetName in selSetNames:  # loop over each set and create a new set if members match selected items
                ...
                if len(setItems) == 0:
                    maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kCreaseSetHasNoMembers' ]%srcSetName)
                    continue
        ...
        if len(newCreaseSets) == 0:
            maya.cmds.warning(maya.stringTable['y_maya_app_general_creaseSetEditor.kSelectedMeshedNotInCreaseSet' ]%srcSetName)
    ...
    def setCreaseLevelIncrementCB(self):
        ...
        result = maya.cmds.promptDialog(
            title=maya.stringTable['y_maya_app_general_creaseSetEditor.kCreaseSetIncrementTitle' ],
            message=maya.stringTable['y_maya_app_general_creaseSetEditor.kEnterCreaseLevelIncrement' ],
            style='float',
    ...
    def setDefaultCreaseSetNameCB(self):
        ...
        while not setDefaultCreaseSetNameDone:
            result = maya.cmds.promptDialog(
                title=maya.stringTable['y_maya_app_general_creaseSetEditor.kCreaseSetNameTitle' ],
                message=maya.stringTable['y_maya_app_general_creaseSetEditor.kEnterDefaultName' ],
            ...
            if result == okBut:
                ...
                if maya.mel.eval( u'containsMultibyte \"%s\"' % val ):
                    ...
                    try:
                        errorMsg = (maya.stringTable['y_maya_app_general_creaseSetEditor.kMultibyteDefaultCreaseSetNameError' ] % val)
    ...
    def updateHighlightedItems_execute(self):
        ...
        for k, v in list(dictContainingItems.items()):
...
# ==============
class PushValueOntoStack(object):
...
class CreaseSetEditor(QWidget):
    ...
    def __init__(self, parent=None, name=None, title=None):
        ...
        if title == None:
            title = maya.stringTable['y_maya_app_general_creaseSetEditor.kCreaseSetEditorTitle' ]
        ...
        actions = [
            (maya.stringTable['y_maya_app_general_creaseSetEditor.kNewCreaseSet'    ],  self.tree.newCreaseSetCB, ':/newLayerSelected.png'), # polyCreateCreaseSet.png #'subdivCrease.png'), #'subdivBaseMesh.png')
            (maya.stringTable['y_maya_app_general_creaseSetEditor.kAddMembers'      ],    self.tree.addMembersCB, ':/trackAdd.png'),
            (maya.stringTable['y_maya_app_general_creaseSetEditor.kRemoveMembers'   ], self.tree.removeMembersCB, ':/trackRemove.png'),
            (maya.stringTable['y_maya_app_general_creaseSetEditor.kSelectMembers'   ], self.tree.selectMembersCB, (':/subdivMirror.png', 'subdivMirror.png')[(maya.cmds.about(api=True)<201300)]), # icon stored as resource in 2013
            (maya.stringTable['y_maya_app_general_creaseSetEditor.kDeleteCreaseSet' ] , self.tree.deleteCreaseSetCB,(':/smallTrash.png'  , 'smallTrash.png'  )[(maya.cmds.about(api=True)<201300)]), # icon stored as resource in 2013
            ]
        ...
        # Edit menu
        menu = self.menubar.addMenu(maya.stringTable['y_maya_app_general_creaseSetEditor.kEditMenu' ])
        selectAction = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kEditSelectFromSel' ], self.tree.selectSetsWithSelectedMembersCB)
        menu.addSeparator()
        bakeAction   = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kEditBake' ], self.tree.bakeOutCreaseSetValuesCB)
        unbakeAction = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kEditUnBake' ], self.tree.unbakeValuesIntoCreaseSetsCB)
        menu.addSeparator()
        subdAction   = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kEditSubdivide' ], self.tree.subdivideBaseMeshCB)
        hardAction   = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kEditEdgeHardness' ], self.tree.hardenCreaseEdgesCB)
        ...
        # Display menu
        menu = self.menubar.addMenu(maya.stringTable['y_maya_app_general_creaseSetEditor.kDisplayMenu' ])
        optimizeDisplayAction = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kDisplayOptimal' ], self.tree.optimizeSubdLevelCB)
        defaultDisplayAction  = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kDisplayDefault' ], self.tree.unoptimizeSubdLevelCB)
        menu.addSeparator()
        updateColorAction     = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kDisplayUpdateColor' ], self.tree.calcCreaseValueColorSetCB)
        toggleColorAction     = addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kdisplayToggleColor' ], self.tree.toggleCreaseValueColorSetCB)
        ...
        # Settings menu
        menu = self.menubar.addMenu(maya.stringTable['y_maya_app_general_creaseSetEditor.kSettingsMenu' ])
        addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kSettingsIncr' ], self.tree.setCreaseLevelIncrementCB)
        addMenuItem(menu, maya.stringTable['y_maya_app_general_creaseSetEditor.kSettingsName' ], self.tree.setDefaultCreaseSetNameCB)
