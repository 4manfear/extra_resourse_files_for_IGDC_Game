...
def _splitPath(filePath):
	...
	dirName, baseName = os_path.split(filePath)
...
def findAllFilesForPattern(pattern, frameNumber):
	...
	result = []
	from os import path as os_path
	if dirName and baseName and os_path.exists(dirName):
		if frameNumber is not None:
			baseName = baseName.replace(_frameTag, "0*" + str(frameNumber))
		regex = _patternToRegex(baseName)
		from os import listdir as os_listdir
		result = [ ''.join((dirName, separator, f)) for f in os_listdir(dirName) if os_path.isfile(os_path.join(dirName, f)) and re.match(regex, f, flags=re.IGNORECASE) ]
