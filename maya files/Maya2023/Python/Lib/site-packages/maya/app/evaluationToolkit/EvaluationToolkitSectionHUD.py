...
from maya.plugin.evaluator.cache_preferences import CP_DBG
...
class EvaluationToolkitSectionHUD(EvaluationToolkitSection):
    ...
    def __init__(self, title, start_closed):
        ...
        with LayoutManager( layout_widget ):
            ...
            self.checkbox_hud_evaluation = cmds.checkBoxGrp( label=''
                                                           , label1=maya.stringTable['y_maya_app_evaluationToolkit_EvaluationToolkitSectionHUD.kEvaluationHUD' ]
                                                           , changeCommand=callback_tool(self, self.callback_evaluation_hud_changed)
                                                           )

            self.checkbox_hud_frame_rate = cmds.checkBoxGrp( label=''
                                                           , label1=maya.stringTable['y_maya_app_evaluationToolkit_EvaluationToolkitSectionHUD.kFrameRateHUD' ]
    ...
    def callback_update_plugin_state(tool, new_state):
        ...
        '''
        assert tool is not None
        CP_DBG.dbg( 'EvaluationToolkitSectionHUD.callback_update_plugin_state({})'.format(tool) )
    ...
    def ui_deleted(tool):
        ...
        '''
        assert tool is not None
        CP_DBG.dbg( 'EvaluationToolkitSectionHUD UI deleted' )
