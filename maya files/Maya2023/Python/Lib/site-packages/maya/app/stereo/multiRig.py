...
def swapDefault( oldDefault, newDefault ):
	...
	msg = maya.stringTable[ 'y_maya_app_stereo_multiRig.kSwapDefaultMsg'  ]
	msg = msg % (oldDefault, newDefault)
	
	_btns = [ maya.stringTable[ 'y_maya_app_stereo_multiRig.kSwapYes'  ],
			  maya.stringTable[ 'y_maya_app_stereo_multiRig.kSwapNo'   ],
			  maya.stringTable[ 'y_maya_app_stereo_multiRig.kSwapAlwaysYes'  ],
			  maya.stringTable[ 'y_maya_app_stereo_multiRig.kSwapAlwaysNo'   ] ]
	...
	result = cmds.confirmDialog( title=maya.stringTable['y_maya_app_stereo_multiRig.kReplace' ],
								 message=msg,
								 button = _btns,
								 defaultButton='Yes',
								 cancelButton='No' )
...
class NamingTemplate(object):
	...
	def addLayer( self ):
		...
		if next_val in defaults:
			template = defaults[next_val]

		self._resolveLayerAndAdd(copy.deepcopy(template))
		self.store()

g3RigSetup = ( maya.stringTable['y_maya_app_stereo_multiRig.kMultiStereoRig' ],
			   "shot",
...
class NamingTemplateManager(object):
	"""
	...
	def store(self):
		...
		for s in self._namingTemplates:
			cmds.optionVar( category="Stereo Camera", sva=[NamingTemplateManager._NamingOV, s.stringify()] )
