	...
	for i in items:
		if i and i.type() == stereoCameraUILabel.UILabelGroup:
			...
			if i.postMenuCommand():
				...
				populateItems( parent, list(i.items()) )
		else:
...
def __addRigMenuItem( rigCmd, camCmd, rig ):
	...
	labelVal = maya.stringTable['y_maya_app_stereo_stereoCameraMenus.kRigContent' ] % {'arg1': rig}
...
def __addCameraSetsMenuItem( rigCmd, camCmd, rig ):
	...
	# Show Left, Center, Right first, then a divider, then
	# other cameras if any.
	labelVal = maya.stringTable['y_maya_app_stereo_stereoCameraMenus.kCameraSet' ] % {'arg1': rig}
	cmds.menuItem(label=labelVal, subMenu=True)
	rigList = [] 
...
def gatherCreateMenuUILabels():
	...
	menuTitle = maya.stringTable['y_maya_app_stereo_stereoCameraMenus.kNewStereoCamera' ]
	menuAnnot = maya.stringTable['y_maya_app_stereo_stereoCameraMenus.kNewStereoCameraAnnot' ]
	rigCount = len(rigs)
	uiLabels = [] 
	for rig in rigs:
		label = menuTitle;
