...
def __call(language, method, rigName, kwords ={}, cmd_args=[]):
	...
	if language == 'MEL':
		...
		if moduleName not in sys_modules:
			path = path.replace('.', '/')
			alteredPath = [sys_path[i]+'/'+path for i in range(len(sys_path))]
			try:
...
def performReloadChk( ):
	...
	directory = os_path.dirname( stereoCameraPath )
	contents = os_listdir( directory )
	for c in contents:
		if c.endswith( '.pyc' ) or c.endswith( '.py' ):
			module_name = c.split( '.' )[0]
			from sys import modules as sys_modules
			if module_name in sys_modules and module_name != __name__:
