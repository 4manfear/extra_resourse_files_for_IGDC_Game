...
def __createCamera( name, parent ):
  """
  Creates a left / right camera.  This is private method and
  ...
  cam = cmds.camera()[0]
  if parent: 
    cam = cmds.parent(cam, parent, relative=True)[0]
...
def __createAControlCamera():
  ...
  """
  return __createCamera( "Master", parent=None )
...
def createRig(basename='stereoCameraProxy', centerCam=None, camRoot=None, partOfMulti=False):
  ...
  left  = __createCamera( rootName + "Left", stereoRigRoot )
  leftShape = cmds.listRelatives( left, path=True, shapes=True )[0]
  right = __createCamera( rootName + "Right", stereoRigRoot )
