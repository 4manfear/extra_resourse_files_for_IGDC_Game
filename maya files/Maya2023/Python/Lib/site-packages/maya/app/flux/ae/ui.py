...
class LayoutInterface(object):
    ...
    def addMethod(obj, methodName):
        ...
        if sys_version_info[0] >= 3:
            obj.__dict__[methodName] = types.MethodType(method, obj)
        else:
    ...
    def getFullName(self):
        ...
        return omui.MQtUtil.fullName( long_type(unwrapInstance(self.parentableLayout)[0]) )

    def addWidget(self, widget, stretchFactor=0, alignment=0):
        if self.parentableLayout is None:
            raise TypeError('Class not parentable: %s' % self.__class__.__name__)

        # MAYA-98628 : Make sure we cast the alignment argument to the correct type. 
        # Failing to do so will raise an exception
        import PySide2.QtCore as core
        self.parentableLayout.addWidget(widget, stretchFactor, core.Qt.Alignment(alignment))
...
class Layout(qt.QWidget):
    ...
    def addWidgetToParent(self, parentName, widget):
        ...
        from sys import version_info as sys_version_info
        long_type = int if sys_version_info[0] >= 3 else long
        currentParent = wrapInstance(long_type(currentParent), qt.QWidget)
