...
qt_dpi = 1.0 if cmds.about(batch=True) else qt.QMainWindow().devicePixelRatio()
...
def createPixmap(imageName):
    ...
    if rawPixmap is None:
        ...
        return wrapInstance(int(rawPixmap), qt.QPixmap)
...
class DropWindow(qt.QDialog):
    ...
    def __init__(self, label='', title='', accepts=None, ui=None, minWidth=450, **kw):
        ...
        if isinstance(label, string_type):
            steps = [label]
            self.accepts = [accepts]
    ...
    def eventFilter(self, widget, event):
        if widget==self.listWidget.viewport() and event.type() == qt.QEvent.Type.Drop:
            ...
            movie = qt.QMovie(os_path.join(futils.getFluxResourcesFolder(), 'DragDrop.gif'))
...
class ListButtonDelegate(qt.QItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        toggleOffset = pix(20+self.parent().margins_toright) if self.parent().showToggleButton else pix(self.parent().margins_toright)
    ...
    def sizeHint(self, option, index):
        ...
        hint.setHeight(p.iconHeight + p.padding*2)

        h = hint.height()
        if index.row()==0:
            h += p.margins_in + p.margins_out
        elif index.row()==p.count()-1:
            h += p.margins_in
        else:
            h += p.margins_in*2
        hint.setHeight(h)
...
class ListButtonWidget(qt.QListWidget):
    ...
    '''
    def __init__(self, parent=None, rightIconWidth=20):
        ...
        self.rightIconWidth = pix(rightIconWidth)
        self.padding = pix(4)

        self.margins_toright = pix(4)
        self.margins_rightbtns = pix(0)
    ...
    def getButtonPressed(self, item, x):
        ...
        for i in range(len(item._leftButtons)):
            if x >= offset and x < offset + self.iconWidth:
                return item._leftButtons[i].name
            offset += self.iconWidth

        toggleOffset = pix(20)+self.margins_toright+self.margins_out if self.showToggleButton else self.margins_toright

        if x>=offset and x < self.rect().width()-toggleOffset - len(item._rightButtons)*(self.rightIconWidth+self.margins_rightbtns):
            return 'textField'

        offset = self.rect().width() - toggleOffset - len(item._rightButtons)*(self.rightIconWidth+self.margins_rightbtns)
        #print("getButtonPressed:"+str(offset)+", toggleoffset:"+str(toggleOffset)+","+str(self.rect().width()))

        for i in range(len(item._rightButtons)):
            if x >= offset and x < offset + self.rightIconWidth:
                return item._rightButtons[i].name
            offset += self.rightIconWidth+self.margins_rightbtns
...
def loadFluxIcons():
    from os import path as os_path
    fluxIconsPath = os_path.dirname(maya.app.flux.__file__)
    fluxIconsPath = os_path.join(fluxIconsPath, 'resources')
    fluxIconsPath = os_path.join(fluxIconsPath, 'icons')
    ...
    for name in fluxIconsNames:
        ...
        iconPath = os_path.join(fluxIconsPath, iconPath)
