import maya
maya.utils.loadStringResourcesForModule(__name__)

import maya.internal.common.ae.template as aetemplate
import maya.internal.nodes.weightgeometryfilter.ae_template as ae

class AETemplate(ae.AETemplate):
    def buildUI(self, nodeName):
        with aetemplate.Layout(self, maya.stringTable[ 'y_maya_internal_nodes_nonlinear_ae_template.kNonlinearDeformerAttributes'  ], collapse=False):
            self.addControls([
                # bend attributes
                "curvature",
                # twist attributes
                "startAngle",
                "endAngle",
                # squash attributes
                "factor",
                "expand",
                "maxExpandPos",
                "startSmoothness",
                "endSmoothness",
                # flare attributes
                "startFlareX",
                "startFlareZ",
                "endFlareX",
                "endFlareZ",
                "curve",
                # sine attributes
                "amplitude",
                "wavelength",
                "offset",
                "dropoff",
                # wave attributes
                "minRadius",
                "maxRadius",
                # bound attributes
                "lowBound",
                "highBound"
            ])

        self.suppress("deformerData")
        self.suppress("matrix")


        super(AETemplate, self).buildUI(nodeName)





# ===========================================================================
# Copyright 2022 Autodesk, Inc. All rights reserved.
#
# Use of this software is subject to the terms of the Autodesk license
# agreement provided at the time of installation or download, or which
# otherwise accompanies this software in either electronic or hard copy form.
# ===========================================================================
