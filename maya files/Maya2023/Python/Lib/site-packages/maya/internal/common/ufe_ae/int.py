'''
UFE instruction on how to create a control for an int attribute.
...
def updateUi(attr, uiControlName):
    ...
    cmds.intFieldGrp(uiControlName, e=True, **{"v" + str(i) : v for i, v in enumerate(attrValues, 1)})
    ...
    for i in range(1, len(attrValues)+1):
        ...
        if cmds.intField(fieldControl, exists=True):
            cmds.intField(fieldControl, e=True, editable=not isLocked)
            if bgClr:
                cmds.intField(fieldControl, e=True, backgroundColor=bgClr)
...
def updateDataReader(*values, **kwargs):
    ...
    Args:
        *values: int field values (from 1 to 4).
    ...
    if len(values) == 1:
        return values[0]
    elif ufeAttr.type == ufe.Attribute.kInt3:
        return ufe.Vector3i(*values)
    else:
        return None

def replace(controlName, ufeAttr, attrUIName, changedCommand):
    updateUi(ufeAttr, controlName)
    cmds.intFieldGrp(controlName, e=True, label=attrUIName)
...
def create(ufeAttr, attrUIName, changedCommand):
    ...
    # When we have a single value, in order to use intSliderGrp we need to
    # be able to query min/max from UFE.
    uiControl = cmds.intFieldGrp(label=attrUIName, adj=0, nf=attrCount)
...
def attachCallbacks(ufeAttr, uiControl, changedCommand):
    ...
    cmds.intFieldGrp(uiControl, edit=True, changeCommand=cb, dragCommand=cb)
