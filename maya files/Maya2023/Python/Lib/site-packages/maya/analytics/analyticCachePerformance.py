...
class analyticCachePerformance(BaseAnalytic):
    ...
    """
    ANALYTIC_LABEL = maya.stringTable['y_maya_analytics_analyticCachePerformance.kAnalyticLabel' ]
    ANALYTIC_DESCRIPTION_SHORT = maya.stringTable['y_maya_analytics_analyticCachePerformance.kAnalyticDescriptionShort' ]

    def __init__(self):
        """
        Initialize the persistent class members
        """
        super(analyticCachePerformance, self).__init__()
    ...
    def run(self):
        ...
        with emModeManager() as em_mgr, PlaybackManager() as play_mgr, CacheEvaluatorManager() as cache_mgr:
            ...
            cache_mgr.discard_frames_out_of_range = False
            support_cache = True
            em_mode = em_mgr.mode
            # EM can be disabled by dynamics evaluator
            em_enable = cmds.evaluationManager(q=True, enabled=True)
            if em_mode == 'dg' or not em_enable:
                support_cache = False
            else:
                ...
                if em_mode == 'dg' or not em_enable:
                    ...
                    support_cache = not cache_mgr.safe_mode_triggered
            ...
            for configuration_to_run in self.configurations_to_run:
                self.debug( 'Running caching configuration "{}"'.format( configuration_to_run[KEY_NAME] ) )
                ...
                caching_results[KEY_CONFIGURATION_NAME] = configuration_to_run[KEY_NAME]
                ...
                # Rebuild foreground and background graph and scheduling, they are typically serial frames and will drastic affect the average FPS on scene will less frames to play
                em_mgr.rebuild( include_scheduling=True )
                play_mgr.maxTime = min_time
                # The first background frame actually needs to redo partitoning, since the regular playback FPS does not include that serial frame, we should not include it here neither
                ...
                play_mgr.maxTime = max_time # This command can trigger cache refill without forcing a foreground frame
                cache_available = cmds.cacheEvaluator( waitForCache=999 )
                ...
                try:
                    frame_range = list(cache_mgr.get_valid_frames()) # Cached Frames XX XX XX
                    caching_results[KEY_FRAMES_CACHED] = frame_range

                    caching_point_data = caching_results[KEY_CACHED_DATA]
                    # Cached node data - skip the title and frame data
                    ...
                    self.error( maya.stringTable['y_maya_analytics_analyticCachePerformance.kBadParse'].format(ex,'\n'.join(cache_info)) )

                # Evacuation time
                self.debug( '--- Evacuating the cache' )
                try:
                    prev_flush_sync = cache_mgr.flush_sync
                    cache_mgr.flush_sync = True
                    start_time = cmds.timerX()
                    cmds.cacheEvaluator( flushCache='destroy' )
                    caching_results[KEY_EVACUATION] = cmds.timerX( startTime=start_time )
                    cache_mgr.flush_sync = prev_flush_sync 
                except Exception as ex:
                    self.error( maya.stringTable['y_maya_analytics_analyticCachePerformance.kBadFlushTime'] )
