...
def maya_file_generator(root_path, skip=None, descend=True):
    ...
    def __is_path_excluded(path_to_check):
        ...
        if os_path.isfile(path_to_check):
            (_,file_ext) = os_path.splitext(path_to_check)
    ...
    for root_path in root_path_list:
        ...
        # If it is a directory walk and check all of its contents
            ...
            for root, dirs, files in os_walk(root_path, topdown=True):
                ...
                for name in (f for f in files if not __is_path_excluded(os_path.join(root,f))):
                    yield os_path.join(root, name)
...
def get_maya_files(generator):
    ...
    for full_path in generator:
        (the_dir,the_file) = os_path.split( full_path )
        if the_dir != last_dir:
