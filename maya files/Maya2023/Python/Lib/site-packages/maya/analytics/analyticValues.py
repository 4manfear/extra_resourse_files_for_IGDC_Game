...
kAnalyticLabel = maya.stringTable['y_maya_analytics_analyticValues.kAnalyticLabel' ]
kAnalyticDescriptionShort = maya.stringTable['y_maya_analytics_analyticValues.kAnalyticDescriptionShort' ]
...
class analyticValues(BaseAnalytic):
    ...
    def run(self):
        ...
        if not self.option(OPTION_DETAILS):
            for order, count in list(rotateOrders.items()):
                self._output_csv( ['transform', 'Rotate Order %s' % rotateOrderNames[order], count] )
            for limitType, count in list(limitTypes.items()):
        ...
        # if necessary.
        if not self.option(OPTION_DETAILS):
            for attr, count in list(incomingCounts.items()):
