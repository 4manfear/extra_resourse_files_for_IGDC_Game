...
class PatternWidget(object):
    ...
    def create_ui(self):
        ...
        checkbox_annotation = maya.stringTable['y_maya_analytics_Window.kEnablePatternAnnotation' ]
        pattern_annotation  = maya.stringTable['y_maya_analytics_Window.kEnterPatternAnnotation'  ]
        delete_annotation   = maya.stringTable['y_maya_analytics_Window.kDeletePatternAnnotation' ]
        ...
        with LayoutManager(self.__list_widget):
            if len(self.pattern_values) > 0:
                ...
                cmds.text( label=maya.stringTable['y_maya_analytics_Window.kClickToAddPattern' ], enable=False )
                cmds.text( label='' )

            cmds.symbolButton( image='RS_create_layer.png'
                             , command=callback_tool(self, partial(PatternWidget.on_add))
                             , annotation=maya.stringTable['y_maya_analytics_Window.kAddListElement' ]
                             )
            cmds.text( label='' )
            cmds.text( label='' )
...
class PathWidget(object):
    ...
    def __init__(self, parent_widget):
        ...
        self.file_choices = { FILE_MODE_FILES : maya.stringTable['y_maya_analytics_Window.kChooseInputFiles' ]
                            , FILE_MODE_DIR   : maya.stringTable['y_maya_analytics_Window.kChooseInputDir'   ]
                            }

        # If a location is set for analytic files use that, otherwise use the current workspace
        from os import environ as os_environ
        if 'MAYA_ANALYTIC_PATH' in os_environ:
            self.__last_path = os_environ['MAYA_ANALYTIC_PATH'].split(':')[0]
        else:
            workspace = cmds.workspace(fullName=True)
            from os import path as os_path
            self.__last_path = os_path.join(workspace, 'scenes')
    ...
    def create_ui(self):
        ...
        checkbox_annotation  = maya.stringTable['y_maya_analytics_Window.kEnablePathAnnotation' ]
        directory_annotation = maya.stringTable['y_maya_analytics_Window.kDirectoryAnnotation'  ]
        file_annotation      = maya.stringTable['y_maya_analytics_Window.kFileAnnotation'       ]
        delete_annotation    = maya.stringTable['y_maya_analytics_Window.kDeletePathAnnotation' ]
        ...
        with LayoutManager( self.__list_widget ):
            with LayoutManager( cmds.rowColumnLayout( numberOfColumns=1, columnAlign=[(1, 'left')]) ):
                self.__use_scene_widget = cmds.checkBox( value=self.use_current_scene
                             ...
                             , label=maya.stringTable['y_maya_analytics_Window.kUseCurrentScene' ]
                             , annotation=maya.stringTable['y_maya_analytics_Window.kUseCurrentSceneAnnotation' ]
                             )
            ...
            with LayoutManager( cmds.rowColumnLayout( numberOfColumns=3
                ...
                if len(self.path_values) > 0:
                    ...
                    cmds.text( label='' )
                    # Informative message on what to do when the list is empty.
                    cmds.text( label=maya.stringTable['y_maya_analytics_Window.kClickToAddPath' ], enable=False )
                    cmds.text( label='' )
            ...
            with LayoutManager( cmds.rowColumnLayout( numberOfColumns=2
                ...
                cmds.symbolButton( image='fileNew.png'
                                 ...
                                 , annotation=maya.stringTable['y_maya_analytics_Window.kAddFileToPath' ]
                                 )
                cmds.symbolButton( image='fileOpen.png'
                                 , command=callback_tool(self, partial(PathWidget.on_add, mode=FILE_MODE_DIR))
                                 , annotation=maya.stringTable['y_maya_analytics_Window.kAddDirectoryToPath' ]
                                 )
    ...
    def on_add(tool, mode):
        ...
        if mode == FILE_MODE_FILES:
            ok_caption = maya.stringTable['y_maya_analytics_Window.kChooseFileButton' ]
            maya_files = maya.stringTable['y_maya_analytics_Window.kChooseFileFormats' ]
            kwargs = { 'fileFilter' : maya_files }
        else:
            ok_caption = maya.stringTable['y_maya_analytics_Window.kChooseDirectoryButton' ]
            kwargs = {}

        last_path = tool.__last_path
        from os import path as os_path
        if not os_path.isdir(last_path):
            last_path = os_path.dirname( last_path )
        ...
        if result:
            ...
            for raw_path in result:
                chosen_path = os_path.normpath(raw_path)
                if os_path.isdir(chosen_path):
                    tool.__last_path = chosen_path
                else:
                    tool.__last_path = os_path.dirname( chosen_path )
...
class AnalyticsWindow(object):
    ...
    def __initialize_from_json(self, json_data):
        ...
        if 'analytics' in config:
            for analytic_name in list(self.analytic_checkboxes.keys()):
    ...
    def __as_json(self):
        ...
        json_data = {}
        analytics_to_run = []
        for analytic_name in list(self.analytic_list.keys()):
    ...
    def default_path():
        ...
        from os import environ as os_environ
        if 'MAYA_ANALYTIC_PATH' in os_environ:
            path = os_environ['MAYA_ANALYTIC_PATH'].split(':')[0]
        else:
            workspace = cmds.workspace(fullName=True)
            from os import path as os_path
            path = os_path.join(workspace, 'scenes')
        return path
    ...
    def save_configuration( tool ):
        ...
        from os import path as os_path
        path = os_path.dirname(tool.configuration_path)

        json_files = 'JSON Files (*.json);;All Files (*.*)'
        result = cmds.fileDialog2( caption=maya.stringTable['y_maya_analytics_Window.kCaptionSaveConfig' ]
                                 , fileMode=FILE_MODE_CREATE
                                 , okCaption=maya.stringTable['y_maya_analytics_Window.kOkSave' ]
        ...
        if result:
            assert len(result) == 1
            from os import path as os_path
            chosen_path = os_path.normpath(result[0])
            tool.configuration_path = chosen_path
            with open(chosen_path, 'w') as config_file:
                json.dump( tool.__as_json(), config_file, indent=4, separators=(',', ': ') )
    ...
    def load_configuration( tool ):
        ...
        json_files = 'JSON Files (*.json);;All Files (*.*)'
        result = cmds.fileDialog2( caption=maya.stringTable['y_maya_analytics_Window.kCaptionLoadConfig' ]
                                 , fileMode=FILE_MODE_FILE
                                 , okCaption=maya.stringTable['y_maya_analytics_Window.kOkLoadConfig' ]
                                 , startingDirectory=path
                                 , fileFilter=json_files
                                 , dialogStyle=2
                                 )
    ...
    def run_analytics( tool, operation ):
        ...
        analytics_to_run = []
        for analytic_name in list(tool.analytic_list.keys()):
...
'''.format( ANALYTICS_RUNNER.analytics
        ...
        if ANALYTICS_RUNNER.return_json and ANALYTICS_RESULTS is not None:
            print(json.dumps( ANALYTICS_RESULTS, indent=4, separators=(',', ': ') ))
    ...
    def on_select_static_analytics(tool):
        ...
        for analytic_name, checkbox in list(tool.analytic_checkboxes.items()):
    ...
    def on_select_results_path(tool):
        ...
        '''
        ok_caption = maya.stringTable['y_maya_analytics_Window.kChooseResultsPathOk' ]
        path_caption = maya.stringTable['y_maya_analytics_Window.kChooseResultsPath' ]
    ...
    def on_select_log_file(tool):
        ...
        Nothing is changed if the dialog is canceled.
        '''
        ok_caption = maya.stringTable['y_maya_analytics_Window.kChooseLogFileOk' ]
        path_caption = maya.stringTable['y_maya_analytics_Window.kChooseLogFile' ]

        from os import path as os_path
        if tool.log_file is None:
            last_path = tool.default_path()
        else:
            last_path = os_path.dirname(tool.log_file)
    ...
    def on_select_all_analytics(tool):
        ...
        for checkbox in list(tool.analytic_checkboxes.values()):
    ...
    def on_info(tool, analytic_name):
        ...
        '''
        window_title = maya.stringTable['y_maya_analytics_Window.kAnalyticInfo' ].format(analytic_name)
        window_message = ''
        try:
            for line in tool.analytic_list[analytic_name].ANALYTIC_DESCRIPTION_DETAILED:
                window_message += line
        except:
            window_message += maya.stringTable['y_maya_analytics_Window.kAnalyticInfoNotAvailable' ]
    ...
    def __build_analytics_window(self):
        ...
        if ANALYTICS_WINDOW is None:
            ANALYTICS_WINDOW = cmds.window( 'AnalyticsUI'
                                          , title=maya.stringTable['y_maya_analytics_Window.kAnalyticObjectsTitle' ]
                                          , iconName=maya.stringTable['y_maya_analytics_Window.kAnalyticObjectIconName' ]
                                          )
        ...
        with LayoutManager(self.__main_widget):
            with LayoutManager(cmds.columnLayout( adjustableColumn=True )):
                ...
                with LayoutManager(cmds.rowColumnLayout( numberOfColumns=5
                    ...
                    cmds.button( label=maya.stringTable['y_maya_analytics_Window.kRunAnalytics']
                               , annotation=maya.stringTable['y_maya_analytics_Window.kRunAnalyticsAnnotation']
                               , command=callback_tool(self, partial(AnalyticsWindow.run_analytics, operation=RUN_ANALYTICS)) )
                    cmds.button( label=maya.stringTable['y_maya_analytics_Window.kListAnalytics']
                               , annotation=maya.stringTable['y_maya_analytics_Window.kListAnalyticsAnnotation']
                               , command=callback_tool(self, partial(AnalyticsWindow.run_analytics, operation=LIST_ANALYTICS)) )
                    cmds.button( label=maya.stringTable['y_maya_analytics_Window.kShowAnalyticCommand']
                               , annotation=maya.stringTable['y_maya_analytics_Window.kShowCommandAnnotation']
                    ...
                    cmds.symbolButton( image='fileSave.png'
                                     ...
                                     , annotation=maya.stringTable['y_maya_analytics_Window.kFileSaveAnnotation' ]
                                     )
                    cmds.symbolButton( image='fileOpen.png'
                                     , command=callback_tool(self, partial(AnalyticsWindow.load_configuration))
                                     , annotation=maya.stringTable['y_maya_analytics_Window.kFileLoadAnnotation' ]
                                     )
                ...
                #
                with LayoutManager(cmds.frameLayout( label=maya.stringTable['y_maya_analytics_Window.kOptions' ], marginWidth=25, marginHeight=4 )):
                    with LayoutManager( cmds.rowColumnLayout( numberOfColumns=2
                                                            , columnAlign=[(1,'center'), (2,'center')]
                        ...
                        with LayoutManager( cmds.rowColumnLayout( numberOfColumns=4
                            ...
                            cmds.symbolButton( image='fileOpen.png'
                                             ...
                                             , annotation=maya.stringTable['y_maya_analytics_Window.kSelectResultsPath' ]
                                             )
                            ...
                            self.results_path_widget = cmds.textField(
                                                  text=results_path
                                                , width=TEXT_WIDTH
                                                , annotation=maya.stringTable['y_maya_analytics_Window.kResultsPathAnnotation' ]
                                                , enterCommand=callback_tool(self, partial(AnalyticsWindow.on_results_path_edit))
                                                )
                            cmds.text( label=maya.stringTable['y_maya_analytics_Window.kResultsPathLabel' ] )
                            #----------------------------------------
                            self.log_enabled_widget = cmds.checkBox( value=ANALYTICS_RUNNER.logger.debugging
                                         ...
                                         , annotation=maya.stringTable['y_maya_analytics_Window.kShowLogEnabledInfo' ]
                                         )
                            cmds.symbolButton( image='fileOpen.png'
                                             , command=callback_tool(self, partial(AnalyticsWindow.on_select_log_file))
                                             , annotation=maya.stringTable['y_maya_analytics_Window.kSelectLogFile' ]
                                             )
                            self.log_file_widget = cmds.textField(
                                                  text=ANALYTICS_RUNNER.logger.log_file_name
                                                , width=TEXT_WIDTH
                                                , annotation=maya.stringTable['y_maya_analytics_Window.kLogFileAnnotation' ]
                                                , enterCommand=callback_tool(self, partial(AnalyticsWindow.on_log_file_edit))
                                                )
                            cmds.text( label=maya.stringTable['y_maya_analytics_Window.kLogFileLabel' ] )
                ...
                #
                #
                with LayoutManager( cmds.frameLayout( label=maya.stringTable['y_maya_analytics_Window.kAnalytics' ] )):
                    ...
                    with LayoutManager( analytic_list_form ):
                        ...
                        with LayoutManager(analytic_list_buttons):
                            cmds.button( label=maya.stringTable['y_maya_analytics_Window.kSelectAllAnalytics']
                                       , command=callback_tool(self, partial(AnalyticsWindow.on_select_all_analytics))
                                       )
                            cmds.button( label=maya.stringTable['y_maya_analytics_Window.kDeselectAllAnalytics']
                            ...
                            self.static_widget = cmds.checkBox( value=ANALYTICS_RUNNER.static
                                         , label=maya.stringTable['y_maya_analytics_Window.kShowStatic' ]
                                         , changeCommand=callback_tool(self, partial(AnalyticsWindow.on_select_static_analytics))
                                         , annotation=maya.stringTable['y_maya_analytics_Window.kShowStaticInfo' ]
                                         )
                        ...
                        with LayoutManager(analytic_list_items):
                            info_annotation = maya.stringTable['y_maya_analytics_Window.kInfoAnnotation' ]
                            self.analytic_checkboxes = {}
                ...
                #    [ +F ] [ +D ]
                #
                self.__path_layout = cmds.frameLayout( label=maya.stringTable['y_maya_analytics_Window.kPathTitle' ]
                                        , annotation=maya.stringTable['y_maya_analytics_Window.kPathAnnotation' ]
                ...
                #    ADD
                #
                self.__pattern_layout = cmds.frameLayout( label=maya.stringTable['y_maya_analytics_Window.kSkipTitle' ]
                                        , annotation=maya.stringTable['y_maya_analytics_Window.kSkipAnnotation' ]
                                        , marginWidth=25, marginHeight=4, collapsable=True, collapse=True )
                # Get out of the frame layout, it will be used explicitly in create_ui
