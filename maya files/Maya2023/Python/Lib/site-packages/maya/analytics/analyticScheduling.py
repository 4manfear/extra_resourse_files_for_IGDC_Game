...
kAnalyticLabel = maya.stringTable['y_maya_analytics_analyticScheduling.kAnalyticLabel' ]
kAnalyticDescriptionShort = maya.stringTable['y_maya_analytics_analyticScheduling.kAnalyticDescriptionShort' ]
...
class analyticScheduling(BaseAnalytic):
    ...
    def run(self):
        ...
        with emModeManager() as em_mgr, PlaybackManager() as play_mgr:
            ...
            BUILD = os_getenv( 'BUILD' )
            # Read the old file
            (old_nodes, old_edges) = self.read_scheduling_file( os_path.join(BUILD, '_MayaSchedulingGraph.dot') )

            # Generate the manual file
            dot_file = os_path.join(BUILD, 'SchedulingGraph.dot')
