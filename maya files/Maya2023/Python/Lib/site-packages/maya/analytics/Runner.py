...
class Runner(object):
    ...
    def __analytics_to_run(self):
        ...
        # Otherwise walk all of the analytics looking for matching ones
            ...
            for (analytic_name, analytic) in list(list_analytics().items()):
                try:
                    if self.static == analytic.is_static:
                        new_analytic = analytic()
                        new_analytic.logger = self.logger
    ...
    def __files_to_analyze(self):
        ...
        if not self.paths:
            ...
            if results_path is None:
                results_path = os_path.join( os_path.dirname( cmds.file(query=True,expandName=True) ), 'MayaAnalytics' )
        ...
        for path in self.paths:
            ...
            for (the_dir, the_files) in dir_list:
                if self.results_path == None:
                    analytic_dir = os_path.join(the_dir, 'MayaAnalytics')
                else:
                    analytic_dir = self.results_path
                for the_file in the_files:
                    relpath = os_path.relpath(the_dir,path)
                    if relpath == '.':
                        relpath = ''
                    if self.results_path == None:
                        analytic_path = namer.name(the_file)
                    else:
                        analytic_path = namer.name(os_path.join(relpath,the_file))
                    analytic_path = os_path.join( analytic_dir, analytic_path )
                    file_path = os_path.join(the_dir, the_file)
                    file_modified_time = os_path.getmtime(file_path)
    ...
    #======================================================================
    def __should_analytic_run( self, analytic, maya_file_modified_time ):
        """
        ...
        from os import path as os_path
        if os_path.exists(analytic.marker_file()):
        ...
        for analytic_file in analytic.output_files():
            ...
            if not os_path.exists(analytic_file):
                analytic.debug( 'Analytic with no results should run' )
                return True

            if self.force:
                analytic.debug( 'Analytic with results but being forced should still run' )
                return True

            analytic_modified_time = os_path.getmtime(analytic_file)
    ...
    def run(self):
        ...
        for (maya_file,file_modified_time,analytic_directory) in files_to_analyze:
            self.__debug( 'Running analytics on file "{0:s}"'.format( maya_file if maya_file is not None else 'None' ) )
            self.__indent(1)
            json_per_file = {}
                ...
                try:
                    ...
                    if not file_is_loaded:
                        try:
                            ...
                            # The HIK plug-in behaves badly, leaving behind some nodes that should be purged here for consistency
                            try:
                                node_types = cmds.pluginInfo('mayaHIK', query=True, dependNode=True)
                                nodes_to_delete = []
                                for node_type in node_types:
                                    nodes_to_delete += cmds.ls(type=node_type)
                                if len(nodes_to_delete) > 0:
                                    cmds.delete( nodes_to_delete )
                            except RuntimeError:
                                pass
                            memory_before = cmds.memory( heapMemory=True, asFloat=True, megaByte=True )
                            ...
                            memory_delta = memory_before - cmds.memory( heapMemory=True, asFloat=True, megaByte=True )
                        except Exception as ex:
                            self.__error( 'Problems reading test file "{}" : {}'.format(maya_file, ex) )
                    ...
                    analytic.debug( 'Output files {0:s}'.format( str( analytic.output_files() ) ) )
                    ...
                    try:
                        with open(analytic.json_file(), 'w') as json_fd:
                            ...
                            # the main intent.
                            json.dump( json_data, json_fd, indent=4, separators=(',', ': ') )
                    ...
                    analytic.error( 'Unknown failure: "{0:s}"'.format( str(ex) ) )
        ...
        if len(combinations_run) > 0:
            print(json.dumps( combinations_run, indent=4, separators=(',', ': ') ))
