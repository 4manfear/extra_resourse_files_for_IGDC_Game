...
with and without the cache evaluator active.
"""
import maya
...
kAnalyticLabel = maya.stringTable['y_maya_analytics_analyticTiming.kAnalyticLabel' ]
kAnalyticDescriptionShort = maya.stringTable['y_maya_analytics_analyticTiming.kAnalyticDescriptionShort' ]

# List of EM modes to be measured
MODE_LIST = ['emp-cache', 'emp+cache']
...
class analyticTiming(BaseAnalytic):
    ...
    cache for a scene that uses VP2:
        ...
        "output" : {
            "emp-cache" : {
                ...
                "Vp2SceneRender"                 : 7152,
                "Vp1SceneRender"                 : 0,
                "ClusterCount"                   : { "invisibility" : 4 },
                "ClusterNodeCount"               : { "invisibility" : 123 },
            ...
            "emp+cache" : {
                ...
                "Vp2SceneRender"                 : 7326,
                "Vp1SceneRender"                 : 0,
                "ClusterCount"                   : { "invisibility" : 4, "cache" : 12 },
                "ClusterNodeCount"               : { "invisibility" : 123, "cache" : 405 },
    ...
    def __count_clusters(evaluator_list):
        ...
        :return: { "EVALUATOR_NAME" : [NUMBER_OF_CLUSTERS, NUMBER_OF_NODES_IN_CLUSTERS]
                 Counts are the totals for the evaluators in the list
        '''
        cluster_counts = {}
        for evaluator in evaluator_list:
            ...
            while cluster_idx < len(cluster_info):
                ...
                cluster_count += 1
            cluster_counts[evaluator] = [cluster_count, clustered_node_count]
    ...
    def __count_cycles():
        ...
        cycle_node_count = 0
        try:
            scheduling_data = json.loads( cmds.dbpeek( op='graph', eg=True, all=True, a=['scheduling', 'verbose'] ) )['scheduling']['clusters']
            for (cluster_name, cluster_contents) in list(scheduling_data.items()):
        ...
        try:
            ...
            for mode in MODE_LIST:
                ...
                with emModeManager() as em_manager:
                    ...
                    node_data[mode]['ClusterCount'] = {}
                    node_data[mode]['ClusterNodeCount'] = {}
                    for (evaluator_name, cluster_counts) in list(self.__count_clusters( cmds.evaluator(en=True, query=True) ).items()):
                        node_data[mode]['ClusterCount'][evaluator_name] = cluster_counts[0]
                        node_data[mode]['ClusterNodeCount'][evaluator_name] = cluster_counts[1]
