...
#======================================================================
class ParentManager(object):
    """
    This class is very similar than Layout Manager excepte that it will jump to
    any parent.
    Prior to jumping it wil backup current parent and will restor it at exit
    """

    def __init__(self, name):
        """
        Simple constructor that just remembers the current Parent and set parent to 'name'.
        """
        self.previousParent = cmds.setParent(q=True)
        cmds.setParent(name)
        self.name = name
    ...
    def __exit__(self, type, value, traceback):
        cmds.setParent(self.previousParent)
#======================================================================
...
def showMessageBox(title, message, icon=None):
    ...
    okButtonString = maya.stringTable['y_maya_common_ui.kOK' ]
    cmds.confirmDialog(
        title=title,
        message=message,
...
def scrollableMessageBox(title, messageCallback):
    ...
    refresh_button = cmds.button( maya.stringTable['y_maya_common_ui.kRefresh' ], annotation=maya.stringTable['y_maya_common_ui.kRefreshAnnotation' ] )

    copy_button = cmds.button( maya.stringTable['y_maya_common_ui.kCopy' ], annotation=maya.stringTable['y_maya_common_ui.kCopyAnnotation' ] )

    cmds.separator( style='single' )

    cmds.text( label=maya.stringTable['y_maya_common_ui.kOutputHighlight' ] )
    filter_field = cmds.textField( smb_output_filter,
                    text='', alwaysInvokeEnterCommandOnReturn=True,
                    annotation=maya.stringTable['y_maya_common_ui.kShowOutputFilterInfo' ] )
    cmds.setParent( '..' )
...
def reloadScrollableMessageBox(messageCallback, output_field, filter_field):
    ...
    try:
        ...
        line_id = ['line0']
        line_text = [maya.stringTable['y_maya_common_ui.kDataNotFound' ]]
        line_select = []
...
def showConfirmationDialog(title, message):
    ...
    okButtonString = maya.stringTable['y_maya_common_ui.kOKButton' ]
    cancelButtonString = maya.stringTable['y_maya_common_ui.kCancelButton' ]
...
def unix_cmd_exists(cmd):
    ...
    from subprocess import call as subprocess_call, PIPE as subprocess_PIPE
    return subprocess_call("type " + cmd, shell=True,
        stdout=subprocess_PIPE, stderr=subprocess_PIPE) == 0
