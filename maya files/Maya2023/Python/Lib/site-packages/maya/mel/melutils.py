...
def createMelWrapper(fn, types = [], retType='void', ignoreDefaultArgs=True, returnCmd=False, outDir=None):
    ...
    if fn.__module__ == '__main__':
        ...
        importStatement = 'python("from %s import %s");'%(fn.__module__,fn.__name__)

    proto = 'global proc %s %s (%s) {'%(retType,fn.__name__,paramList)
    if len(args) == 0:
        tmpDecl = ""
        pyCall = 'python("%s()")'%(fn.__name__)
    else:
        tmpDecl = "string $tmp = " + argListStr + "; "
        pyCall = 'python(("%s(" + $tmp + ")"))'%(fn.__name__)
