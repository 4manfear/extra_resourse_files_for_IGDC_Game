...
# allow some safe Python access
from os import environ as os_environ


_shellLogHandler = None
_shellStdoutLogHandler = None
_guiLogHandler = None
...
def loadStringResourcesForFile( scriptPath, fullModulePath, resourceFileName ):
    ...
    if scriptPath != '':
        ...
        localizedPath = os_path.normpath(os_path.join( scriptPath, 'scripts', path, resourceFileName ))
        try:
            from io import open as io_open
            with io_open(localizedPath, encoding = "utf-8" ) as localizedFile:
                exec(localizedFile.read())
...
def loadStringResourcesForModule( moduleName ):
    ...
    try:
        ...
        module = sys_modules[moduleName]
    except:
        raise RuntimeError( 'Failed to load base string resources for module %s because it has not been imported' % moduleName )

    from os import path as os_path
    modulePath, moduleFileName = os_path.split( module.__file__ )
    moduleName, extension = os_path.splitext( moduleFileName )
    
    resourceFileName = moduleName + '_res.py'
    
    # Try to find the base version of the file next to the module
    try:
        baseVersionPath = os_path.join( modulePath, resourceFileName )
        from sys import version_info as sys_version_info
        if sys_version_info[0] >= 3:
            from io import open as io_open
            with io_open(baseVersionPath, encoding = "utf-8") as baseFile:
                exec(baseFile.read())
        else:
    ...
    if cmds.about( uiLanguageIsLocalized=True ):
        ...
        from os import environ as os_environ
        pluginResourcePathEnvironmentVariable = os_environ.get('MAYA_PLUG_IN_RESOURCE_PATH')
...
def getPossibleCompletions(input):
    ...
    try:
        ...
        for index in range(sys_maxsize):
...
def helpNonVerbose(thing, title='Python Library Documentation: %s', forceload=0):
    ...
    try:
        ...
        try:
            ...
            from sys import modules as sys_modules
            thingObj=eval(thingStr,sys_modules['__main__'].__dict__)
...
def guiLogHandler():
    ...
    from os import environ as os_environ
    format = os_environ.get('MAYA_GUI_LOGGER_FORMAT', '%(name)s : %(message)s')
    _guiLogHandler.setFormatter( logging.Formatter(format) )

    appLoggerName = os_environ.get('MAYA_DEFAULT_LOGGER_NAME', '')
    log = logging.getLogger(appLoggerName)
    log.addHandler(_guiLogHandler)

    global _shellStdoutLogHandler
    if _shellStdoutLogHandler is not None:
        # stdout logging used in batch mode only
        log.removeHandler(_shellStdoutLogHandler)
        _shellStdoutLogHandler = None
    return _guiLogHandler
...
def shellLogHandler():
    ...
    from os import environ as os_environ
    shellStream = os_environ.get('MAYA_SHELL_LOGGER_STREAM')
    if shellStream is not None:
        import sys
        shellStream = getattr(sys, shellStream, None)

    _shellLogHandler = logging.StreamHandler(shellStream)
    format = os_environ.get('MAYA_SHELL_LOGGER_FORMAT', '%(name)s : %(levelname)s : %(message)s')
    ...
    if shellStream is None:
        # Use same settings as Maya API/command logging
        logLevels = [ "all", "info", "result", "warning", "error", "none"]
        stderrLevel = os_environ.get('MAYA_BATCH_STDERR_LOGGING_LEVEL', 'all').lower()
        if stderrLevel in logLevels:
            if stderrLevel == "none":
                _shellLogHandler.setLevel(logging.CRITICAL)
            if stderrLevel == "error":
                _shellLogHandler.setLevel(logging.ERROR)
            elif stderrLevel == "warning":
                _shellLogHandler.setLevel(logging.WARNING)
            else:
                _shellLogHandler.setLevel(logging.INFO)

        stdoutLevel = os_environ.get('MAYA_BATCH_STDOUT_LOGGING_LEVEL', 'none').lower()
        if stdoutLevel in logLevels and stdoutLevel != "none":
            global _shellStdoutLogHandler
            from sys import stdout as sys_stdout
            _shellStdoutLogHandler = logging.StreamHandler(sys_stdout)
            _shellStdoutLogHandler.setFormatter(logging.Formatter(format))
            if stdoutLevel == "error":
                _shellStdoutLogHandler.setLevel(logging.ERROR)
            elif stdoutLevel == "warning":
                _shellStdoutLogHandler.setLevel(logging.WARNING)
            else:
                _shellStdoutLogHandler.setLevel(logging.INFO)

            log.addHandler(_shellStdoutLogHandler)
...
def formatGuiException(exceptionType, exceptionObject, traceBack, detail=2):
    ...
    if issubclass(exceptionType, SyntaxError):
        ...
        exceptionMsg = futureStr(exceptionObject.args[0])
    else:
        # We would formerly use exceptionObject.args[0], if available.
        ...
        exceptionMsg = futureStr(exceptionObject)
    ...
    if detail == 0:
        ...
        if detail == 1:
            ...
            if len(excLines) > 0:
                # MAYA-105821:
                # This substitution (re.sub), already implemented before MAYA-105821, is required because traceback might 
                # contain the pattern r'<unprintable.*object>' if it was unable to cast the object using str().
                #
                # In Python 3, re.sub() raises an error if arguments 'repl' or 'string' contain unescaped 
                # backslashes (\), while Python 2 tolerated it. This change was introduced in Python 3.7.
                #
                # Workaround: Use a Callable, which simply returns exceptionMsg, instead of a string.
                # The argument (match object) passed to the Callable is not used.
                from sys import version_info as sys_version_info
                if sys_version_info[0] >= 3:
                    excLines[-1] = re.sub(r'<unprintable.*object>', (lambda x: exceptionMsg), excLines[-1])
                else:
...
def formatGuiResult(obj):
    ...
    if isinstance(obj, str) or isinstance(obj, text_type):
        return obj
    else:
        return repr(obj)
