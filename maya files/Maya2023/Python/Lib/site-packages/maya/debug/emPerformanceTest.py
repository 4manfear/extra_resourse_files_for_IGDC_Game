...
def switchMayaViewport(newMode):
    ...
    if newMode == emPerformanceOptions.VIEWPORT_1:
        ...
        raise NameError( maya.stringTable['y_maya_debug_emPerformanceTest.kSwitchViewportError' ] % str(newMode) )
...
class emPerformanceRun(object):
    ...
    def testTheType(self, theTestType):
        ...
        if theTestType == emPerformanceOptions.TEST_PLAYBACK:
            ...
            raise NameError( maya.stringTable['y_maya_debug_emPerformanceTest.kTestTheTypeError' ] % str(theTestType) )
        ...
        with CacheEvaluatorManager() as cacheMgr:
            ...
            with emModeManager() as mgr:
                ...
                if self.fileName:
                    ...
                    self.progress.startPhase( maya.stringTable['y_maya_debug_emPerformanceTest.kReadingMessage' ] )
                        ...
                        for testType in self.options.testTypes:
                            ...
                            if not timingResults:
                                ...
                                if len(timingResults) == 2:
                                    ...
                                    raise TypeError( maya.stringTable['y_maya_debug_emPerformanceTest.kDirtyRefreshError' ] % str(timingResults) )
                ...
                if self.fileName:
                    self.progress.startPhase( maya.stringTable['y_maya_debug_emPerformanceTest.kNewMessage' ] )
    ...
    def testPlayback(self):
        ...
        try:
            with PlaybackManager() as play_mgr:
                ...
                self.progress.startPhase( maya.stringTable['y_maya_debug_emPerformanceTest.kPlayMessage' ] % (self.evalMode, self.options.iterationCount) )
    ...
    def testRefresh(self):
        ...
        self.progress.startPhase( maya.stringTable['y_maya_debug_emPerformanceTest.kRefreshMessage' ] % (self.evalMode, self.options.iterationCount * 10) )
    ...
    def testDirtyRefresh(self):
        ...
        # Since refreshes are fast compared to playback do it 10 times
        self.progress.startPhase( maya.stringTable['y_maya_debug_emPerformanceTest.kDirtyRefreshMessage' ] % (self.evalMode, self.options.iterationCount * 10) )
        for idx in range(0, self.options.iterationCount * 10):
...
class emPerformanceOptions(object):
    ...
    COLUMN_TITLE_START_FRAME = maya.stringTable['y_maya_debug_emPerformanceTest.kColumnTitleStartFrame']
    COLUMN_TITLE_END_FRAME = maya.stringTable['y_maya_debug_emPerformanceTest.kColumnTitleEndFrame']
    COLUMN_STAT_AVG = maya.stringTable['y_maya_debug_emPerformanceTest.kColumnTitleAvg' ]
    COLUMN_STAT_MAX = maya.stringTable['y_maya_debug_emPerformanceTest.kColumnTitleMax' ]
    COLUMN_STAT_MIN = maya.stringTable['y_maya_debug_emPerformanceTest.kColumnTitleMin' ]
    COLUMN_STAT_TIME = maya.stringTable['y_maya_debug_emPerformanceTest.kColumnTitleTime']
    ...
    def setEvalModes(self, newEvalModes):
        ...
        if isinstance(newEvalModes, list):
            self.evalModes = [mode.lower() for mode in newEvalModes]
        else:
            self.evalModes = [newEvalModes.lower()]
    ...
    def getColumnTitle(self, viewport, type, mode, stat):
        ...
        """
        return '%s %s %s %s' % (viewport, type, mode.upper(), stat)
...
def findMayaFiles(directory):
    ...
    for content in os_listdir(directory):
        subpath = os_path.join(directory, content)
        if os_path.isdir(subpath):
            filesFound += findMayaFiles( subpath )
        elif os_path.isfile(subpath):
...
class progressState(object):
    ...
    def __init__(self, fileCount, phaseCount, enable):
        ...
        self.enable = enable
        self.currentFileName = maya.stringTable['y_maya_debug_emPerformanceTest.kStartup' ]
        self.progressPerFile = 1.0 / fileCount
        self.totalProgress = 0
        self.currentFile = 0
        self.currentPhase = 0
        self.currentPhaseName = maya.stringTable['y_maya_debug_emPerformanceTest.kReadingInitPhase' ]
        self.progressPerPhase = self.progressPerFile / phaseCount
        if enable:
            winTitle = maya.stringTable['y_maya_debug_emPerformanceTest.kProcessingMessage' ] % fileCount
            cmds.progressWindow( title=winTitle, progress=0, status=maya.stringTable['y_maya_debug_emPerformanceTest.kInitializingMessage' ], isInterruptable=True )
    ...
    def startFile(self, newFileName):
        ...
        self.currentFile += 1
        self.currentPhase = 0
        self.currentPhaseName = maya.stringTable['y_maya_debug_emPerformanceTest.kReadingPhase' ]
...
def emPerformanceTest(
    ...
    if filesAndDirectories:
        for path in filesAndDirectories:
            if os_path.isdir( path ):
                ...
                print(maya.stringTable['y_maya_debug_emPerformanceTest.kWrongFileWarning' ] % path)

        if len(mayaFiles) == 0:
            print(maya.stringTable['y_maya_debug_emPerformanceTest.kNoFileError' ])
            return
    else:
        mayaFiles = [None]

    kExceptionError = maya.stringTable['y_maya_debug_emPerformanceTest.kExceptionError' ]
    if resultsFileName == 'stdout' or resultsFileName == 'cout':
        from sys import stdout as sys_stdout
        outputFile = sys_stdout
    elif resultsFileName == 'stderr' or resultsFileName == 'cerr':
        from sys import stderr as sys_stderr
        outputFile = sys_stderr
        ...
        if len(dirList) == 1:
            resultsFileName = os_path.join( dirList[0], resultsFileName )
        try:
    ...
    if options.usesEvaluationManager():
        if not evaluationManagerExists():
            print(maya.stringTable['y_maya_debug_emPerformanceTest.kNoEvaluationManagerError' ])
    ...
    if outputFile:
        outputFile.write( titleObject.columnTitles() + os_linesep )
    else:
        outputData.append( titleObject.columnTitles() )
    from os import path as os_path
    for fileName in mayaFiles:
        if fileName:
            currentProgress.startFile( os_path.basename(fileName) )
        else:
            currentProgress.startFile( maya.stringTable['y_maya_debug_emPerformanceTest.kCurrentScene' ] )
        perform = emPerformanceRun( fileName=fileName, options=options, progress=currentProgress )
        perform.runTests()
        if outputFile:
            outputFile.write( perform.columns() + os_linesep )
        else:
