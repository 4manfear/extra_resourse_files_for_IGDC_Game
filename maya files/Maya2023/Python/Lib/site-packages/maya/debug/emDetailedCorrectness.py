...
def __summarizeCorrectnessInScene(resultsDir, dataRoot):
    ...
    nodeChanges = {}
    nodeTypes = {}
    from os import path as os_path, sep as os_sep, makedirs as os_makedirs
    (path,fileName) = os_path.split( cmds.file( query=True, loc=True ) )
    path = path.replace( dataRoot, '' )
    if len(path) > 0 and (path[0] == '/' or path[0] == os_sep):
        path = path[1:]

    # Copy the results files in place
    resultsPath = os_path.join( resultsDir, path, fileName )
    print('Copying results to %s' % resultsPath)

    if not os_path.isdir( resultsPath ):
        os_makedirs( resultsPath )
    from shutil import move as shutil_move
    for mode in list(modeNames.keys()) + ['dg']:
        for suffix in ['txt', 'png']:
            resultsBaseName = 'SCENE.%s.%s' % (mode, suffix)
            resultsFile = os_path.join( resultsDir, resultsBaseName )
            destinationFile = os_path.join( resultsPath, resultsBaseName )
            shutil_move( resultsFile, destinationFile )

    # Move the file containing the EM graph over, if it exists
    emPlugFileName = 'EMplugs.txt'
    resultsEmPlugFile = os_path.join( resultsDir, emPlugFileName )
    if os_path.exists( resultsEmPlugFile ):
        shutil_move( resultsEmPlugFile, os_path.join( resultsPath, emPlugFileName ) )

    rawDifferences = 0
    rawValues = {}
    rawValues['dg'] = {}
    for (comparisonType, result) in list(results.items()):
    ...
    summaryFile = os_path.join(resultsPath, 'SUMMARY.txt')
    ...
    print('--- %d node type results' % len(list(nodeTypes.keys())))
    ...
    print('--- %d attribute change results' % len(list(attrChanges.keys())))
    ...
    print('--- %d node change results' % len(list(nodeChanges.keys())))
    ...
    for plug in list(rawValues['dg'].keys()):
        summaryFd.write( '%s\n' % plug )
        for comparisonType in list(rawValues.keys()):
...
def __summarizeCorrectness(resultsDir='.', dataRoot='.', files=None):
    ...
    if files == None:
        ...
        for loadFile in files:
            if not os_path.exists( os_path.join(dataRoot, loadFile) ):
                print('ERROR: No file %s, aborting' % os_path.join( dataRoot, loadFile ))
                return
        for loadFile in files:
            cmds.file( os_path.join(dataRoot,loadFile), force=True, open=True )
            __summarizeCorrectnessInScene( resultsDir, dataRoot )
            resultsPaths += [os_path.join(dataRoot,loadFile)]
    return resultsPaths
...
def __getFileLocations():
    ...
    if not RESULTS_ROOT:
        if RESULTS_ENV in os_environ:
            RESULTS_ROOT = os_getenv( RESULTS_ENV )
        else:
    ...
    if not DATA_LOCATION_ROOT:
        if DATA_LOCATION_ENV in os_environ:
            DATA_LOCATION_ROOT = os_getenv( DATA_LOCATION_ENV )
        else:
...
def onDirectory():
    ...
    if not directoryToAnalyze or (len(directoryToAnalyze) == 0):
        ...
        for root, dirs, files in os_walk(directoryToAnalyze[0]):
            ...
            for name in files:
                if name[-3:] == '.ma' or name[-3:] == '.mb':
                    from os import path as os_path
                    filesToAnalyze.append( os_path.join(root, name) )
