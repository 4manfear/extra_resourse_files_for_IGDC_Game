...
RESULTS_ERRORS = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintIssuesFound' ]
RESULTS_NONE   = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintNoResults'   ]
RESULTS_OKAY   = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintNoProblems'  ]
READY_TO_RUN   = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintReadyToRun'  ]
SELECT_ALL     = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintSelectAll'   ]
SELECT_NONE    = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintSelectNone'  ]
BENEFIT        = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintBenefit'     ]
DESCRIPTION    = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintDescription' ]
CODE           = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintCode'        ]
NO_CODE        = maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintNoCode'      ]
...
class SceneLintWindow(DebugTrace):
    ...
    def __callback_run_selected_issues(tool):
        ...
        try:
            ...
            if cmds.checkBox(tool.all_issues_checkbox, query=True, value=True):
                issue_list = list(tool.issue_descriptions.keys())
                tool.issue_results = cmds.sceneLint()

            # Otherwise pass in the list of currently selected issues
            else:
                for (issue_name, issue_widget) in list(tool.issue_checkboxes.items()):
            ...
            tool.err( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintParseError' ].format( ex ) )
            tool.issue_results = {}
    ...
    def get_mitigation_code(self, issue_name):
        ...
        try:
            ...
            if KEY_SELECT not in widgets:
                ...
                complete_code = 'print "{}"'.format( NO_CODE )
            else:
                ...
                if widgets[KEY_TYPE] == VALUE_CHECKBOX:
                    ...
                    if cmds.checkBox( widgets[KEY_SELECT], query=True, value=True ):
                        ...
                        complete_code = 'print "{}"'.format( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintNotFixing' ].format( issue_name, mitigation_name ) )
                    ...
                    if object_list is None:
                        complete_code = 'print "{}"'.format( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintNothingSelected' ].format( issue_name, mitigation_name ) )
                    else:
                        complete_code += 'OBJECTS = [{}]\n'.format( ','.join(['"{}"'.format(obj) for obj in object_list]) )
                        complete_code += mitigation_code
            return complete_code

        except Exception as ex:
            # If an error occurs then generate empty code and show the problem
            self.err( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintCodeError' ].format( issue_name, ex ) )
            return 'pass'
    ...
    def fix_selected_results(self, issue_name):
        ...
        try:
            ...
            self.err( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintApplyError' ].format( issue_name, ex, fix_code ) )
            ...
            if issue_name in tool.issue_results:
                if cmds.checkBox(issue_widget, query=True, value=True):
                    try:
                        ...
                        tool.warn( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintIssueFixFailed' ].format( issue_name, ex ) )
    ...
    def deselect_all_mitigation_objects(self, issue_name):
        ...
        try:
            ...
            self.err( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintErrorDeselectObjError' ].format( issue_name, ex ) )
    ...
    def select_all_mitigation_objects(self, issue_name):
        ...
        try:
            ...
            self.err( maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintErrorSelectObjError' ].format( issue_name, ex ) )
    ...
    def __callback_select_all_issues(tool):
        ...
        for issue_widget in list(tool.issue_checkboxes.values()):
...
"nurbs-tessellation": {
        ...
        '''
        for (issue_name, issue_widget) in list(self.issue_checkboxes.items()):
    ...
    def __build_window_buttons(self):
        ...
        with LayoutManager( cmds.rowLayout( numberOfColumns=3
            ...
            cmds.button( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintRunSelected' ],             command=callback_tool(self, self.__callback_run_selected_issues) )
            cmds.button( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintClearResults'],            command=callback_tool(self, self.callback_clear_results) )
            cmds.button( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintApplyFixes'  ], command=callback_tool(self, self.__callback_apply_fixes) )
    ...
    def __build_issue_fix_buttons(self, issue_name):
        ...
        with LayoutManager( cmds.rowLayout( numberOfColumns=4
            ...
            cmds.button( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintButtonApplyFixLabel' ]
                       , annotation=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintApplFixAnnotation' ]
                       , width=50
                       , command=callback_tool(self, partial(self.__callback_fix_selected_results, issue_name=issue_name)) )
            cmds.text( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintFixOr' ] )
            cmds.button( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintCopyCode' ]
                       , annotation=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintCopyCodeAnnotation' ]
                       , width=50
                       , command=callback_tool(self, partial(self.__callback_copy_fix_to_clipboard, issue_name=issue_name) ) )

            cmds.text( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintFixButtonText' ] )
    ...
    def build_mitigation_widgets( self, issue_name, mitigation ):
        ...
        try:
            ...
            info = maya.stringTable[ 'y_maya_debug_SceneLintWindow.kSceneLintNoDescription'  ]
        ...
        with LayoutManager( widgets[KEY_LAYOUT] ):
            ...
            if len(mitigation_objects) == 0:
                widgets[KEY_SELECT] = cmds.checkBox( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintSelectNoObjectMitigation'], value=True )
    ...
    def __build_mitigation_section(self, issue_name, mitigation_list):
        ...
        Will not be created if the mitigation must be done manually.
        ...
        try:
            ...
            with LayoutManager(cmds.columnLayout( adjustableColumn=True )):
                with LayoutManager( cmds.rowColumnLayout( adjustableColumn=3
                    ...
                    if len(mitigation_list) > 1:
                        cmds.text( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintSelectFixType' ] )
                        ...
                        cmds.text( label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintOneFixType' ].format( first_mitigation[KEY_NAME] ), annotation=first_mitigation[KEY_DESCRIPTION] )
                        cmds.text( label='' )
            ...
            cmds.text( parent=self.issue_frame_layouts[issue_name], label=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintNoMitigation' ] )
    ...
    def __build_issue_widgets(self):
        ...
        for issue_count in range(0, len(list(self.issue_descriptions.keys()))):
        ...
        with LayoutManager( cmds.rowColumnLayout( numberOfColumns=2, adjustableColumn=2
            ...
            for issue_name, issue_description in list(self.issue_descriptions.items()):
                # The issue test
    ...
    def __build_window(self):
        ...
        if self.window_name is None:
            ...
            self.window_name = cmds.window( WINDOW_NAME
                                          , title=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintTitle' ]
                                          , iconName=maya.stringTable['y_maya_debug_SceneLintWindow.kSceneLintIconName' ]
