...
class EmptyContext(object):
    ...
    def comparison_is_valid():
        ''':return: True, empty contexts provide valid comparisons'''
        return True
...
def __is_maya_file(path):
    ...
    return os_path.isfile(path) and ((path[-3:] == '.ma') or (path[-3:] == '.mb'))
...
def __find_em_plugs(relevant_nodes, results_filename):
    ...
    try:
        ...
        if results_filename is None:
            ...
            plug_file = os_path.join( tempfile_gettempdir(), 'EMPlugs.txt' )
    ...
    if not json_plugs or 'plugs' not in json_plugs or 'dirtyPlugs' not in json_plugs['plugs']:
        print('WARNING: No output from plug list')
        return em_plugs

    for node, per_node_list in list(json_plugs['plugs']['dirtyPlugs'].items()):
...
def run_correctness_test( reference_mode
    ...
    :param data_types:     List of data types to include in the analysis. These are the possibilities:
                               ...
                               vertex: Not currently supported, See mesh data type
                               mesh  : Attributes on the mesh shape that hold vertex positions
                                       This is currently buggy and may produce evaluation errors or bad results if used
                                       Vastly increases analysis time for scenes with high-resolution meshes
    ...
    try:
        ...
        if results_path != None:
            ...
            if not os_path.isdir(results_path):
                from os import makedirs as os_makedirs
                os_makedirs(results_path)

            em_plug_file_name = os_path.join(results_path, 'EMPlugs.txt')
            ...
            if file_name != None:
                ...
                if os_path.isabs(file_name):
                    results_path = os_path.join(results_path, os_path.basename(file_name))
                else:
                    results_path = os_path.join(results_path, file_name)
            else:
                results_path = os_path.join(results_path, 'SCENE')
        ...
        if 'screen' in data_types:
            if results_path == None:
                from tempfile import gettempdir as tempfile_gettempdir
                image_dir = tempfile_gettempdir()
                if file_name != None:
                    # Absolute paths cannot be appended to the results path. Assume
                    # that in those cases just using the base name is sufficient.
                    if os_path.isabs(file_name):
                        image_dir = os_path.join(image_dir, os_path.basename(file_name))
                    else:
                        image_dir = os_path.join(image_dir, file_name)
                else:
                    image_dir = os_path.join(image_dir, 'SCENE')
            else:
        ...
        with emModeManager() as em_mgr, PlaybackManager() as play_mgr:
            ...
            for mode_num in range(len(modes)):
                ...
                with extra_context:
                    ...
                    if extra_context.comparison_is_valid():
                ...
                if mode_state is not None:
                    ...
                    # If the comparison was not legal to make then play it safe and report no errors
                    error_count = 0
                    comparison = { }
