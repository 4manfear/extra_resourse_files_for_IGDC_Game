...
show formatted detailed results in the script editor window:
    ...
    for (result_type,result_list) in results.items():
        ...
        for (plug,values) in result_list.items():
            print '------' + plug
            try:
    ...
    for (resultType,result) in results.items():
...
class EMCorrectnessContext(object):
    ...
    #----------------------------------------------------------------------
    def comparison_is_valid(self):
        '''
        Ask if the current state of the EM provides valid data for comparison.
        :return: True when the EM is still active
        '''
        return cmds.evaluationManager(query=True, mode=True)[0] != 'off' and cmds.evaluationManager(query=True, enabled=True)
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.cache_mgr.__exit__(*sys_exc_info())
            raise ex
        try:
            self.hik_mgr.__enter__()
        except Exception as ex:
            self.hik_mgr.__exit__(*sys_exc_info())
            raise ex
...
def emCorrectnessTest( fileName=None
                     ...
                     , dataTypes=['matrix','vertex','screen'] # vertex data type is not currently used
