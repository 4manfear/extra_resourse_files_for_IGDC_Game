...
class graphStructure(object):
    ...
    def __init_from_json(self, json_string):
        ...
        if self.evaluation_graph:
            try:
                ...
                for (node, plug_dictionary) in list(plug_json.items()):
        ...
        try:
            ...
            for (_, node_connections) in list(self.raw_json[KEY_CONNECTIONS].items()):
                ...
                for connections in node_connections['downstream']:
                    ...
                    self.connections += ['{} {}'.format(src, dst) for (src, dst) in list(connections.items())]
        except Exception as ex:
            # If the plugs weren't included then of course they are not present
    ...
    def write(self, fileName=None):
        ...
        out.write( json.dumps( self.raw_json, indent=4, separators=(',', ': ') ) )
        out.close()
    ...
    def __compare_lists(list1, list2):
        ...
        additions = []
        removals = []
        for (entry, entry_count) in list(list_totals.items()):
            if entry_count == 1:
