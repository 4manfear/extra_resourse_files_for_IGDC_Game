...
class emModeManager(DebugTrace):
    ...
    def __save_state(self):
        ...
        self.original_reduce_graph_rebuild = self.reduce_graph_rebuild
        return self
    ...
    def __init__(self):
        ...
        self.original_reduce_graph_rebuild = False
    ...
    def setMode(self, modeName):
        ...
        for (evaluator, node_types) in list(node_types_to_enable.items()):
            for node_type in node_types:
                try:
        ...
        for (evaluator, node_types) in list(node_types_to_disable.items()):
            for node_type in node_types:
                try:
    ...
    def restore_state(self):
        ...
        for (evaluator, restored_node_types) in list(self.original_evaluator_node_types.items()):
        ...
        self.reduce_graph_rebuild = self.original_reduce_graph_rebuild
    ...
    @property
    def reduce_graph_rebuild(self):
        '''Return the current status of the transientPseudoEvaluationNodeManager mode'''
        return cmds.evaluationManager(query=True, reduceGraphRebuild=True)

    @reduce_graph_rebuild.setter
    def reduce_graph_rebuild(self, newValue):
        '''Enable/disable the transientPseudoEvaluationNodeManager rebuild mode'''
        cmds.evaluationManager(reduceGraphRebuild=newValue)
