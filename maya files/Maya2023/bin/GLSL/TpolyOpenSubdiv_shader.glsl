...
vec4 GeneratePatchCoordForUniformSubdiv(vec2 localFaceCoord)
{
    ...
    //
    ivec3 ptexIndex = texelFetch(OsdPatchParamBuffer, gl_PrimitiveID).xyz;
	int faceID = (ptexIndex.x & 0xfffffff);
...
void emit(int index, vec3 normal, vec2 uvs[4], vec4 colors[4])
{
    // PatchCoord
#ifdef FVAR_ADAPTIVE
    ...
    outpt.texCoord.st =
		...
		outpt.vcolor =
			vec4( mix( mix(colors[0].x, colors[1].x, st.s ), mix(colors[3].x, colors[2].x, st.s ), st.t),
			      mix( mix(colors[0].y, colors[1].y, st.s ), mix(colors[3].y, colors[2].y, st.s ), st.t),
				  mix( mix(colors[0].z, colors[1].z, st.s ), mix(colors[3].z, colors[2].z, st.s ), st.t),
				  mix( mix(colors[0].w, colors[1].w, st.s ), mix(colors[3].w, colors[2].w, st.s ), st.t)  );
	#endif
#else
	...
	#ifdef FACEVARYING_COLOR
		// perform a direct fetch and ignore the colors passed in
...
void main()
{
	...
	vec4 colors[4]; // stub: unused here.  Calculated directly in the emit() function
    ...
    for (int i=0; i < 4; i++) {
        ...
        emit(vertexEmitOrdering[i], n0, uvs, colors);
    }
...
void main()
{
    ...
    uvs[3] = fetchFVarUV(3);
		...
		#ifdef FACEVARYING_COLOR
		colors[0] = fetchFVarColor(0);
		colors[1] = fetchFVarColor(1);
		colors[2] = fetchFVarColor(2);
		colors[3] = fetchFVarColor(3);
		#endif
    #endif
    ...
    for (int i=0; i < 3; i++) {
            ...
            #endif
        #endif
        emit(i, n0, uvs, colors); // emit positions, normals, and uvs
    }
