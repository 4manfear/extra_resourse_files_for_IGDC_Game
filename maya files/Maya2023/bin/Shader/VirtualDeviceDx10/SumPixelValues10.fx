...
Texture2D gSumTexture : InputTexture
<
    string UIName = "Input Texture";
> = NULL;

// Filter input sampler.
sampler gSumSamp = sampler_state 
{
    Texture   = <gSumTexture>;
};
...
// Pixel shader.
float4 PS_SumPixelValues(VS_TO_PS In) : SV_Target
{
    float4 result = 0.0f; 
    result += gSumTexture.Sample(gSumSamp, In.OffsetCoords0);
    result += gSumTexture.Sample(gSumSamp, In.OffsetCoords1);
    result += gSumTexture.Sample(gSumSamp, In.OffsetCoords2);
    result += gSumTexture.Sample(gSumSamp, In.OffsetCoords3);
    return result;
}
...
technique10 Main
{
    pass p0
    {
        SetVertexShader(CompileShader(vs_4_0, VS_SumPixelValues()));
        SetPixelShader(CompileShader(ps_4_0, PS_SumPixelValues()));
    }
}
