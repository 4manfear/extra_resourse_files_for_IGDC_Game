...
texture2D gHDRLatLongTex : InputTexture
<
string UIName = "Input Texture";
> = NULL;

// The HDR LatLong texture sampler.
sampler gHDRLatLongSamp = sampler_state 
{
...
pixelOutput PS_IBLConvolution(VS_TO_PS In)
{
    ...
    float3 N = gDirectionTex.Sample(DirectionSamp, gBlockTexCoord0.xy + floor(In.UV) * gPixelSize.xy).xyz;
    ...
    for( int t = 0; t < 2; ++t)
    {
        ...
        for( int s = 0; s < 4; ++s)
        {
            ...
            L = gDirectionTex.Sample(DirectionSamp, p1);
            float3 color = gHDRLatLongTex.Sample(gHDRLatLongSamp, p1).rgb;
...
technique10 Main
{
    pass p0
    {
        SetVertexShader(CompileShader(vs_4_0, VS_IBLConvolution()));
        SetPixelShader(CompileShader(ps_4_0, PS_IBLConvolution()));
    }
}
