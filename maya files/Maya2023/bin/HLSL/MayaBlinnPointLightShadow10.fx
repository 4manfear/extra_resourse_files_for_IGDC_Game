...
extern float mayaPointLight_lightOn = 1.000000; 
extern float mayaPointLight_intensity = 1.000000; 
extern float3 mayaPointLight_color = { 1.000000, 1.000000, 1.000000 }; 
extern float mayaPointLight_emitsDiffuse = 1.000000; 
extern float mayaPointLight_emitsSpecular = 1.000000; 
// Light position semantic:lightposition0, lightposition1, ...,  lightposition15
// 
extern float3 mayaPointLight_position : lightposition0; 

extern TextureCube mayaShadowPointLight_shadowMap; 
extern sampler shadowSamp; 
extern float mayaShadowPointLight_shadowBias = 0.022200; 
extern int mayaShadowPointLight_shadowMapSize = 512; 
extern float2 mayaShadowPointLight_depthRange = { 0.000000, 0.000000 }; 
extern float mayaShadowPointLight_filterRadius = 1.000000; 
extern float3 mayaShadowPointLight_shadowColor = { 0.000000, 0.000000, 0.000000 }; 
extern bool mayaShadowPointLight_mayaGlobalShadowOn = 0; 
extern bool mayaShadowPointLight_mayaShadowOn = 0; 
...
irradiance mayaPointLight( float lightOn, float intensity, float3 color, float emitDiff, float emitSpec, 
                           float3 lightPosition, float3 Pw)
{ 
    const float epsilon = 0.00001f; 
    float3 L = lightPosition - Pw; 
    float d = length( L ) + epsilon; 
    float atten = 1.0f / pow( d + 1.0f, 0.0 + epsilon); 
    float3 c = lightOn * atten * intensity * color; 
    irradiance irrad; 
...
{		
	...
	float3 lookup = Pw - lightPosition;							
	float Distance = length(lookup);																				
	float depthRange = range.y-range.x;	
	float3 normalizeLookup = normalize(lookup);
																										
	float SampledDistance = (map.Sample(samp, normalizeLookup).r)*depthRange + range.x;	
...
{ 
    ...
    irradiance v_mayaPointLight = mayaPointLight( mayaPointLight_lightOn, mayaPointLight_intensity, mayaPointLight_color, mayaPointLight_emitsDiffuse, mayaPointLight_emitsSpecular, mayaPointLight_position, inputs.Pw ); 
    float3 v_Pw = Pw( inputs.Pw ); 
    s_mayaIrradiance = mayaShadowPointLight( v_mayaPointLight, mayaPointLight_position, v_Pw, mayaShadowPointLight_shadowMap, shadowSamp, mayaShadowPointLight_shadowBias, mayaShadowPointLight_depthRange, mayaShadowPointLight_filterRadius, mayaShadowPointLight_shadowColor, mayaShadowPointLight_mayaGlobalShadowOn, mayaShadowPointLight_mayaShadowOn ); 
